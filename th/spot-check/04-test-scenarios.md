...
> **At a Glance**
> **โมดูล:** [[spot-check]] &nbsp;·&nbsp; **จำนวน scenario รวม:** drill-down ต่อ persona ข้ามทุก persona &nbsp;·&nbsp; **Persona ที่ครอบคลุม:** Inventory Controller, Counter, Audit / Config
...
- **Inventory Controller** — เจ้าของการดำเนินการ; ตาม [[spot-check/03-user-flow-inventory-controller]]
- **Counter** — การป้อนข้อมูลพื้นที่; ตาม [[spot-check/03-user-flow-counter]]
- **Audit / Config** — Auditor + Sysadmin (โดยปริยาย) การสังเกต การ config; ตาม [[spot-check/03-user-flow-audit-config]] หมายเหตุ: การอนุมัติ rollup-adjustment (Approver / Finance) ลงจอดฝั่ง [[inventory-adjustment]] ไม่ใช่บน spot-check โดยตรง
...
| 9 | Flag รายการเสียหายข้าม normal count | Counter → Inventory Controller | Counter พบรายการเสียหายบน sheet | Detail-comment flag พร้อม attachment photo; Inventory Controller review; อาจเพิ่มบรรทัดเองหรืออ้างอิงไป write-off ผ่าน [[inventory-adjustment]] โดยตรง |
...
| 13 | การ post backdate rollup — reject | Inventory Controller | Period ที่บรรจุ rollup adjustment เป็น `closed` ตาม [[inventory]] `INV_VAL_008` | Submit ของ rollup adjustment ถูก reject; Inventory Controller ต้อง escalate ไป Finance Manager เพื่อเปิด period ใหม่หรือรอ |
...
- ที่เกี่ยวข้อง: [[spot-check/03-user-flow]] (matrix handoff ที่หน้านี้ใช้), [[spot-check/02-business-rules]] (`SPC_VAL_*` / `SPC_AUTH_*` / `SPC_POST_*`), [[physical-count/04-test-scenarios]] (scenario คู่เทียบการนับเต็มที่มีโครงสร้าง period สามชั้น), [[inventory-adjustment/04-test-scenarios]] (scenario ฝั่ง rollup)
