...
| HoD route ล้มเหลว | ไม่มี HoD ตั้งค่าสำหรับแผนกของเอกสาร | ตั้งค่า HoD บน [[master-data/department]] |
...
- **Assigned users vs roles** `assigned_users` ยอมรับทั้ง user ID หรือ role descriptor (resolve ผ่าน [[access-control/application-role]]) รายการว่างที่ stage approve ที่ไม่ใช่ HOD = ใครก็ตามที่มี role ของ workflow-stage
...
- [[purchase-request]] — consumer หลัก (`purchase_request_workflow`)
- [[store-requisition]] — ผู้ใช้หลายขั้นแบบมาตรฐาน (`store_requisition_workflow`)
- [[purchase-order]] — การอนุมัติมูลค่าสูง (`purchase_order_workflow`)
- [[good-receive-note]], [[inventory-adjustment]], [[vendor-pricelist]], [[physical-count]], [[spot-check]] — การ gate ด้วยเวิร์กโฟลว์แบบ optional
- [[access-control/application-role]] — Role descriptor ใน `assigned_users`
