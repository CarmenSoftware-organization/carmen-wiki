...
User Activity คือ **timeline forensic ที่เน้น actor** — ทุก login, logout, การ refresh token, การเปลี่ยน password, การเปลี่ยน role และการเปิดหน้าที่ละเอียดอ่อนต่อผู้ใช้ แตกต่างจาก [[reporting-audit/activity]] ซึ่ง **เน้น entity** (หนึ่งแถวต่อการเปลี่ยนแถวธุรกิจ); user-activity เป็นหนึ่งแถวต่อ **event ของ user-action** ไม่ว่าจะมีการเปลี่ยนแถวธุรกิจหรือไม่
...
ดู [[reporting-audit/activity]] สำหรับนิยามตารางเต็ม
...
- [[reporting-audit/activity]] — ตาราง tenant ที่อยู่เบื้องหลัง; user-activity คือ projection ที่กรอง + join เหนือมันบวก session
- [[access-control/user]] — `actor_id` / `user_id` ของ session resolve ผ่าน `tb_user` ของแพลตฟอร์ม
- [[access-control/permission]] — สิทธิ์ view-permission ขับว่าการเปิดหน้าละเอียดอ่อนตัวไหน emit แถว `view`
