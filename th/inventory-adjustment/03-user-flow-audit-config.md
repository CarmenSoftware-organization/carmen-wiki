...
- Lot-recall trace รวมเอกสาร adjustment กับการรับ [[good-receive-note]] และ data การบริโภค / ผลต่างของ [[store-requisition]] / [[physical-count]] ผ่าน `tb_inventory_transaction` join ร่วม
...
2. **Forward trace** การเคลื่อนไหวทั้งหมดที่บริโภคจาก lot: SR issues (ผ่าน [[store-requisition]] join), stock-out write-offs (`tb_stock_out_detail` join ผ่าน `current_lot_no` บนฝั่ง inventory), การปรับปริมาณ credit-note, transfer-outs
...
- ที่เกี่ยวข้อง: [[inventory]] — `INV_AUTH_008` (scope การกำหนดค่า Sysadmin span นิยาม location-type / costing-method / period นอกเหนือจาก adjustment-type), `INV_AUTH_009` (scope การอ่าน Auditor span ข้อมูล inventory ทั้งหมด)
- ที่เกี่ยวข้อง: [[good-receive-note]] — lot-recall trace ของ Auditor join adjustments กับการรับ GRN ผ่าน inventory transaction ที่แชร์; รูปแบบ Auditor flow สะท้อน [[good-receive-note/03-user-flow-audit-config]]
- ที่เกี่ยวข้อง: [[physical-count]] / [[spot-check]] — Auditor review variance-rollup auto-posts (ริเริ่มโดย `ADJ_POST_006` บน count commit) สำหรับความสมเหตุสมผลและ SoD compliance
