...
|---|---|---|
| **PO Pipeline** | 6 stage: Not Sent / Sent / Partial / Closed / Completed / Rejected — จำนวน + progress bar | → [[purchase-order]] กรองตาม status |
| **PR ค้างสำหรับการสร้าง PO** | PR ID, Requester, Dept, Date Approved, badge (`Pending PO` / `Hold for Info` / `Overdue Follow-up`) | → [[purchase-request]] |
| **การส่งของล่าช้า** | PO ID, Vendor, Dept, Due Date, "ล่าช้า N วัน" (สี destructive) | → [[purchase-order]] |
...
| **5 ผู้ขายสูงสุดตามค่าใช้จ่าย** | bar แนวนอน + label จำนวน | → [[vendor-pricelist]] |
| **ตารางส่งของ** | 3 stat card: จำนวนวันนี้ / สัปดาห์นี้ / สัปดาห์หน้า | → [[good-receive-note]] expected วันนี้ |
| **PO ที่รับเกิน** | PO #, Material, Code, Vendor, Ordered, Received, Variance `+N`, badge "Variance Flagged" | → [[good-receive-note]] |
...
| คำถาม | คำตอบ |
|---|---|
| ทำไมมาตรวัดถึง static? | **ยังเป็น mock data** — On-Time / Completeness จะ resolve ไปยังรายงาน query dataset จาก [[reporting-audit]] |
| PR ใดบ้างขึ้นใน "Pending PRs for PO Creation"? | [[purchase-request]] ที่ `workflow_current_stage = "approved"` AND `po_id IS NULL` เกณฑ์ SLA ขับเคลื่อนชนิด badge |
| อะไร flag PO เป็น "Overdue"? | `expected_delivery_date < CURRENT_DATE` AND ยังรับไม่ครบเทียบ [[good-receive-note]] |
| Over-Received variance reconcile ที่ไหน? | join PO line → committed GRN line ที่ `received_qty > ordered_qty`; ดู [[costing]] สำหรับการ post variance |
...
| แถว Over-Received หายไปหลัง reload | mock array re-import ต่อ render — เสถียรในปัจจุบัน; ไม่เสถียรเมื่อ live | ตรวจสอบกับ commit log ของ [[good-receive-note]] เมื่อ wire แล้ว |
| Top Vendors bar แสดงชื่อ vendor ซ้ำ | Mock fixture quirk — `vendor_id` ไม่ถูก de-dup | live query group ตาม `vendor_id` และ join [[vendor-pricelist]] |

---

## 5. แหล่งข้อมูล (Dev)

- **Pipeline bucket** — group-count บน [[purchase-order]] โดย `status` → 6 ค่า `PoPipelineKey`
- **Pending PR สำหรับ PO** — [[purchase-request]] ที่ `workflow_current_stage = "approved"` AND `po_id IS NULL`; badge ตาม SLA (`Pending PO` < N วัน, `Overdue Follow-up` ≥ N วัน)
- **Overdue Deliveries** — [[purchase-order]] line ที่ `expected_delivery_date < CURRENT_DATE` AND ยังรับไม่ครบเทียบ [[good-receive-note]]
- **On-Time Delivery / Order Completeness** — KPI ตามช่วงเวลาจาก [[reporting-audit]] บน GRN ที่ commit เทียบกับวัน/จำนวนคาดหวังของ PO
- **ค่าใช้จ่ายตามหมวด** — sum จำนวน PO line จัดกลุ่มตาม [[product/category]] เดือนปัจจุบัน vs YTD
...
- **Over-Received POs** — join [[purchase-order]] line กับ [[good-receive-note]] line ที่ commit ซึ่ง `received_qty > ordered_qty`
...
- [[purchase-order]] — ระบบบันทึก transactional
- [[purchase-request]] — แหล่งต้นน้ำสำหรับ Pending PRs for PO Creation
- [[good-receive-note]] — การรับของปลายน้ำที่ขับ On-Time, Completeness, Over-Received
- [[vendor-pricelist]] — vendor master เบื้องหลัง Top Vendors bar
- [[reporting-audit]] — query dataset สำหรับมาตรวัด KPI
- [[costing]] — การจัดการ variance สำหรับจำนวนรับเกิน
