...
> **At a Glance**
> **Gate:** ทุก route ที่ถูก guard มี `requiredPermission="resource.action"` (หรือ `requireSuperAdmin`) บน `PrivateRoute` &nbsp;·&nbsp; **ลำดับการ resolve:** bootstrap → super-admin → key ระดับแพลตฟอร์ม → key ระดับ cluster &nbsp;·&nbsp; **ข้อยกเว้น bootstrap:** จำนวนผู้ใช้รวม 0 หรือ 1 ⇒ login ข้าม gate ≥1-permission และ `hasPermission` คืน `true` &nbsp;·&nbsp; **เมื่อไม่ผ่าน:** `<AccessDenied>` ภายใน `<Layout>` (sidebar ยังมองเห็นอยู่) &nbsp;·&nbsp; **Gate ภายในหน้า:** `<Can>` ห่อ Add/Edit/Delete บนหน้า list และ edit ของ management ส่วนใหญ่ (key `.create`/`.update`/`.delete` แบบ cluster-scoped สำหรับ Clusters/Business Units); ภายในหน้าจอของโมดูล RBAC เองมีเพียงหน้า detail ของ User Platform ที่ใช้ (`user_platform.manage`)
...
|---|---|---|---|
| 1 | Login ตอน bootstrap — แพลตฟอร์มมีผู้ใช้รวม 0 หรือ 1 คน | `login()` รับ session ที่มี permission เป็นศูนย์; ทุก `hasPermission` คืน `true` ดังนั้นทุก route, รายการ sidebar และ block `<Can>` เปิดหมด | เส้นทางที่ตั้งใจไว้สำหรับ admin คนแรก: sign in, สร้าง role, assign สงบลงทันทีที่มี row ผู้ใช้คนที่สอง; session ที่ค้างอยู่จะไม่ตรวจสอบ count ซ้ำจนกว่าจะ refresh/login |
...
| 9 | dev build ที่ response ของ permission ว่างเปล่า | `DEV_MOCK_EFFECTIVE_PERMISSIONS` (key ของ platform-management ทั้งหมด, `is_super_admin: false`) ถูกแทนที่เฉพาะใน `import.meta.env.DEV` | ไม่เคยทำงานใน production build; อย่าตีความการเข้าถึงในโหมด dev ว่าเป็น grant |
