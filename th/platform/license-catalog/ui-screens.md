...
**คอลัมน์:** ลำดับ (ชิปตัวเลขเล็ก ๆ ขอบสีเตือนเมื่อ bundle อื่นถือเลขเดียวกัน — คำนวณจาก**ชุดที่ไม่ผ่านการกรองทั้งหมด** เพราะ bundle ที่ถูกกรองออกจากมุมมองก็ยังแย่งลำดับเดียวกันบนฟอร์มขายอยู่ดี) · code (ลิงก์เข้า `LicenseFeatureGroupEdit` ตัวพิมพ์ monospace) · ชื่อ + คำอธิบาย (เซลล์เดียวกัน คำอธิบายตัดให้เหลือบรรทัดเดียว; ตั้งแต่ PR #287 จะต่อท้าย badge **Interface** เมื่อ `kind === 'interface'` โดยอ่านเป็น `kind ?? 'standard'` ดังนั้น gateway ก่อน 2026-09-10 ก็แค่ไม่แสดง badge) · **Features** (จำนวนบวกแถบสัดส่วนที่ใช้ตัวหารร่วมทั้งหน้าเมื่อรู้ค่าตัวหาร — ดู §6 ว่าตัวหารนั้นพึ่งพาอะไรจริง ๆ) · จำนวนสัญญา (ข้อความสรุปเฉย ๆ บนหน้านี้ ตัวเลขที่จะกลายเป็นคำเตือนก็ต่อเมื่อมีคนพยายามลบหรือปิดการขาย bundle นั้น §4.1) · badge active/inactive · เมนูต่อแถว (Edit / Delete)
