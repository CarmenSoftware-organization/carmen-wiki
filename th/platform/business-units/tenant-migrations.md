...
description: หน้าจอ standalone ระดับ fleet (/tenant-migrations) ที่ตรวจสอบและ apply schema migration ของฐานข้อมูล tenant ที่ค้างอยู่ทั่วทุก business unit — reuse cluster.read (บวก feature flag tenant_migrations) ที่ระดับ route แต่ทุก action จริงถูก gate เพิ่มเป็น super-admin เท่านั้น
published: true
...
> **At a Glance**
> **หน้าจอ:** `TenantMigrationManagement` (`/tenant-migrations`, เพิ่มเมื่อ 2026-06-30) &nbsp;·&nbsp; **Route gate:** `cluster.read` (reuse key เดียวกับ [Business Units](/th/platform/business-units)) **บวก feature flag `tenant_migrations`** บน `PrivateRoute` ตรวจหลัง permission gate — ใหม่นับตั้งแต่ sync ก่อนหน้า เป็น pattern `feature=` เดียวกับที่ทุกโมดูลในเล่มนี้มีแล้ว &nbsp;·&nbsp; **Action gate:** ทุก action Check/Apply/Deploy ถูกจำกัดเพิ่มเป็น `isSuperAdmin` — ทั้งฝั่ง client และที่ backend guard &nbsp;·&nbsp; **Sidebar:** รายการ "Tenant Migrations" ในกลุ่ม Organization (`src/components/nav/platformNav.ts` — นิยาม nav item อยู่ที่นี่แล้ว ไม่ใช่ `Layout.tsx`) &nbsp;·&nbsp; **ความสัมพันธ์:** มุมมองรายการระดับ fleet ของ action เดียวกับที่การ์ด `TenantMigrationCard` ต่อ BU บน [Business Units — UI Screens](/th/platform/business-units/ui-screens) §4.5 ทำทีละ BU

## 1. ภาพรวม

Tenant Migrations เป็นหน้า standalone ไม่ใช่ sub-route ของ Business Units — แต่ทำงานกับ object โดเมนเดียวกันทุกประการ (ทุกแถว `business_unit` ที่ active) และความสามารถ backend เดียวกับการ์ด **Tenant Migrations** ที่บันทึกไว้แล้วบน[หน้าแก้ไข Business Units](/th/platform/business-units/ui-screens) §4.5 การ์ดนั้นตรวจสอบและ apply migration ให้กับ BU หนึ่งตัวที่กำลังแก้ไขอยู่ ส่วนหน้านี้ (`TenantMigrationManagement.tsx` เข้าถึงจากกลุ่ม Organization ใน sidebar ไม่ใช่จากหน้าจอ Business Units ใด ๆ) แสดงรายชื่อ business unit **ทั้งหมด** ในตารางเดียว ให้ operator ตรวจสอบ apply หรือ deploy แบบ batch ได้ทั่วทั้ง fleet จากมุมมองเดียว

หน้านี้โหลดรายการแถวจาก endpoint `businessUnitService.getAll()` ปกติ (endpoint เดียวกับที่หน้ารายการ Business Units ใช้) ดังนั้น **การเข้าถึงหน้านี้และเห็นตาราง BU ต้องมี `cluster.read` บวก feature flag `tenant_migrations`** — pattern การ reuse route เดียวกับที่ Business Units ใช้กับสาม route ของตัวเอง (ดู [Business Units — บทบาทและ Persona](/th/platform/business-units) §4) แต่ทุก operation ของ migration จริง ๆ เป็นอีกเรื่องหนึ่ง: ทั้ง frontend และ backend จำกัด Check/Apply/Deploy-all ไว้ให้เฉพาะ session ระดับ super-admin เท่านั้น (§3) การแบ่งนี้ — สิทธิ์อ่านกว้างสำหรับรายการ สิทธิ์เขียนแคบสำหรับ action — คล้ายกับการ gate ของการ์ดต่อ BU เอง (§4.5 ที่นั่นระบุว่า "ทุกคนที่แก้ไขได้เห็นการ์ด action ถูก gate ด้วย super-admin") ต่างกันตรงที่ที่นี่แม้แต่ **การตรวจสอบสถานะแบบอ่านอย่างเดียว** ก็อยู่ในข้อจำกัดนี้ด้วย เพราะต่างจากหน้าแก้ไข BU (ที่แสดงการ์ดสำหรับ BU เดียวที่ scope ด้วย `cluster.update` อยู่แล้ว) จุดประสงค์ทั้งหมดของหน้านี้คือสถานะ migration ระดับ fleet ซึ่ง backend ถือเป็นความสามารถเดียวที่สงวนไว้สำหรับ super-admin เท่านั้น ไม่ว่าจะถามถึง BU ไหนก็ตาม
...
- **การ์ดสรุป Fleet Sync** (`FleetSync`) — การ์ดแนวนอนแสดง `{จำนวนที่ in-sync} / {BU ทั้งหมด}` ด้วยตัวเลขขนาดใหญ่แบบ monospace, progress bar สามสี (เขียว/เหลือง/แดง) สำหรับ in-sync/behind/errored และแถว legend พร้อมจำนวน ก่อนรัน "Check all" ครั้งแรกจะแสดง "Not checked yet" แทน bar ช่อง action ทางขวาของการ์ดมีปุ่ม **Check all**, **Deploy all**, และ **Export**
...
- **ตาราง BU** (`DataTable`, 3 คอลัมน์ sticky ซ้าย) — คอลัมน์: **Code** (ลิงก์ไป `/business-units/:id/edit`), **Name**, **Status** (badge — "In sync" / "N behind" / "Error" / "Not checked" บวกบรรทัด inline "Applying X/Y … ชื่อ migration ปัจจุบัน" ระหว่างที่ Apply ต่อแถวกำลัง stream หรือ error message inline), **Last checked** (HH:MM:SS ตามเวลา client ตรึงความกว้างไว้ที่ `w-28` นับตั้งแต่ sync ก่อนหน้า) และคอลัมน์ row-actions (ไอคอน **Check** และ **Apply** ที่แสดงเฉพาะเมื่อแถวนั้นมี migration ค้างอยู่) **คอลัมน์ Pending บนหน้าจอถูกถอดออกแล้ว** นับตั้งแต่ sync ก่อนหน้า — badge Status พูดจำนวนอยู่แล้ว ("3 behind" / "In sync") ไฟล์ CSV export (ด้านล่าง) ยังมีคอลัมน์ Pending อยู่ เพราะที่นั่นตัวเลขเปล่า ๆ มีประโยชน์จริงสำหรับคำนวณต่อ
...
**ป้ายของปุ่ม Deploy all เปลี่ยนไปนับตั้งแต่ sync ก่อนหน้า** เดิมปุ่มนี้เป็นสีแดงแบบ destructive ตลอดเวลาไม่ว่าสถานะจะเป็นอย่างไร — แดงตั้งแต่ยังไม่ได้กด "Check all" (หน้ายังไม่รู้ว่าจะไปแตะอะไร) และยังแดงอยู่แม้ pending จะเป็นศูนย์ ตอนนี้ปุ่มใช้ `variant={behindCount > 0 ? 'destructive' : 'outline'}` — เป็นสีแดงเฉพาะเมื่อมี BU ที่ behind มากกว่าศูนย์ พร้อมป้ายที่บอกรัศมีของตัวเอง ("Deploy 3 behind" แทน "Deploy all" เฉย ๆ) และถูก disable (ไม่ใช่แค่จางลง) เมื่อตรวจแล้วพบว่าไม่มีอะไรให้ deploy เหตุผลจาก comment ของปุ่มเองคือโดเมนนี้ไม่มี endpoint cancel/rollback ปุ่มที่ดูเร่งด่วนที่สุดบนจอจึงไม่ควรเป็นปุ่มที่ย้อนกลับไม่ได้มากที่สุด
...
- `../carmen-platform/src/components/TenantMigrationCard.tsx` — การ์ดต่อ BU ที่บันทึกไว้ใน [Business Units — UI Screens](/th/platform/business-units/ui-screens) §4.5 ใช้ service เดียวกัน
- `../carmen-platform/src/App.tsx:251` — route `/tenant-migrations` (`requiredPermission="cluster.read"` บวก `feature="tenant_migrations"`); `src/components/nav/platformNav.ts` — รายการ sidebar (นิยาม nav item อยู่ที่นี่แล้ว ไม่ใช่ `Layout.tsx`)
...
- [Business Units](/th/platform/business-units) และ [Business Units — UI Screens](/th/platform/business-units/ui-screens) §4.5 — การ์ดต่อ BU และ gotcha การ reuse key `cluster.*` ที่หน้านี้สืบทอดมา
