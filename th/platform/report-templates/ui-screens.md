...
- Gate: route guard `report_template.read` / `.create` / `.update` (**และ** `feature="report_templates"`); Add Template (ทั้ง header **และ** empty-state — ไม่มีตัวใดที่ไม่ถูก gate), Edit/Delete ของ row และ toggle Edit ห่อด้วย `<Can>` (session ที่ถือแค่ `report_template.read` เห็น dropdown ของ row ว่างเปล่า)
...
- **เพิ่มเมื่อ 2026-07-23 (เดิมชื่อ `kind`):** field `Template Type` (List/Form, จำเป็น) เป็น field แรกของการ์ด Template Info; เมื่อเป็น Form จะโชว์ checkbox `is_default` และ `report_group` แบบ select จากรายการคงที่ `FORM_REPORT_GROUPS` แทน input อิสระ (และบังคับ `is_standard = true` ตอน submit); chip input ของ BU ถูก disable และล้างค่าสำหรับเทมเพลตแบบ Form; หน้ารายการมีคอลัมน์ Template Type ใหม่และ filter ที่สาม (Template Type); default sort เปลี่ยนเป็น `name:asc`
- Not-found gating และ `doc_version` optimistic lock เพิ่มมาที่หน้า edit
- **เพิ่มเมื่อ 2026-08-22:** การ์ด "Metadata" เดิมถูกลบออก — audit ("Created/Updated ... by ...") ย้ายไปแสดงเป็นบรรทัดเดียวใน `PageHeader` แทน (`normalizeAudit()` + `AuditMeta variant="header"`); คอลัมน์ Created/Updated ของหน้ารายการก็เปลี่ยนไปใช้ `auditColumns()`/`AuditMeta` ร่วมกันแบบเดียวกัน (relative time + tooltip แบบ absolute)
- **เพิ่มเมื่อ 2026-08-31 เป็นต้นไป (ข้ามโมดูล):** action **View History** ปรากฏทั้งใน dropdown ของแถวใน list และในแถว `actions` ของ header หน้า edit gate ด้วย `activity_log.read` พร้อม `clusterId={PLATFORM_SCOPED_RECORD}`

## 2. References

- ../carmen-platform/src/pages/ReportTemplateEdit.tsx
- ../carmen-platform/src/pages/ReportTemplateManagement.tsx
- ../carmen-platform/src/components/nav/platformNav.ts (รายการ sidebar — ไม่ใช่ Layout.tsx)
