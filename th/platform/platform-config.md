...
| **Expiry Thresholds** (`expiry_thresholds`) | `subscription_days`, `bu_quota_days`, `seat_days`, `interface_days` (ตัวที่สี่ เพิ่มเมื่อ 2026-09-09 พร้อมการแยก interface licence — `platform-config.schema.ts:164`) — ทั้งหมด `30`, 1–365 | เป็นเกณฑ์**แสดงผล/นับ**ล้วน ๆ — ขึ้นหรือลดค่าเปลี่ยนแค่ว่า badge "ใกล้หมดอายุ" เริ่มขึ้นเมื่อไหร่บนหน้า [Licenses](/th/platform/licenses) และ Clusters และตัวเลขในแถบสรุปของ `expiring_soon` รวมอะไรบ้าง **ไม่มีใครถูกบล็อกเพิ่มหรือลดจากการ์ดนี้** — มันอยู่หัวข้อเดียวกับ License Enforcement เพราะทั้งคู่เกี่ยวกับวงจรชีวิตของ license เท่านั้น ไม่ใช่เพราะใช้กลไกร่วมกัน สามกระบวนการอ่านค่านี้แยกกัน (รายการ/ตัวนับ cluster ของ micro-cluster, สรุป subscription ของ micro-business และ `ExpiryThresholdContext` ของ frontend เอง ซึ่งเรียก `refresh()` ทันทีหลังบันทึกให้ `/licenses` กับ `/clusters` สะท้อนหน้าต่างใหม่โดยไม่ต้องรีโหลดทั้งแอป) แต่ละที่มี cache 60 วินาทีของตัวเอง | `ExpiryThresholdsService` ใน micro-cluster และ micro-business; `ExpiryThresholdContext.tsx` ฝั่ง frontend (endpoint สาธารณะ ไม่มี permission — ดู §4.1) |
| **Platform Migration** (`platform_migration`) | `api_enabled` (`false`) | สวิตช์เปิด/ปิด `/api-system/platform/migrations/*` ซึ่งรัน `prisma migrate deploy` บน**ฐานข้อมูลกลางที่ทุก cluster ใช้ร่วมกัน** ไม่ใช่ของ BU ใด BU หนึ่ง เคยเป็น env `PLATFORM_MIGRATION_API_ENABLED` การย้ายมาที่นี่ทำให้**ตัวคีย์ config เอง**ต้องเป็น super-admin เท่านั้นจึงจะเขียนได้ (§4.3) — แต่นั่นไม่ใช่เรื่องเดียวกับการบอกว่าเส้นทางที่เครื่องเข้าถึงได้ของฟีเจอร์นี้หายไป: endpoint ที่สวิตช์นี้คุมอยู่ `/api-system/platform/migrations/*` ถูกอนุญาตโดย `PlatformMigrationGuard.canActivate()` ซึ่งรับ**อย่างใดอย่างหนึ่ง**ระหว่าง header `x-deploy-token` ที่ตรงกัน (ตรวจ**ก่อน**เสมอ ระบุ `deployActor: 'ci:deploy-token'`) **หรือ** session ของ super-admin (เป็นทางสำรอง ไปถึงได้ก็ต่อเมื่อไม่มี token มา ระบุ `deployActor: 'super-admin:<user_id>'`) — เส้นทางเครื่องไม่เคยถูกถอดออกเลย มีแค่สวิตช์เปิด/ปิดตัวมันเองเท่านั้นที่ตอนนี้เป็นของมนุษย์เท่านั้นที่แก้ได้ `PlatformMigrationGuard` ยัง cache สวิตช์นี้ไว้ 60 วินาทีด้วย ตามคำเตือนในการ์ด — เพิ่งเปิดสวิตช์ยังอาจได้ 403 อีกไม่เกินหนึ่งนาที ซึ่งเป็นพฤติกรรมที่ถูกต้อง ไม่ใช่บั๊ก | `platform-migration.guard.ts` (backend-gateway) รูปแบบ cache 60 วิเดียวกับ License Enforcement |
