...
**ตัว enum เองไม่เคยถูกลบ** `enum enum_email_sender_purpose` ยังคงประกาศอยู่ใน schema (`schema.prisma:1415-1425`) และไม่มีคอลัมน์ใดอ้างถึงมันเลย — ค้นทั่ว repository หาชื่อ type นี้พบแค่การประกาศของมันเอง มันยังไม่ใช่ enum สามค่าดั้งเดิมด้วย: migration `20260808120000_email_sender_purpose_per_flow` — หนึ่งชั่วโมง**ก่อน**การปรับดีไซน์ `20260808130000_email_profile_master_and_routing` ด้านบน ในวันเดียวกัน — เพิ่มชื่อห้าเส้นทางอีเมล (`register`, `verify_email`, `invitation`, `forgot_password`, `notification`) เข้าไปใน enum type **เดียวกัน**นี้ ตอนนี้มันจึงมีแปดค่าคาบเกี่ยวกันสองยุคของดีไซน์ ที่ไม่มีอะไรอ้างถึงเลย คอมเมนต์ของ migration ตัวก่อนหน้านั้นบอกกฎ fallback ได้ถูกต้อง*ณ ตอนที่มันรัน*: "เส้นทางที่ยังไม่มีโปรไฟล์ของตัวเองจะถอยไปใช้ `no_reply` ในโค้ด" — ถูกต้องสำหรับหนึ่งชั่วโมงระหว่างสอง migration และกลายเป็นเท็จหนึ่งชั่วโมงถัดมาเมื่อ `20260808130000` แทนที่ fallback ตายตัว `no_reply` ด้วย `default` ที่ผู้ดูแลตั้งเองใน `email_routing` (§3)

**คอมเมนต์อีกสองจุดถือคำกล่าวอ้างแบบก่อนปรับดีไซน์เดียวกันนี้ ในไฟล์ที่หน้านี้อ้างถึงที่อื่นอยู่แล้ว** คอมเมนต์ของ Prisma model เหนือ `tb_email_sender_profile` โดยตรง (`schema.prisma:1428-1431`) ยังเขียนว่า "One active row per purpose... so `no_reply` can live on a different provider than support" — โมเดลหนึ่ง-purpose-ต่อ-หนึ่งแถวที่ migration คู่นี้แทนที่ไปแล้ว JSDoc ระดับ class ของ `EmailSenderProfileService` เอง (`email-sender-profile.service.ts:58-60`, micro-cluster) กล่าวอ้างเดียวกันด้วยคำต่างกัน: "CRUD with secret encryption, masking, and one-non-deleted-row-per-purpose enforcement" ทั้งคู่เป็นเท็จเมื่อเทียบกับโค้ดข้างใต้วันนี้ — `create()`/`update()` ของ service เอง (§5) บังคับความไม่ซ้ำที่ `name` ไม่ใช่ `purpose` และ `purpose` ไม่มีเป็นคอลัมน์อีกต่อไปแล้ว อย่าใช้คอมเมนต์ทั้งสามนี้ — ของ enum ที่ถูกทิ้งร้าง, ของตาราง, หรือของ service class — เป็นคำอ้างอิงพฤติกรรม fallback หรือความไม่ซ้ำในปัจจุบัน ทั้งสามบรรยายสถาปัตยกรรมที่ migration คู่ 2026-08-08 แทนที่ไปแล้ว
...
- **ทดสอบสาม session ที่ตารางใน §4.3 ของหน้า landing ระบุไว้ ไม่ใช่แค่ "`email_setting.*` เพียงอย่างเดียว"** `email_setting.*` โดยไม่มี `platform_config.*` เลย สร้างแค่ error ฝั่งอ่านเท่านั้น — ปุ่ม Edit Routing กั้นด้วย `!routingError` ดังนั้น `GET` ที่ล้มเหลวแปลว่าปุ่มไม่ render เลย และกับดักฝั่งเขียนไปไม่ถึงจากการให้สิทธิ์ชุดนั้น กับดักฝั่งเขียนต้องการ `platform_config.read` มีอยู่ (ผังจึงโหลดและปุ่มปรากฏ) พร้อมกับ `platform_config.manage` ไม่มี — ชุดที่แคบกว่าและต่างออกไป (หน้า landing §4.3) ส่วนคำถามว่า deployment จริงมี role ที่ seed `platform_config.read` ไว้โดยไม่มี `.manage` หรือไม่ ไม่ได้ถูกยืนยันจากซอร์สโค้ด และถูกระบุไว้ตรงนั้นเป็นคำถามเปิด ไม่ใช่ข้อสันนิษฐาน
...
- `../carmen-turborepo-backend-v2/packages/prisma-shared-schema-platform/prisma/migrations/20260808120000_email_sender_purpose_per_flow/migration.sql` — เพิ่มค่าห้าชื่อเส้นทางเข้าไปใน `enum_email_sender_purpose` หนึ่งชั่วโมงก่อนการปรับดีไซน์ด้านล่าง (§2.1)
...
- `../carmen-turborepo-backend-v2/apps/micro-cluster/src/cluster/email-sender-profile/email-sender-profile.service.ts` (บรรทัด 58-60) — CRUD, การบังคับใช้ `doc_version`, `passwordPatch()`, ความไม่ซ้ำของชื่อ (§5), และ JSDoc ระดับ class ที่ล้าสมัย (§2.1)
