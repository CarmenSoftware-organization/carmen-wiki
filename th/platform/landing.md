...
- **กลุ่ม Platform ขาดไป 4 จาก 7 แถวปัจจุบัน**: User Platform (ส่วนหนึ่งของ [Platform RBAC](/th/platform/rbac)), SQL Workbench, และอีกสองแถวที่เพิ่มหลังจากหน้านี้ทบทวนครั้งล่าสุด — Platform Config กับ Email Settings (ทั้งคู่อยู่ `navGroup.platform`) — บวก Feature Flags (ตั้งใจไม่ gate ตามแถว nav ของมันเอง — ดู [SQL Workbench](/th/platform/sql-workbench) §4 สำหรับ pattern "ตั้งใจไม่ gate" แบบเดียวกันที่ใช้กับคีย์อื่น)
