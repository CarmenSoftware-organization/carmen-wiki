...
**ฝั่งเขียน** `EmailRoutingCard.handleSave()` เรียก `platformConfigService.update('email_routing', payload)` → `PUT /api-system/platform/configs/email_routing` (`EmailRoutingCard.tsx:124`) ด่านฝั่ง **frontend** ที่จะเข้าถึงปุ่มนี้ได้เลยคือ `canManage` — `hasPermission('email_setting.manage')` คำนวณครั้งเดียวที่หน้า (`EmailSettingManagement.tsx:48`) แล้วส่งลงมาเป็นเงื่อนไขการแสดงผลของ action ใน `SectionHeading` (`EmailSettingManagement.tsx:140-154`); `EmailRoutingCard` เองไม่มีการตรวจ permission ของตัวเองเลย เชื่อใจหน้าทั้งหมด ส่วน endpoint ฝั่ง **backend** ที่การบันทึกนั้นเรียกจริงต้องมี `platform_config.manage` (`platform_configs.controller.ts:236-238`, decorator `@RequirePlatformPermission` ของ route `PUT` ที่บังคับโดย `PlatformPermissionGuard` *ก่อน*ตัว handler จะทำงาน) — และ `writeKeyDenial()` ซึ่งเป็นด่านที่สองรายคีย์ของ handler เอง ไม่ได้เพิ่มอะไรสำหรับ `email_routing` เลย มันเข้มขึ้นเฉพาะ `license` กับ `platform_migration` เท่านั้น (บันทึกไว้ที่ [Platform Config](/th/platform/platform-config) §3.3) `email_routing` ไม่ใช่หนึ่งในสองคีย์นั้น ดังนั้น `platform_config.manage` เพียงอย่างเดียวคือสิ่งที่ endpoint ตรวจ

**สองคีย์นี้ไม่ตรงกัน ในทิศทางที่สร้างกับดักที่ดูใช้งานได้จริง** session ที่มี `email_setting.manage` แต่**ไม่มี** `platform_config.manage` จะเห็นปุ่ม "Edit Routing" render เต็มรูปแบบ กดได้ (ด่าน frontend ผ่าน) เปิดตัวแก้ไข สลับปลายทางของทุก flow chip ได้ และกด Save ได้ — แล้วคำขอจะโดน 403 ถูกปฏิเสธโดย `PlatformPermissionGuard` ก่อนที่ `writeKeyDenial()` หรือโค้ดแอปพลิเคชันใด ๆ จะทำงานเลย เพราะ resource ที่ decorator ของ endpoint ระบุคือ `platform_config` ไม่ใช่ `email_setting` ไม่มีชุด permission ใดในทิศทางตรงข้ามที่ทำแบบนี้ได้: session ที่มีแค่ `platform_config.*` และไม่มี `email_setting.*` เลย เข้า `/platform/email-settings` ไม่ได้ตั้งแต่แรก เพราะตัว route เองต้องการ `email_setting.read` (`App.tsx:493`) — ช่องว่างนี้จึงเกิดทางเดียว และกระทบทั้งฝั่งอ่าน (error ค้างถาวร ไม่ใช่กับดัก 403) พอ ๆ กับฝั่งเขียน (ตัวแก้ไขที่ดูใช้งานได้แต่บันทึกไม่ได้)

**นี่ยืนยัน ไม่ใช่ขัดแย้ง กับสิ่งที่โมดูล Platform Config เองยกธงไว้ล่วงหน้า** โมดูลนั้นบรรยายรูปแบบเดียวกันนี้จากฝั่งของมันเอง (§3.5 ของหน้า landing) และตั้งใจปล่อยการตรวจสอบไว้ให้งานนี้ทำ เมื่ออ่านทั้งสองฝั่งอย่างเป็นอิสระ ทั้งสองตรงกัน: `email_setting.manage` จำเป็นแต่ไม่พอที่จะบันทึก routing map จากหน้าจอนี้ และ — รายละเอียดที่หน้า Platform Config ไม่จำเป็นต้องพูด เพราะเขียนจากฝั่งเขียนอย่างเดียว — `email_setting.read` ก็ไม่พอที่จะแค่*ดู*มันโดยไม่มี error ด้วยซ้ำ

### 4.3 สิ่งที่หมายความสำหรับผู้ทดสอบ

ให้ปฏิบัติต่อ "จัดการ Email Settings ได้" กับ "จัดการ Platform Config ได้" เป็น RBAC resource สองอย่างที่ deployment จริงอาจให้แยกกันได้ ตาม source map เองที่ระบุว่าโปรดักต์นี้ไม่มี static permission-key catalog นอกเหนือจาก literal ที่จุดเรียก บทบาทที่สร้างมาให้เจ้าหน้าที่ support จัดการโปรไฟล์ผู้ส่งและทดสอบมันได้ โดยไม่ให้เข้าหน้า Platform Config ที่กว้างกว่า จะทำให้เจ้าหน้าที่คนนั้นเห็นผัง routing ที่ดูพัง (error การโหลดค้างถาวร) และถ้ากดปุ่มนั้นได้จริง ก็จะเจอความล้มเหลวการเขียนที่เงียบ ๆ การให้ `platform_config.read`/`.manage` ควบคู่ไปกับ `email_setting.read`/`.manage` คือชุดเดียวที่ทำให้หน้าจอทั้งหน้าทำงานตามที่ออกแบบไว้
