...
**ไม่มีในรายการนี้:** แถวเมนูสองแถวไม่มี feature key เลย จึงไม่มีบรรทัดในตารางนี้: **Feature Flags** เอง (`/platform/features` gate ด้วย permission เท่านั้น §4 ของหน้า landing) และการตรวจ superAdminOnly ระดับ nav ของ **Super Admins** เป็นอิสระจากคีย์ `feature: 'super_admins'` ของมัน (แถวที่ 20) — คีย์นั้นยังมีอยู่และยัง gate แถวผ่าน `hide`/`inactive` ได้ปกติ `superAdminOnly` เป็นด่านที่สองแยกต่างหากซ้อนทับอยู่บนมันเท่านั้น
