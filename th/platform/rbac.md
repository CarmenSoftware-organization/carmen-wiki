...
SPA ยังจัดกลุ่มสี่หน้าจอไว้ใต้ระบบควบคุมการเข้าถึงนี้ในกลุ่ม **Platform** ของ sidebar หน้านี้ครอบคลุมสองหน้าจอ — **Roles** และ **Permission Catalog** — อย่างครบถ้วน ประกอบกันเป็น pipeline เดียวตั้งแต่การนิยาม key ไปจนถึงการบังคับใช้การเข้าถึง ส่วน **Super Admins** และ **User Platform** สรุปไว้ด้านล่างและใน [UI Screens](/th/platform/rbac/ui-screens) เท่านั้น — ทั้งคู่มีแผนแยกเป็นโมดูล Platform ระดับบนสุดของตัวเองในแผน resync ปัจจุบัน (ดูหมายเหตุขอบเขตด้านบน) การตรวจสอบแบบละเอียด — หน้าจอ, คอลัมน์, endpoint — จึงเป็นของ task นั้น ไม่ใช่ของหน้านี้

ครอบคลุมอย่างครบถ้วนในหน้านี้:

- **Permission Catalog (`/platform/category-permissions` → `PermissionCatalog`)** — หน้าอ้างอิงแบบ read-only ของ permission key ทุกตัวที่ backend กำหนด จัดกลุ่มตาม resource **ไม่มีรายการ nav และไม่มี permission gate ที่ระดับ route ของ SPA เลย** — `App.tsx` ห่อ route นี้ด้วย `<PrivateRoute>` เปล่า ๆ ไม่มี `requiredPermission`/`feature` ผู้ใช้ platform ที่ login แล้วคนใดก็ตามที่รู้หรือได้รับ URL จึงเปิดหน้านี้ได้ อย่าตีความว่า "ข้อมูลไม่มีการป้องกัน" เพราะการดึงข้อมูลของหน้านี้เอง (`GET /api-system/platform/permissions`) ถูกบังคับที่ฝั่ง backend ด้วย `platform_role.read` (`RequirePlatformPermission('platform_role.read')`, `platform-permissions.controller.ts`) — session ที่ไม่มี key นี้จะเข้าถึงหน้าได้แต่ call ของ catalog จะ 403 (SPA ขึ้น toast แจ้ง error แล้วโชว์ grid ว่างเปล่า ไม่ใช่หน้าจอ "access denied" ฝั่ง client) route ย้ายมาจาก `/platform/permissions` เมื่อ 2026-08-20 (commit `8df0b10` ใน repo `carmen-platform`); เข้าถึงได้จากปุ่ม header บนหน้า list ของ Roles SPA สร้างหรือแก้ไขรายการใน catalog ไม่ได้
...
สรุปไว้เท่านั้นในหน้านี้ — ดูหมายเหตุขอบเขตด้านบนว่าทำไม:

- **User Platform (`/platform/user-platform` → `UserPlatformManagement`, `/platform/user-platform/:userId` → `UserPlatformEdit`)** — assign role ให้ผู้ใช้ แต่ละ assignment มี scope กำกับ: ทั้งแพลตฟอร์มหรือ cluster ที่ระบุ การ์ด "Roles & Scope" บนหน้า detail คือจุดที่เพิ่มและลบ assignment ถูกเขียนใหม่อีกครั้งตั้งแต่ sync ครั้งก่อนให้เป็น "ทะเบียนสิทธิ์" (`carmen-platform` PR #250/#251)
- **Super Admins (`/platform/super-admins` → `SuperAdminManagement`)** — ทะเบียน add/remove ของผู้ใช้ที่ bypass ทุกการตรวจสอบ permission การเป็นสมาชิกที่นี่คือ flag ไม่ใช่ role ถูกเขียนใหม่สองครั้งนับตั้งแต่ sync ครั้งแรกเมื่อ 2026-06-10: ครั้งแรกเป็น `DataTable` ที่ค้นหาได้ แล้วเขียนใหม่อีกครั้งเมื่อ 2026-09-02 (`carmen-platform` PR #244) เป็นทะเบียนคนแบบการ์ด (avatar, ชื่อ/สถานะ/อีเมล/id, บรรทัด audit แบบย่อ "granted … by …" และ self-removal guard ที่แทนที่ปุ่ม Remove ด้วยข้อความอธิบายบนแถวของตัวเอง)
...
- **ระวังสับสน seed "Role permissions" ของ Platform Migrations กับโมดูลนี้.** คอนโซล seed ที่ `/platform/migrations` มี op ชื่อ `seed-role-permission` ("Role permissions: Bindings between app roles and permissions") ที่ **`carmen-platform` PR #281** (2026-09-04, commit `157a65e` ใน repo `carmen-platform` — มี PR #281 อีกตัวที่ไม่เกี่ยวกันอยู่ใน `carmen-turborepo-backend-v2` อย่าสับสนสองอันนี้) ขยายให้เลือกได้หลาย Business Unit พร้อมกัน (โหมด `multiple` ใหม่ของ `BuSwitcher`) แทนที่จะรันทีละ BU op นี้ seed **catalog role ระดับ tenant** ที่ Carmen Inventory เองใช้ (role แบบ `Requestor`/`Approver` ผูกกับ Business Unit — `../carmen-turborepo-backend-v2/packages/prisma-shared-schema-platform/prisma/seed.role-permission.ts`) ซึ่งไม่เกี่ยวกับตาราง `tb_platform_role`/`tb_platform_permission` ของโมดูลนี้เลย op ที่ seed ข้อมูลของ*โมดูลนี้*จริง ๆ คือ `seed-platform-role-permission` ซึ่งเป็นระดับแพลตฟอร์มทั้งหมด ไม่มีพารามิเตอร์ BU และ PR นี้ไม่ได้แตะต้อง
