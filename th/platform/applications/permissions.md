...
| 10 | session ของ super-admin หรือ bootstrap | gate `application.*` ทุกตัวผ่านโดยไม่สน grant — [resolver ของ RBAC](../rbac/permissions.md) short-circuit ก่อนการตรวจสอบ key ใด ๆ | อย่า QA เมทริกซ์ของ gate ของโมดูลนี้จาก session ของ super-admin; มันไม่มีทางเผยให้เห็น key ที่ขาดหายไป |
