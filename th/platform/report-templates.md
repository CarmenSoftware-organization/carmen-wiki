...
- **Print Template Mapping** — ถูกลบออกจากผลิตภัณฑ์เมื่อ 2026-07-24 (carmen-platform commit `de11377`; ตามด้วย backend-gateway proxy และ permission key `print_template_mapping.*` ที่ถูกลบเมื่อ 2026-07-23 ใน carmen-turborepo-backend-v2 commit `c135bb21e`) เคยเป็นตาราง routing ที่ตัดสินว่าเทมเพลต `kind="print"` ตัวไหน render เอกสารแต่ละประเภท ตอนนี้การตัดสินใจนั้นทำบนตารางนี้โดยตรงผ่าน `template_type = "form"` + `report_group` + `is_default` แสดงผลผ่านหน้าจอ [Form Groups](/th/platform/report-templates/form-groups) ความสามารถหนึ่งอย่างที่ไม่ได้ย้ายมาด้วยคือการกำหนด default ต่อ business unit — โมดูลเดิม route เทมเพลต default ต่างกันได้ต่อ BU แต่กลไก `is_default` ปัจจุบันไม่มีสิ่งทดแทน
