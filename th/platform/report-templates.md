...
Route `/report-templates`, `/report-templates/new`, และ `/report-templates/:id/edit` ถูก gate ด้วย array `allowedRoles` เดียวกัน — Role ภายใน Carmen ระดับ admin สามค่า Role อื่นใดที่ล็อกอินแล้วและเข้า route เหล่านี้จะเห็น `<AccessDenied>` ([auth-roles](/th/platform/auth-roles) อธิบายกลไก guard ไว้)
...
- [auth-roles](/th/platform/auth-roles) — เป็นเจ้าของกลไก `PrivateRoute` + `hasRole()` ที่ gate route ทั้งสามของ report-templates ให้กับ Role ระดับ admin
...
- [clusters](/th/platform/clusters) — หน้าจอที่ถูก gate ระดับ admin อีกหนึ่งหน้าที่อยู่ใน allow-list เดียวกันกับ report-templates เป็น cross-reference ที่มีประโยชน์ว่าแพลตฟอร์มจัดระเบียบขอบเขตลูกค้าเหนือระดับ BU อย่างไร
- [users](/th/platform/users) — จัดการฟิลด์ `platform_role` บนแต่ละบัญชีผู้ใช้ที่ตัดสินใจในที่สุดว่าใครจะผ่าน gate `allowedRoles` ของโมดูลนี้
