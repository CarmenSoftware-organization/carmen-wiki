...
- **ใหม่ — `tb_cluster_license`:** ledger การซื้อโควตา BU ของ cluster (`licensed_bus`, `start_date`/`end_date`, ฟิลด์ยกเลิก) โควตาที่มีผลคือใบที่ชนะใบเดียว (view `v_cluster_bu_quota`) ไม่ใช่ผลรวม — ไม่มีใบที่ชนะ = โควตา `0` จริง ไม่ใช่ "ไม่จำกัด" กลายเป็น `bu_cap`/`bu_cap_end_date` บน `Cluster` interface แทนที่ `max_license_bu` เดิม
