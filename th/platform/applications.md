...
- `../carmen-turborepo-backend-v2/packages/prisma-shared-schema-platform/prisma/schema.prisma` — `tb_application` (บรรทัด 65), `tb_application_api` (บรรทัด 90); ตรวจซ้ำ 2026-09-05 ด้วยการอ่าน source โดยตรงกับ checkout ปัจจุบันของ repo (HEAD `a48904d2`) — ทั้งเลขบรรทัดและทุก field ยืนยันแล้วว่าไม่เปลี่ยนตั้งแต่ sync ก่อนหน้า ตัวเนื้อหาตารางเองเปลี่ยนล่าสุดโดย `35c2d67c9` (2026-06-16, เพิ่ม `device`) และ `8e53bbe` (2026-06-12, เพิ่ม `doc_version`) — ระบุวันที่ด้วย `git log -L`, แก้ไข "2026-07-16" ที่เคยระบุผิดสำหรับ commit ของ `doc_version`
- `../carmen-turborepo-backend-v2/scripts/generate-app-api-catalog/run.ts` — generator ของ catalog
- `../carmen-turborepo-backend-v2/apps/backend-gateway/src/platform/applications/app-api-catalog.generated.ts` — catalog ที่ generate จริง (900 key / 148 โมดูล ตาม commit `fa64299f1`, 2026-09-04)
