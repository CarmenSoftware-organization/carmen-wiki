...
- **`enum_good_received_note_status`**: enum สถานะเอกสารสำหรับ `tb_good_received_note.doc_status` Default `draft` มีสี่ค่า:
  ...
  - `committed` — เหตุการณ์ posting เดียวเกิดขึ้นแล้ว: inventory on-hand เพิ่ม FIFO / average-cost layer อัปเดต journal entry เขียน `received_qty` ของบรรทัด PO ต้นทางเลื่อน เอกสารถูกล็อก; การแก้ไขต้องใช้ `tb_credit_note` กับ GRN นี้หรือการปรับชดเชยใน [[inventory-adjustment]]
...
- โมดูลที่เกี่ยวข้อง: [[purchase-order]] (ต้นทางผ่าน `purchase_order_detail_id`), [[purchase-request]] (จุดเริ่ม ถึงผ่าน PO↔PR bridge id เก็บบน detail_item), [[inventory]] (ปลายทาง — inventory transaction คือที่ที่ข้อมูล lot, expiry และ cost-layer อยู่), [[costing]] (การสร้าง FIFO / average-cost layer ตอน commit), [[inventory-adjustment]] (การแก้ไขหลัง commit), [[vendor-pricelist]] (การตรวจสอบ price-variance กับราคา GRN ต่อหน่วย), [[product]] (reference สินค้าต่อบรรทัด)
