...
> **At a Glance**
> **Persona:** Finance &nbsp;·&nbsp; **โมดูล:** [[inventory-adjustment]] &nbsp;·&nbsp; **Scenarios:** ~26
...
| FN-PERM-08 | Finance พยายาม back-post เข้างวด `closed` | **Deny ตาม `ADJ_VAL_011` / [[inventory]] `INV_VAL_008`** Re-open เป็น scope Finance Manager ตาม [[inventory]] `INV_AUTH_006` Finance ขอ re-open; Finance Manager grant ถ้าพิเศษ |
...
| FN-VAL-02 | Approve fail บน closed period (`ADJ_VAL_011`) | งวดปิดระหว่าง Controller forward และ Finance approval | Rejection ตาม [[inventory]] `INV_VAL_008`; Finance ขอ Finance Manager re-open หรือประสาน current-period restatement |
...
- Cross-link: [[inventory]] — `INV_AUTH_005` (scope การอนุมัติ Finance), `INV_AUTH_006` (Finance Manager period transitions), `INV_XMOD_008` (การ reconcile inventory-to-GL)
- Cross-link: [[good-receive-note]] — Finance อาจ chain flow credit-note สำหรับ vendor-recall recovery ต่อ GRN ต้นทาง
- Cross-link: [[vendor-pricelist]] — reference cost-anomaly ประวัติ
- Cross-link: [[costing]] — FIFO / WA cost picks ที่ Finance ตรวจสอบบน adjustments ขาออก
