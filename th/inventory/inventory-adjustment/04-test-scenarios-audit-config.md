...
Surface การกำหนดค่าจริงเพียงอย่างเดียวที่ทดสอบได้สำหรับโมดูลนี้คือ master ของ reason code (`tb_adjustment_type`) ซึ่งครอบคลุมแล้วโดย `../carmen-inventory-frontend-e2e/tests/031-adjustment-type.spec.ts` — CRUD ปกติ, code ไม่ซ้ำ และ toggle active/inactive ไม่มี threshold, การ map GL, การแบ่งแยกหน้าที่ หรือการกำหนดค่าบังคับแนบเอกสารให้ทดสอบ และไม่มีหน้าจอ Auditor เฉพาะสำหรับโมดูลนี้; audit trail ของการ commit คือแผง **Stock movements** ของเอกสาร (`GET /{id}/stock-movements`) และ ledger [transaction](/th/inventory/inventory/transaction)
