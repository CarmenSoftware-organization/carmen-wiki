...
Inventory Controller คือ **อำนาจการอนุมัติระหว่าง threshold auto-approve และ Finance threshold** Matrix ด้านล่างใช้แกนสองขั้น (การอนุมัติแบนด์ Controller และการ commit count-rollup) จากมุมมองของ Controller แถวมาจาก Section 2 (Entry Point and Primary Flow) ของไฟล์นี้; การอ้างอิงกฎหมายถึง [[inventory-adjustment/02-business-rules]] § 4 (กฎ Authorization) และ § 5 (กฎ Posting)
...
- **อำนาจ commit count-rollup** — commit บรรทัดผลต่างจาก [[physical-count]] / [[spot-check]] runs ซึ่งจุดชนวนการสร้าง `tb_stock_in` / `tb_stock_out` auto-rollup ตาม `ADJ_POST_006`; cross-reference [[inventory]] `INV_XMOD_003` / `INV_XMOD_004`
...
Controller **ไม่** มีอำนาจอนุมัติ above-Controller-threshold (Finance ตาม `ADJ_AUTH_005`), ไม่กำหนดค่า reason codes / thresholds (Sysadmin ตาม `ADJ_AUTH_008`) และไม่รัน period-end close (Finance Manager ตาม [[inventory]] `INV_AUTH_006`)
...
8. **Post จุดชนวน (เมื่อ Approve)** Fan-out เดียวกับ flow auto-approve ของ Store Keeper ตาม [[inventory]] `INV_POST_002`: `tb_inventory_transaction`, `tb_inventory_transaction_detail` (`qty < 0` สำหรับ stock-out), หนึ่งหรือมากกว่าแถว `tb_inventory_transaction_cost_layer` (FIFO multi-row หรือ WA single ตาม `ADJ_CALC_006` / `ADJ_CALC_007`), GL journal (`Dr Breakage Expense ฿2,500 / Cr Inventory ฿2,500`) `inventory_transaction_id` ของ detail ประทับ
...
1. เปิดเอกสาร [[physical-count]] / [[spot-check]] ที่เสร็จที่ `tb_count_stock.status = completed` บรรทัดผลต่าง stage: ต่อ `(location, product, lot)` ความต่างระหว่าง qty ทางกายภาพและระบบ
...
- ที่เกี่ยวข้อง: [[inventory]] — ทุกการอนุมัติของ Controller post ไปยัง inventory; `INV_AUTH_003` (Controller เป็น second signature ในลำดับชั้น inventory), `INV_POST_001` / `INV_POST_002` (post effects), `INV_XMOD_003` / `INV_XMOD_004` (path posting count-variance)
- ที่เกี่ยวข้อง: [[physical-count]] — Controller commit ผลต่างนับซึ่ง auto-create adjustment rollups
- ที่เกี่ยวข้อง: [[spot-check]] — การนับบางส่วน; รูปแบบ auto-rollup เดียวกัน
- ที่เกี่ยวข้อง: [[costing]] — Controller validate defensibility ของ cost-per-unit บน new-lot stock-in และ outlier FIFO / WA picks บน stock-out
- ที่เกี่ยวข้อง: [[good-receive-note]] — Controller อาจ inspect GRN ต้นทางเมื่อ review การ write-off recall ขนาดใหญ่หรือ adjustment vendor-replacement
- ที่เกี่ยวข้อง: [[vendor-pricelist]] — Controller cross-check cost-per-unit บน new-lot stock-in กับ last-price ของ vendor pricelist
