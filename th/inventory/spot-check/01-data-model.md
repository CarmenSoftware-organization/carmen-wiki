...
> **FK สำคัญ:** header `→ tb_location`; detail `→ tb_product` และ `→ tb_unit` (`inventory_unit_id`) การเชื่อม variance rollup ไปยัง [[inventory-adjustment]] เป็น JSON เท่านั้น (`tb_stock_in.info.spotCheckId` / `tb_stock_out.info.spotCheckId`) — ไม่มี FK ใน Prisma
> **รูปแบบการตรวจสอบ:** มาตรฐาน `created_*` / `updated_*` / `deleted_*`; **ต้นไม้แบนสองระดับ** (ไม่มี period parent — ad-hoc, ไม่ผูกกับงวด ต่างจาก [[physical-count]]); spot-check เองไม่เขียนลง inventory ledger — การ post adjustment คือจุดเชื่อมต่อ
...
> **TODO:** ขยายแต่ละเอนทิตีเป็นตารางฟิลด์เต็มเมื่อมี source ใน carmen/docs (หรือ spec authoritative ทางเลือก) cross-reference กับ convention ของรูปทรงตารางใน [[physical-count/01-data-model]] และ [[inventory-adjustment/01-data-model]] เพื่อความสม่ำเสมอ
...
- โมดูลที่เกี่ยวข้อง: [[inventory]] (ledger ที่ variance adjustment เขียนลง), [[inventory-adjustment]] (variance rollup post เป็น `tb_stock_in` / `tb_stock_out`), [[physical-count]] (คู่เทียบการนับเต็มที่ใช้ pattern การ integrate rollup-to-adjustment เดียวกัน; ดู [[physical-count/01-data-model]] สำหรับ infrastructure ที่แชร์), [[costing]] (default การตีมูลค่า variance สืบทอดจาก costing ฝั่ง adjustment)
