...
> **Workflow lifecycle:** Cost-layer row born (inbound — GRN commit หรือ GRN save บน BU แบบ average) → picked (FIFO/WA outbound) → revalued (credit-note) → period-close anchor (เฉพาะวิธี average เขียน `tb_inventory_period_snapshot`; FIFO นำ lot ไปต่อแทน) → period-open rollforward
> **เจาะลึกหน้า per-persona ด้านล่าง — แต่ละหน้าคือหน้าแก้ไขที่ชี้ไปพฤติกรรมจริง**
> **ตรวจสอบซ้ำ 2026-09-22:** `tb_period*` เปลี่ยนชื่อเป็น `tb_inventory_period*`; ค่าเฉลี่ยเป็นต่อสินค้า; การรับ GRN พก landed cost (extra cost + FOC); GRN บน BU แบบ average โพสต์ตอน save และถอนได้ก่อน commit; stock-in / stock-out ตอนนี้มีขั้น draft → commit; มีโมดูล GL แล้วแต่ไม่เชื่อมกับ inventory

> **แก้ไข (ยืนยันแล้ว 2026-07-22, ตรวจซ้ำ 2026-09-22):** สามหน้า per-persona ที่ลิงก์จาก Section 3 (Finance, Inventory Controller, Auditor) เคยเอกสารหน้าจอเฉพาะ — valuation-policy console, sub-ledger ↔ GL reconciliation dashboard, adjustment cost-pick-preview approval queue, read-only audit workspace — ไม่มีสิ่งใดมีอยู่จริง Engine **ไม่มี UI ที่ gate ด้วย persona ของตัวเอง**: มันทำงานเป็นผลข้างเคียงของ GRN commit (หรือ save บน BU แบบ average), SR issue, inventory-adjustment commit, credit-note approval, และ period-end close โดยแต่ละอย่าง gate ด้วย permission ทั่วไปของ source document นั้นเอง `enum_stage_role` (enum role เดียวใน tenant schema) คือ `{create, approve, purchase, issue, view_only}` — ไม่มีสมาชิก `finance` หรือ `auditor` สอดคล้องกับการแก้ไขเดียวกันที่ทำแล้วบน [inventory/03-user-flow](/th/inventory/inventory/03-user-flow) ซึ่งพบว่า "ไม่มี Finance role, GL reconciliation, หรือ period-lock flow" สำหรับ period-end surface เดียวกันที่ engine โมดูลนี้เชื่อมต่อด้วย

## 1. ภาพรวม

หน้านี้คือ **จุดเริ่มต้นภาพรวม** สำหรับชุด user-flow ของโมดูล `costing` Costing เป็นโมดูลที่ผิดปกติเมื่อเทียบกับ document module พี่น้อง — ไม่มี workflow document เดียวที่ลำดับ draft → saved → committed lifecycle เล่นออก และไม่มี document tree แยกเลย Costing เป็น **ชั้นพฤติกรรมเหนือ inventory cost-layer ledger** ดังนั้น lifecycle ที่จะอธิบายคือ **lifecycle ของ unit cost** ที่ `(location_id, product_id, lot_no)`: มันถูก **เกิด** บน inbound `tb_inventory_transaction_cost_layer` row (GRN receipt ตั้ง landed `cost_per_unit` และ `extra_cost_amount`; ภายใต้ Average running average ระดับสินค้าถูกคำนวณใหม่และประทับซ้ำ), ถูก **เลือก** โดยทุก outbound ที่ตามมา (FIFO จาก lot เก่าที่สุด; Average ที่ค่าเฉลี่ยของสินค้าที่มีอยู่), ถูก **ตีมูลค่าใหม่** โดย credit-note-amount adjustments (`diff_amount`), สามารถถูก **ถอน** ขณะที่ GRN ของมันยังเป็น `saved` บน BU แบบ average (void / reject / แก้ไข — ถูกปฏิเสธเมื่อ lot ถูกใช้ไปแล้ว) และ — **เฉพาะ business unit ที่ใช้วิธี average เท่านั้น** — ถูก **roll forward** ที่ period close เข้า opening cost ของงวดถัดไปและล็อกเข้า `tb_inventory_period_snapshot` Business unit ที่ใช้วิธี FIFO นำ lot คงเหลือไปต่อเป็นแถว `close_period`/`open_period` บน cost-layer และไม่เขียนแถว snapshot เลย

Section 2 ข้างล่างอธิบาย **cost-flow lifecycle** — ชุด canonical ของ transitions ที่ถูกกฎหมายบน unit cost จากการสร้าง inbound ถึง consumption outbound ถึง period rollforward ไม่มี "เส้นทาง persona ผ่าน state space นี้" ให้เอกสาร เพราะไม่มีหน้าจอเฉพาะ persona อยู่จริง — ผู้ถือ permission ของเอกสารที่เกี่ยวข้อง (GRN save/commit, SR approve, adjustment commit, credit-note approve, หรือ `inventory_management.period_end.execute`) จุดชนวน engine เดียวกันด้วยวิธีเดียวกัน Section 3 ลิงก์ไปสามหน้าแก้ไขที่อธิบายว่าเคย claim อะไรและพฤติกรรมจริงอยู่ที่ไหน Section 4 ถูกลดเหลือ handoff ข้ามโมดูลจริงเพียงหนึ่งเดียวที่โมดูลนี้เกี่ยวข้อง
...
sequenceDiagram
    ...
    Note over INV,COST: เฉพาะ business unit ที่ใช้วิธี average: เขียนแถว tb_inventory_period_snapshot ด้วย (COST_POST_007) business unit ที่ใช้วิธี FIFO ไม่เขียนแถว snapshot เลย
```

State machines สองตัวอยู่ในโมดูลนี้: **per-cost-layer-row** lifecycle (degenerate — แต่ละ cost-layer row เขียน immutable ตอน post, optionally revalued ผ่าน `diff_amount` row, optionally ถูกถอนขณะที่ GRN บน BU แบบ average ของมันยังเป็น `saved`, optionally rolled forward ที่ period close) และ **per-period** lifecycle ที่ mirror โมดูล inventory's `tb_inventory_period.status` ภายใน route ของโมดูลนี้เอง status นี้เคลื่อนแค่ `open → closed` เท่านั้น — พื้นผิว message-pattern ของ `period-end.controller.ts` มีแค่ `find-all` / `find-current` / `close` / `find-review` ไม่มี action lock/re-open ค่า `locked` มีอยู่บน enum และถูกตั้งโดย inventory-period service **แยกต่างหาก** ที่อยู่หลังหน้าจอ [system-config/period](/th/inventory/system-config/period) (`/inventory-periods`, `system_admin.inventory_period`) นอกโมดูลนี้ Transitions ด้านล่างครอบคลุมทั้งสอง lifecycle ตามที่ route ของโมดูลนี้เองรองรับจริง

### 2.1 Transitions ของ Cost-layer

| From state | Action | To state | Allowed for | Pre-conditions |
| ---------- | ------ | -------- | ----------- | -------------- |
| `(none)` | inbound cost-layer write (engine เลือก inbound cost) | `posted` | ผู้ถือ permission ของ source document เอง (GRN commit — หรือ GRN save บน BU แบบ average; SR ที่อนุมัติ transfer-in; count overage; stock-in **commit**) — ไม่มี gate แยกฝั่ง costing | Source document ถึงจุดใน flow ของตัวเองที่ post เข้า ledger; สำหรับ GRN `grn_date` ต้องอยู่ในงวดสินค้าคงคลัง `open`/`locked` Engine เขียน `cost_per_unit` (landed, 2 ตำแหน่ง), `extra_cost_amount`, `average_cost_per_unit` (BU แบบ average), `lot_seq_no` / `lot_no` ตาม `COST_POST_001` |
| `posted` (GRN บน BU แบบ average ยังเป็น `saved`) | void / reject / แก้ไขที่เปลี่ยน qty ของ GRN | soft-deleted (และโพสต์ใหม่เมื่อแก้ไข) | ผู้ถือ `procurement.goods_received_note.delete` / `.reject` / `.update` | การเคลื่อนไหวมี `info.posted_at_save = true`; ไม่มี lot ที่รับเข้ามาถูกใช้ที่อื่น (มิฉะนั้น `GRN_RECEIPT_ALREADY_CONSUMED`, 409) ค่าเฉลี่ยของสินค้าถูกประทับใหม่จาก layer ที่เหลือ (`COST_POST_011`) |
...
### 2.2 Per-period transitions (mirror ของ `tb_inventory_period.status` ภายใน route ของโมดูลนี้เอง)
...
| `open` (หรือ `locked`) | accept cost-layer writes | เหมือนเดิม | ผู้ถือ permission ของเอกสารที่ trigger เอง | **แก้ไข 2026-09-22 — มีกฎ resolve สองแบบอยู่ร่วมกัน** **การรับ GRN** ถูก stamp เข้างวดที่ช่วงวันที่ครอบคลุม `grn_date` (`resolvePeriodForDate` → `findOpenPeriodForDate`, `inventory-period.helper.ts`; นับรวมวันสุดท้าย; งวดที่แคบที่สุดชนะเมื่อทับซ้อน) และถูก **ปฏิเสธ** (`GRN_DATE_OUTSIDE_OPEN_PERIOD`, 422) เมื่อไม่มีงวด `open`/`locked` ครอบคลุมวันที่นั้น — ไม่มีการ re-date **Issue, transfer, credit note, แถว EOP** ยังใช้งวดเปิดปัจจุบันหรือ `resolveDocumentPeriod` (`inventory-transaction.service.ts:1801,1910,2274,2752…`) ดังนั้น issue ที่ backdate จะตกในงวดเปิดแทนที่จะถูกปฏิเสธ |
| `open` (หรือ `locked`) | close period | `closed` | ผู้ถือ `inventory_management.period_end.execute` | Blocking-document gate ทั้งหมดผ่าน (ดู [inventory/period-end](/th/inventory/inventory/period-end) § 2) รันแบบ atomic ภายใต้ row lock พร้อม re-validation; เขียน lot carry-over; tenant ที่ใช้วิธี average ได้แถว `tb_inventory_period_snapshot` ด้วย; สร้างงวดถัดไปแบบ `open` อัตโนมัติ route ของโมดูลนี้เอง (`period-end.controller.ts`) เปิดเผยแค่ `find-all` / `find-current` / `close` / `find-review` — ไม่มี action lock หรือ re-open ที่นี่ |
| `closed` | — | — | — | `locked` เป็นค่า status ที่โมดูลนี้สังเกตได้ (หน้าจอ period-end ปฏิบัติกับงวดที่ `locked` เหมือน `open` สำหรับการค้นหา "งวดปัจจุบัน" และ `findOpenPeriodForDate` รับทั้งสองค่า) แต่ไม่ได้ตั้งค่าเอง — มันถูกเขียนโดย inventory-period service **แยกต่างหาก** ที่อยู่หลังหน้าจอ [system-config/period](/th/inventory/system-config/period) นอกขอบเขตของ wiki module นี้ |
...
- [Inventory Controller](./03-user-flow-inventory-controller.md) — แก้ไข: ไม่มี cost-pick-preview approval queue; `tb_stock_in`/`tb_stock_out` ถูกสร้างเป็น `draft` และโพสต์โดยผู้ถือ permission เดียวกันผ่าน `PATCH …/commit` (อัปเดต 2026-09-22) โดยไม่มีผู้อนุมัติแยกระหว่างทาง
...
| --- | ------- | -- | ----------------------- |
| Source document ใดก็ตามที่ post stock movement (GRN commit — หรือ GRN save บน BU แบบ average; SR issue; inventory-adjustment commit; credit-note approval) | เอกสารถึงจุด posting ของตัวเอง | Costing engine (invoke แบบ synchronous ในการเรียกเดียวกัน) | Engine เขียนแถว cost-layer เป็นผลข้างเคียง — ไม่มี queue ไม่มีขั้นตอนอนุมัติแยก และไม่มีการทวนเฉพาะ persona ระหว่างทาง ไม่มีอะไรถูกโพสต์เข้า GL |
| Movement ตกลงในงวดที่เปิดอยู่ | — | Period-end close (`inventory_management.period_end.execute`) | เมื่อ blocking document ทั้งหมดผ่านและ physical count เสร็จ การปิดงวดจะนำ lot คงเหลือทุกตัวไปต่อ (ทั้งสองวิธี) และเพิ่มเติมทำ snapshot `tb_inventory_period_snapshot` (เฉพาะ business unit ที่ใช้วิธี average) ดู [inventory/period-end](/th/inventory/inventory/period-end) § 2 สำหรับตาราง gate เต็ม — costing ไม่มี gate ของตัวเองนอกเหนือจากหน้านั้น |

## 5. References

- `../carmen/docs/costing/enhanced-costing-engine.md`
- Sibling: [calculation-methods.md](./calculation-methods.md)
- Sibling: [01-data-model.md](./01-data-model.md) — `tb_inventory_transaction_cost_layer` canonical (รวม `extra_cost_amount`), `tb_inventory_period_snapshot`, `tb_business_unit.calculation_method`, `tb_product.standard_cost` และ enum ที่ใช้ใน transitions ของ Section 2
