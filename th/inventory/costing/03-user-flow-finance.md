...
- **ไม่มีการโพสต์ GL จาก cost-layer write — อัปเดต 2026-09-22** ตอนนี้มีโมดูล general ledger ใน backend แล้ว (`apps/micro-business/src/gl/`, ตาราง `tb_gl_jv*` / `tb_gl_balance`, gateway `application/gl-jv`, `gl-posting`, `gl-reports`) แต่ **ไม่ได้เชื่อมกับ inventory**: `GlPostingService.post()` (`gl-posting.service.ts:293`) โพสต์ journal voucher ที่สร้างด้วยมือ (`gl-jv.service.ts:214`), จาก template (`gl-jv-template.service.ts:786`) หรือโดย path reversal / closing ของตัวเอง; `enum_gl_jv_source.inventory` ไม่มีผู้เขียน; `inventory-transaction.service.ts`, `period-end.service.ts` และโมดูล GRN ไม่ import อะไรจาก `src/gl/` เอกสารออกแบบระดับวางแผนที่กฎของโมดูลนี้อ้างถึง (`../carmen-inventory-frontend-e2e/docs/persona-doc/System Process/proc-03-cost-calculation.md`) ยังอธิบายสถานะปัจจุบันได้ถูกต้อง: **"Does not generate GL journal entries (separate accounting integration — TBC)."** หน้า `routes/accounting` ของ frontend แสดงแถวที่ปั้นขึ้นมา (`accounting-documents.ts`)
- **ไม่มี reconciliation dashboard, tolerance, หรือกลไก compensating-journal อยู่จริง** ไม่มีอะไรใน `inventory-transaction.service.ts` หรือ `period-end.service.ts` เทียบ cost-layer activity กับ `tb_gl_balance` หรือ ledger ภายนอกใด ๆ
...
| Finance รัน sub-ledger ↔ GL reconciliation | มีโมดูล GL แล้ว (2026-09) แต่ inventory ไม่เคยโพสต์เข้าไป จึงยังไม่มีอะไรให้ reconcile | [costing/02-business-rules](/th/inventory/costing/02-business-rules) § 6 (`COST_XMOD_009`) |
...
- Backend: `../carmen-turborepo-backend-v2/apps/micro-business/src/inventory/period-end/period-end.controller.ts` (พื้นผิว endpoint ครบถ้วน), `.../inventory-transaction/inventory-transaction.service.ts` (ไม่มี GL fan-out), `.../gl/gl-posting/gl-posting.service.ts` (GL posting service — JV เท่านั้น ไม่ถูกเรียกจาก inventory)
