...
- **`enum_inventory_doc_type`**: จำแนกโมดูล source บน `tb_inventory_transaction.inventory_doc_type` เจ็ดค่า ไม่มี default ประกาศบน column (ทุก transaction ต้องระบุ):
  - `good_received_note` — receipt จาก [good-receive-note](/th/inventory/good-receive-note) commit (`saved → committed`)
...
| 4 | Valuation method บนสินค้า | PRD §3 อ้างอิง `valuationMethod: FIFO | WEIGHTED_AVERAGE` ที่กำหนดต่อสินค้า | Costing method ต่อสินค้าอยู่บน model **product** (`tb_product`) — ไม่ใช่บน inventory โมดูล inventory **บริโภค** method เมื่อเลือก cost บน transaction outbound (FIFO scan `tb_inventory_transaction_cost_layer` โดย `lot_seq_no` ascending; weighted-average อ่าน `average_cost_per_unit` ล่าสุด) | ย้ายเอกสาร valuation-method ไปใต้ [product](/th/inventory/product) / [costing](/th/inventory/costing) Inventory บันทึกกฎ **consumption** ไม่ใช่ configuration |
