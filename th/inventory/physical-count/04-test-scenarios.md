...
> **At a Glance**
> **โมดูล:** [[physical-count]] &nbsp;·&nbsp; **จำนวน scenario รวม:** ~20 ข้าม persona + ~90 ต่อ persona (skeleton) &nbsp;·&nbsp; **Persona ที่ครอบคลุม:** Count Lead, Counter, Audit / Config
...
- **Count Lead** — Inventory Controller / Inventory Manager เจ้าของการดำเนินการ; ตาม [[physical-count/03-user-flow-count-lead]]
- **Counter** — Counter / Store Keeper การป้อนข้อมูลพื้นที่; ตาม [[physical-count/03-user-flow-counter]]
- **Audit / Config** — Approver / Finance Reviewer + Auditor + Sysadmin การอนุมัติ การสังเกต การ config; ตาม [[physical-count/03-user-flow-audit-config]]
...
| 10 | Flag รายการเสียหายข้าม normal count | Counter → Count Lead | Counter พบรายการเสียหายที่ไม่อยู่บน sheet | Detail-comment flag พร้อม attachment photo; Count Lead review; อาจเพิ่มบรรทัดเองหรืออ้างอิงไป write-off ผ่าน [[inventory-adjustment]] โดยตรง |
...
| 15 | การ post backdate การนับ — reject | Count Lead | Period ที่บรรจุการนับเป็น `closed` ตาม [[inventory]] `INV_VAL_008` | Submit ของ rollup adjustment ถูก reject; Count Lead ต้อง escalate ไป Finance Manager เพื่อเปิด period ใหม่ |
...
- ที่เกี่ยวข้อง: [[physical-count/03-user-flow]] (matrix handoff ที่หน้านี้ใช้), [[physical-count/02-business-rules]] (`PHC_VAL_*` / `PHC_AUTH_*` / `PHC_POST_*`), [[inventory-adjustment/04-test-scenarios]] (scenario ฝั่ง rollup, scenario 5–6 ที่นั่นทับซ้อนกับ row 1–9 ที่นี่)
