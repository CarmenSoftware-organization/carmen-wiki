...
| ------- | --------- | ------------- | ----------------- |
| `PHC_VAL_001` | `tb_physical_count_period.status` ต้องเป็น `counting` อยู่แล้วสำหรับ period เป้าหมาย | สร้างเอกสาร count (`POST /physical-counts`) | Reject ด้วย `"Physical Count Period is not in counting status"` การเปลี่ยนเป็น `counting` ทำโดย **Start Period Close** (`POST /period-ends/start-counting`, `PeriodEndService.startCounting`, 2026-08-27) ซึ่งตัวมันเองจะปฏิเสธตราบใดที่ยังมี GRN ที่ไม่ใช่ `committed`/`voided`, stock-in / stock-out ที่ไม่ใช่ `completed`/`cancelled`/`voided` หรือ SR ที่มีเลขที่แล้วซึ่งอยู่ที่ `draft`/`in_progress` ลงวันที่ในงวดนั้น (`PERIOD_END_START_COUNTING_BLOCKED`, 422) frontend (`pc-component.tsx`) ไม่ส่ง create เลยขณะที่ `period.status !== "counting"` — แต่แสดง dialog "Counting has not started" พร้อมปุ่ม **Go to Period End** แทน |
...
| `PHC_CALC_002` (การคำนวณ on-hand ใหม่) | `reviewItems()` คำนวณ `on_hand_qty` ของ **ทุก** บรรทัดเป็น `Σ tb_inventory_transaction_detail.qty` ที่ `location_id` ของการนับ จัดกลุ่มตาม `product_id` โดย **ไม่มี date cut-off** — เป็นยอดคงเหลือ ledger ปัจจุบัน ณ ตอนกด "Submit for Review" ไม่ใช่ค่าที่ frozen ตอนสร้าง sheet `reviewItems()` ยัง stamp `counted_at` / `counted_by_id` บนบรรทัดที่ถูกนับเป็นครั้งแรกด้วย (fix `4eb1abfa5`, 2026-08-27) — ก่อน fix นี้ sheet ที่นับผ่านขั้น review เท่านั้นจะถูก `submit()` ปฏิเสธว่า "not counted" |
...
- **E2E:** `../carmen-inventory-frontend-e2e/tests/` — ยังไม่มี spec physical-count; manual catalog `docs/test-cases/750-physical-count.md` (41 case, ตรวจทานซ้ำ 2026-09-20); gap catalog ของ period-end `docs/test-cases/gaps/900-period-end-gap.md` (TC-PE-31xxxx) ครอบคลุมการ unlock ด้วยปุ่ม Start
