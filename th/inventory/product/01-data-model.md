...
ตาราง **gallery รูปภาพสินค้า** (เพิ่ม 2026-05-20 — แก้ความแตกต่าง "ไม่มี `tb_product_media`" บางส่วน ดู Section 5 ข้อ 3) เก็บหนึ่งแถวต่อภาพที่ upload ที่เชื่อมโยงกับสินค้า; ตัวไฟล์อยู่หลัง reference `file_token` (S3 / object-store จัดการโดย file service ของ platform, parallel กับรูปแบบ JSON `attachments` บนตาราง comment)

| ฟิลด์ | Prisma Type | Nullable | คำอธิบาย |
| ----- | ----------- | -------- | ----------- |
| `id` | `String @db.Uuid` | No | Primary key; `gen_random_uuid()` |
| `product_id` | `String @db.Uuid` | No | FK ไปยัง `tb_product.id` ด้วย `onDelete: Cascade` — แถวรูปจะถูกลบเมื่อ parent product ถูก hard-delete (ต่างจากรูปแบบ soft-delete ที่ใช้ทุกที่ในโมดูลนี้) |
| `file_token` | `String @db.VarChar` | No | Reference ไปยังไฟล์ที่ upload ใน file service ของ platform Resolve เป็น URL CDN/S3 ผ่าน application layer |
| `caption` | `String? @db.VarChar` | Yes | คำบรรยายที่อ่านได้แสดงใต้รูป (optional) |
| `alt_text` | `String? @db.VarChar` | Yes | alt-text สำหรับ accessibility (screen reader) และ SEO |
| `sort_order` | `Int` | No | default `0` ลำดับการแสดงใน gallery; ตัวเลขน้อยปรากฏก่อน |
| `is_primary` | `Boolean` | No | default `false` รูป primary / cover ที่กำหนด (UI บังคับให้ true ได้อย่างมากหนึ่งต่อสินค้า) |
| `created_at` / `created_by_id` | — | Yes | Timestamp audit มาตรฐาน |
| `updated_at` / `updated_by_id` | — | Yes | Timestamp audit มาตรฐาน |
| `deleted_at` / `deleted_by_id` | — | Yes | Timestamp soft-delete หมายเหตุ: cascade ฝั่ง parent ยังใช้บน hard product delete แต่การลบรูปประจำวันเป็น soft ผ่าน `deleted_at` |

**Constraints:** `@id` บน `id` FK `product_id → tb_product.id` ด้วย `onDelete: Cascade` (FK cascading เดียวในโมดูล product — ต่างกับรูปแบบ `NoAction` ที่ใช้ที่อื่น)
...
| 3 | Product Media (รูป / เอกสาร / วิดีโอ / โมเดล 3D) | `PROD-PRD.md` § 3.1.4 (FR-PRD-031..040): สื่อ primary ที่กำหนด การเรียงลำดับ thumbnail การติด tag อัตโนมัติด้วย AI การส่งมอบบนมือถือ `UI-PRD-013` อ้างถึง gallery สื่อในมุมมองรายละเอียดของสินค้า | **แก้บางส่วน (2026-05-20)** `tb_product_image` ตอนนี้เป็นตาราง first-class (ดู Section 2.9) พร้อมคอลัมน์ `file_token`, `caption`, `alt_text`, `sort_order` และ `is_primary` — ครอบคลุมส่วน "รูป primary ที่กำหนด" / "การเรียงลำดับ" / "gallery สื่อ" ของ PRD **ยังไม่ได้ model:** สื่อที่ไม่ใช่รูป (เอกสาร, วิดีโอ, โมเดล 3D) ที่ยังอยู่ใน `tb_product_comment.attachments`; การ derive thumbnail; การ tag อัตโนมัติด้วย AI; การส่งมอบบนมือถือ (ทั้งหมดเป็นเรื่องของ application-layer / CDN มากกว่า schema) | เคสรูปภาพปิดโดย schema สำหรับสื่อที่ไม่ใช่รูป เพิ่มตาราง `tb_product_media` แยก หรือขยาย `tb_product_image` ด้วย discriminator `media_type` ฟีเจอร์ Thumbnail / CDN / AI อยู่นอกขอบเขตของ schema นี้ |
