...
- **การปรับนโยบาย location** เมื่อ `min_qty` / `max_qty` / `par_qty` ที่ตั้งค่ารู้สึกผิดสำหรับ location (stock-out คงที่ที่ min หรือ overstock คงที่เหนือ max) Store Keeper post feedback บน `tb_product_location` (ผ่านแท็บ **Location Assignment** ของรายละเอียดสินค้า ซึ่งนับจาก 2026-08 แสดง **ชั้นวาง** ของสินค้าที่ location นั้นด้วย — [master-data/shelf](/th/inventory/master-data/shelf)) หมายเหตุ `re_order_qty` ไม่ได้ตั้งค่าที่นั่นอีกต่อไป; API derive จาก on-hand เทียบกับ `max_qty` / `par_qty` Handoff คือ **Product Administrator** — แก้ไขในรอบนี้; ฉบับร่างก่อนหน้า route ไปยัง "Inventory Controller" แยกต่างหากตาม `INV_AUTH_004` แต่ edit surface เดียวที่ยืนยันแล้วสำหรับฟิลด์เหล่านี้คือ product-edit form ของ Product Administrator เอง
...
- **feedback นโยบายต่อ location route ไปยัง Product Administrator** feedback Min / max / par / reorder log ผ่าน comment บน tab Location Assignment ของสินค้า; **Product Administrator** หยิบและแก้ค่าบน tab เดียวกันนั้น การมีส่วนร่วมของ Store Keeper ต่อความกังวลเรื่องนโยบายจบ (แก้ไขในรอบนี้ — ฉบับร่างก่อนหน้า route ไปยัง "Inventory Controller" แยกต่างหาก; ไม่มี edit surface เช่นนั้นอยู่จริง)
...
- Parent overview: [03-user-flow.md](./03-user-flow.md) — canonical state machine ของ record สินค้า (Section 2 — Store Keeper เป็นผู้บริโภคของสถานะ `active`) และตาราง handoff ข้าม persona (Section 4 — Store Keeper → Product Administrator บน barcode ไม่ตรง / การแก้ไขโน้ตการจัดการ / feedback นโยบายต่อ location)
...
- ที่เกี่ยวข้อง: [inventory](/th/inventory/inventory) — `tb_product_location` อ่านที่นี่; อำนาจ **edit** เป็นของ Product Administrator บน Location Assignment tab ของโมดูล product เอง (แก้ไขในรอบนี้ — ไม่พบ edit surface แยกในโมดูล inventory; [inventory/02-business-rules](/th/inventory/inventory/02-business-rules) `INV_AUTH_004` เองก็ถูกทำเครื่องหมายเป็น unconfirmed) feedback นโยบายต่อ location route ไปยัง Product Administrator
