...
| `active | inactive` | soft-delete | `soft-deleted` | Product Administrator (`PRD_AUTH_001`) | **ไม่มี guard ที่ implement ไว้** (แก้ไข 2026-09-22 — `delete()` ของ `products.service.ts` เช็คแค่ว่าแถวมีอยู่; การเช็ค on-hand / เอกสาร / สูตรตาม `PRD_LIFE_004` เป็น design intent) ตั้ง `deleted_at`, `deleted_by_id` บนสินค้าและแถว unit-conversion ของมัน `(code, name)` ใช้งานได้ใหม่ |
...
- **ไม่มี bulk import** เวอร์ชันก่อนอธิบาย import แบบ dry-run / strict-commit ว่าเป็นเส้นทาง create หลัก; ไม่มี endpoint หน้าจอ หรือ Bruno request สำหรับนำเข้า (ยืนยัน 2026-09-22) สินค้าถูกสร้างทีละรายการผ่านฟอร์ม (หรือโดยการเรียก API โดยตรง) รายการสามารถ **export** ได้
...
| Store Keeper | นโยบายสต๊อกต่อ location ต้องปรับ (min / max / par / reorder รู้สึกผิดสำหรับ location) | Product Administrator | Store Keeper post comment บนสินค้าหรือบน `tb_product_location` (ผ่าน tab Location Assignment ของสินค้า) **แก้ไขในรอบนี้:** ฉบับร่างก่อนหน้าของหน้านี้ route handoff ไปยัง "Inventory Controller" แยกต่างหากตาม `INV_AUTH_004` — ไม่มี edit surface เช่นนั้นอยู่จริง; ฟิลด์ตัวเลขของ `tb_product_location` ถูกแก้ไขโดยตรงบนฟอร์มแก้ไขของโมดูล Product เอง (tab Location Assignment, `pd-location-tab.tsx`) ซึ่งเป็น surface เดียวกับที่ Product Administrator ใช้กับฟิลด์อื่นทุกฟิลด์ `INV_AUTH_004` ของโมดูล inventory เองก็ถูกทำเครื่องหมายเป็น *(unconfirmed)* ด้วยเหตุผลนี้ |
...
- Sibling: [01-data-model.md](./01-data-model.md) — canonical `enum_product_status_type` (สามค่า: `active | inactive | discontinued`) และ `enum_unit_type` (`order_unit | ingredient_unit`); รูปทรงห่วงโซ่การจำแนก (สามระดับ ไม่ใช่ห้า) สถานะ `discontinued` เป็น first-class แล้ว — ดู Section 2 สำหรับ transition
