...
|---|---|---|
| **SR Summary (Pipeline)** | 4 stage: Requested / In Process / Approved / Issued — จำนวน + sub-label | → [store-requisition](/th/inventory/store-requisition) กรองตาม stage |
| **Sent Back SRs** | SR#, Requester, Date Sent, Sender, Reason + ปุ่ม **Bulk Submit** | → [store-requisition](/th/inventory/store-requisition) detail; bulk action re-submit |
| **Reject List (SR & รายการ)** | SR#, Material, Requester, Rejected By, Reason | → [store-requisition](/th/inventory/store-requisition) detail |
| **SR รออนุมัติ** | PR No, Description, Amount, Stage — tab วันนี้ / สัปดาห์ / เดือน | → [store-requisition](/th/inventory/store-requisition) detail |
| **การบริโภคของฉัน (ส่วนตัว)** | Donut + total ตรงกลาง + legend 3 ส่วน | — |
| **SR รอรับ** | scrollable: PR No, Description, Amount | → [store-requisition](/th/inventory/store-requisition) receipt view |
| **การบริโภคของแผนก (F&B Dept)** | bar แนวนอน + legend | — |
| **My Templates** | รายการปุ่มชื่อ template SR ที่บันทึกไว้ — คลิกเพื่อโหลด template | → [store-requisition](/th/inventory/store-requisition) ใหม่พร้อม template |
...
| "Bulk Submit" บน Sent Back re-submit จริงไหม? | (Inferred — ต้องยืนยันกับ UI จริง) Mock ไม่มี handler; wire จริงควร batch re-submit ผ่าน workflow ของ [store-requisition](/th/inventory/store-requisition) |
...
| "My Consumption" คำนวณอย่างไร? | sum บน inventory ที่บริโภคแล้ว (recipe + wastage + manual) ที่ `requestor_id = current_user` จัดกลุ่มตาม [product/category](/th/inventory/product/category) |
| อะไรเข้า "SR Awaiting Received"? | [store-requisition](/th/inventory/store-requisition) ที่ `status = "issued"` AND `destination_location_id IN (location ของ user)` AND ยังไม่ receipt |

## 4. การแก้ปัญหา

| อาการ | สาเหตุ | การดำเนินการ |
|---|---|---|
| จำนวน pipeline stage ไม่ตรงกับรายการ [store-requisition](/th/inventory/store-requisition) | Mock fixture ไม่ใช่ live | จะถูกแก้เมื่อ `useApprovalPending` ถูก mount |
...
- **Pipeline counts** — group-count บน [store-requisition](/th/inventory/store-requisition) ตาม workflow stage โดย project `workflow_current_stage` ไปยัง 4 bucket
- **Sent Back / Reject List** — `workflow_action_history` กรองเป็น send-back / reject join กลับไป [store-requisition](/th/inventory/store-requisition)
- **Awaiting Approval** — `useApprovalPending({ doc_type: "sr" })` (`hooks/use-approval.ts`); tab ต่างกันที่ filter `doc_date`
- **My Consumption** — sum inventory ที่บริโภคแล้ว (recipe + wastage + manual) ที่ `requestor_id = current_user` จัดกลุ่มตาม [product/category](/th/inventory/product/category)
- **SR Awaiting Received** — [store-requisition](/th/inventory/store-requisition) ที่ `status = "issued"` AND `destination_location_id IN (location ของ user)` AND ยังไม่ receipt
...
- [store-requisition](/th/inventory/store-requisition) — ระบบบันทึก transactional เบื้องหลังทุก tile
- [store-requisition/stock-replenishment](/th/inventory/store-requisition/stock-replenishment) — SR ตัวแปรที่ระบบสร้างให้ ซึ่งอาจปรากฏใน pipeline
- [inventory](/th/inventory/inventory) — รองรับ aggregate การบริโภคและ flow รอรับ
- [purchase-request](/th/inventory/purchase-request) — เอกสารพี่น้องที่ใช้ framework การอนุมัติร่วมกัน
