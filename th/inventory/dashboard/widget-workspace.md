...
> **หมายเหตุเรื่องชื่อ (แก้ไขเมื่อ 2026-07-16):** ตาราง backend จริงคือ `tb_dashboard_personal_widget` (ข้อมูลของหน้านี้ — tenant schema, `packages/prisma-shared-schema-tenant/prisma/schema.prisma` บรรทัด ~6205: `dataset_id`, `widget_type`, `order_index`, `params`, scope ด้วย `user_id`) และตารางพี่น้อง `tb_dashboard_bu_widget` (บรรทัด ~6185, scope ระดับ BU, widget ที่ admin curate ไว้ให้สมาชิกทุกคนใน business unit เห็น — เป็นฟีเจอร์**คนละตัว นอก scope ของหน้านี้** ที่ขับเคลื่อนแดชบอร์ด landing ระดับโมดูลใน Procurement/Inventory-management ฯลฯ ไม่ใช่หน้านี้) หมายเหตุฉบับก่อนหน้าอ้างว่าตารางอีกสี่ตารางเป็นของจริง — **ทั้งสี่ตารางไม่มีอยู่จริงแล้วหรือไม่เคยมีอยู่จริง**: `tb_widget_dashboard`, `tb_widget_dashboard_item`, และ `tb_widget_default_layout` ไม่ปรากฏในทุก Prisma schema (tenant, platform หรือ file) หรือใน micro-data/micro-report เลย ส่วน `tb_widget_workspace` *เคย*มีอยู่จริงในช่วงสั้นๆ — ถูกสร้างโดย migration `20260512180928_widget_system_replace_dashboard` (tenant schema) — แต่ถูก drop ไปพร้อมกับ `tb_widget_dashboard`, `tb_widget_dashboard_item`, `tb_widget_default_layout`, และ `tb_widget_comment` โดย migration `20260521040013_remove_widget_system` (`DROP TABLE IF EXISTS "tb_widget_workspace" CASCADE`, forward-only/idempotent, "Drop widget subsystem") ไม่มี reference เหลืออยู่เลยใน `schema.prisma` ปัจจุบันใดๆ หรือใน source `.ts`/`.go` ใดๆ และไม่มีฟีเจอร์ data-explorer/saved-query ใดในโค้ดเบสที่เคยใช้ตารางนี้ ไม่มี cross-link สำหรับตารางนี้ — [reporting-audit/widget](/th/inventory/reporting-audit/widget) เองก็ถูก flag ว่า document ตารางตระกูลที่ fabricate เดียวกันนี้ และต้องการการแก้ไขของตัวเองเช่นกัน
