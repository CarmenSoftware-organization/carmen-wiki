...
| `in_progress` | approve (stage นี้ ไม่ใช่ final) | `in_progress` | Current-stage approver | Approver อยู่ใน `user_action.execute[]` สำหรับ `workflow_current_stage`; `last_action` กลายเป็น `approved`; stage cursor advance ไม่มี amount threshold ใดมา gate สิ่งนี้ — เป็นเพียงจำนวน stage และการกำหนดของ workflow เท่านั้น |
...
- [Procurement Manager](./03-user-flow-procurement-manager.md) — ทำหน้าที่เป็นผู้อนุมัติใน workflow stage ที่กำหนดไว้ (กลไกทั่วไปเดียวกับผู้อนุมัติรายอื่น — ไม่พบ escalation ตาม amount-threshold ที่ยืนยันได้); ถือสิทธิ์ delete-in-draft
