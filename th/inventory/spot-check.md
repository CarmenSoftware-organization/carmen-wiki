...
**ข้อค้นพบที่สอง ซึ่งตอนนี้ได้รับการแก้ไขแล้วใน frontend: ฟอร์มสร้างไม่มี code view/edit ที่ตายแล้วอีกต่อไป** รอบ 2026-07-15 พบว่า `ScForm` (`sc-form.tsx`) ship มาพร้อม branch `isView`/`isEdit`, ปุ่ม Edit/Delete และ hook `useUpdateSpotCheck`/`useDeleteSpotCheck` ที่เข้าถึงไม่ได้ — มันถูก render จาก `spot-check-by-location-content.tsx` ที่เดียวในโหมด "add" พร้อม `hideLocation` เท่านั้น เมื่อ 2026-09-04 (`0647b32e`, "remove the ScForm edit path that was dead since day one") โค้ดส่วนนั้นถูกลบออกแล้ว; ตอนนี้ `ScForm` เป็นฟอร์มสร้างอย่างเดียว และ `spot-check/:id` ยังคง render หน้า entry/counting (`ScEntryComponent`) คู่ `update()` / `delete()` ฝั่ง backend (`PATCH /spot-checks/:id`, guard `spotCheck.update` / `spotCheck.delete`) ยังมีอยู่และทำงานได้เมื่อเรียกตรง (Bruno `inventory/spot-check/PATCH-update-…`, `DELETE-remove-…`) แต่ไม่มีปุ่มใดใน UI ที่ shipped ไปถึงมันได้
...
- E2E tests: `../carmen-inventory-frontend-e2e/` — ยังไม่มี Playwright spec สำหรับโมดูลนี้ (`ls tests/ | grep -i 'spot\|check'`); manual test-case catalog `docs/test-cases/760-spot-check.md` (44 cases, re-verify กับ live component เมื่อ 2026-09-20 — ตอนนี้ระบุชัดเจนว่าไม่มีหน้า view/edit/delete) และ stories ที่ generate แล้ว `docs/user-stories/760-spot-check.md` (32)
