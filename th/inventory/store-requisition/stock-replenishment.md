...
Stock Replenishment คือ **variant ที่ขับโดยนโยบายของ [store-requisition](/th/inventory/store-requisition)** Job ที่ตั้งเวลากวาดทุกคู่ `(product, location)` ที่มี threshold ตั้งค่าและเสนอ SR draft สำหรับทุก location ใต้ par Inventory Controller review ตัดและ submit — จากจุดนั้นเป็น SR ปกติที่วิ่งผ่าน workflow อนุมัติและ fulfillment มาตรฐาน
...
| Override par / max สำหรับสินค้า | [product](/th/inventory/product) → tab location → แก้ `tb_product_location` | มีผลใน run ถัดไป |
...
|---|---|---|
| สินค้าไม่ปรากฏใน draft | ไม่มีแถว `tb_product_location` หรือ `min_qty = 0` | เพิ่ม / อัปเดตแถวนโยบายใน [product](/th/inventory/product) |
...
- [store-requisition](/th/inventory/store-requisition) — ประเภทเอกสารที่ผลิต; วงจรชีวิตปลายน้ำเหมือนกัน
- [inventory](/th/inventory/inventory) — แหล่ง `on_hand` สำหรับการคำนวณ deficit
