...
> **Re-sync 2026-09-22:** วันที่ issue ถูก resolve เทียบกับ inventory period ที่เปิด (`resolveIssueDate()`, dialog `issue_date_pattern`) และ stamp เป็น `issue_at` / `issue_by_id`; เพดาน `issued_qty ≤ approved_qty` **ไม่ถูกบังคับใช้**; การ block งวดปิดที่รอบ 2026-07-15 หาไม่พบตอนนี้มีจริงแล้ว; แท็บ Stock Movement แสดง lot ที่ post แล้ว
...
graph LR
    ...
    pick -->|"PATCH .../approve (ขั้นสุดท้าย)"| completed(("completed")):::current
...
| ป้อน `issued_qty` ต่อบรรทัด | ✅ (`SR_AUTH_007`) — เพดาน `≤ approved_qty` **ไม่ถูกบังคับใช้** (`SR_VAL_008` แก้ไขแล้ว) | ❌ |
...
| แก้ส่วนหัว | ❌ | ❌ |
| Issue นอกทุก open period | ถูกถาม `issue_date_pattern`; `today` ถูกปฏิเสธ (`SR_VAL_014`) | — |
...
2. **Re-check ความพร้อมต้นทางตอน issue** dialog Inventory Information ต่อแถวแสดง on-hand ต้นทาง live; ไม่มี pre-check ฝั่ง server — guard เดียวคือการบริโภค cost-layer ที่ throw `Insufficient stock` ระหว่าง fan-out ขั้นสุดท้าย (`SR_VAL_013`) ถ้า on-hand live ลดต่ำกว่า `approved_qty` ตั้งแต่อนุมัติ Fulfiller ควร short-fulfil (decision branch ด้านล่าง)
3. **หยิบสินค้าจริงและป้อน `issued_qty` ต่อบรรทัด** สิ่งที่หยิบจริง ในหน่วย UoM ของสินค้า ทั้งฟอร์ม (`z.coerce.number()` ไม่มีขอบเขต) และ backend ไม่จำกัดค่าไว้ที่ `approved_qty` — `SR_VAL_008` เป็น convention ไม่ใช่ control การ **Save** ระหว่างทางที่ขั้นนี้ (`PUT` พร้อม `stage_role: "issue"`) stamp `issue_at` / `issue_by_id` แล้ว
4. **บันทึก context เพิ่ม** Comment แบบอิสระต่อบรรทัด; attachment เขียนไป `tb_store_requisition_detail_comment`
5. **คลิก Issue** เรียก endpoint `PATCH .../approve` เดียวกันเป๊ะกับปุ่ม "Approve" ของ Approver (`useIssueStoreRequisition` และ `useApproveStoreRequisition` คือ mutation เดียวกัน ต่างกันแค่ `stage_role: "issue"`) backend resolve วันที่ issue ก่อน (`resolveIssueDate()`): `sr_date` ของ SR เองต้องอยู่ใน open period (มิฉะนั้น `SR_DATE_OUTSIDE_OPEN_PERIOD`), ใช้วันนี้เมื่อวันนี้อยู่ใน open period มิฉะนั้น client ถูกถาม `issue_date_pattern` — *open-period* ลงวันที่ issue เป็นวันสุดท้ายของ period ของ SR, *today* ถูกปฏิเสธ (`SR_ISSUE_DATE_TODAY_OUTSIDE_PERIOD`) เมื่อนี่เป็นขั้นสุดท้าย (`workflow_next_stage === '-'`) เอกสารกลายเป็น `completed`, `issue_at` / `issue_by_id` ถูก stamp และ `executeTransferOnComplete()` fire
6. **Inventory fan-out fire** สำหรับแต่ละบรรทัดที่ `issued_qty > 0` `InventoryTransactionService.executeTransfer()` insert แถว `tb_inventory_transaction` (`inventory_doc_type = store_requisition` ลงวันที่ `doc_date = issue_at` และวางใน period ของวันที่นั้น) พร้อม child `tb_inventory_transaction_detail` ที่บรรจุ `lot_no` (assign แบบ FIFO โดยระบบ), `expiry_date` และ `cost_per_unit`; stamp id บน `tb_store_requisition_detail.inventory_transaction_id`; ลด on-hand ต้นทาง; สำหรับ `sr_type = transfer` เพิ่ม on-hand ปลายทาง (สำหรับ `sr_type = issue` เข้าสถานที่ `direct` transfer-in หักลบเป็นศูนย์ — on-hand ปลายทางไม่เปลี่ยน) การ post GL/journal-entry ยังไม่ยืนยัน
7. **เอกสารเปลี่ยนสถานะ** `doc_status = in_progress → completed`; `workflow_history` ได้ entry สุดท้าย SR ล็อกจากการแก้เพิ่ม; แท็บ **Stock Movement** (`GET .../stock-movements`) สลับจาก preview ที่อิงบรรทัดเป็นแถวที่ post แล้ว (`is_posted = true`, `lot_no`, `cost_per_unit`, `total_cost` จริง) **แก้ไขรอบนี้:** ไม่มี handoff ปลายน้ำไปยัง "Receiver" ที่ยืนยันได้ — ดู [03-user-flow-receiver.md](./03-user-flow-receiver.md)

## 3. Branch การตัดสินใจ

- **Stock-out ตอน issue (on-hand live < `approved_qty`)** — การลด `issued_qty` ให้เท่ากับสิ่งที่มีจริงแล้ว issue ต่อไปเป็นเส้นทางที่ตั้งใจไว้ ถ้า Fulfiller issue มากกว่า on-hand `createFifoConsumption()` / `createAverageConsumption()` จะ throw `Insufficient stock. Requested: …, Available: …` ภายใน transaction ของ fan-out; เนื่องจากการ update ส่วนหัว commit ไปแล้ว SR จึงถูกทิ้งไว้ในสถานะ `completed` โดยไม่มี movement ถูกเขียน — เป็น ordering hazard ที่รู้กัน ไม่ใช่ validation ที่สง่างาม (`SR_VAL_013`, `SR_POST_006`)
...
- **ความพยายาม issue ในงวดปิด** — **ยืนยันแล้วว่ามีจริง (2026-09-18)** ถ้า `sr_date` ของ SR ไม่อยู่ใน inventory period ที่เปิด issue call จะล้มเหลวด้วย `SR_DATE_OUTSIDE_OPEN_PERIOD` (422) — โดยทั่วไปคือ SR ที่ submit ด้วย `sr_date_pattern = today` เข้า period ที่ยังไม่เปิด; จะ issue ได้เมื่อ period นั้นเปิด ถ้าวันนี้เองอยู่นอกทุก open period dialog date-pattern จะปรากฏและรับเฉพาะ *open-period* (`SR_VAL_014`)
...
- Sibling: [02-business-rules.md](./02-business-rules.md) — `SR_VAL_008` (quantity invariant — ไม่ถูกบังคับใช้), `SR_VAL_013` (guard สต๊อกต้นทางภายใน fan-out), `SR_VAL_014` / `SR_VAL_016` (open-period gate และ `issue_date_pattern`), `SR_AUTH_007` (อำนาจขั้น issue), `SR_POST_005`–`SR_POST_006` (การเดินขั้นสุดท้ายและ inventory fan-out), `SR_POST_010` (reject ทั้งเอกสาร → `voided`)
