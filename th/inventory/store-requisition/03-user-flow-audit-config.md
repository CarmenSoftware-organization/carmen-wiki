...
> - **ไม่มีการ post SR → GL** *อัปเดต 2026-09-22:* ตอนนี้ backend มี GL core แล้ว (`apps/micro-business/src/gl/`, migrations `20260909170000_gl_core_master`, `20260914030000_gl_core_jv_ledger`) แต่ `grep -rn store_requisition apps/micro-business/src/gl/` ได้ศูนย์ผลลัพธ์ — ไม่มีสิ่งใดใน commit path ของ SR ที่สร้าง journal ดังนั้นยังไม่มีสิ่งใดที่เฉพาะกับ SR ให้ role "Finance" ตรวจสอบหรือ reconcile ที่นี่
> - **Closed-period gate — มีจริงแล้ว แต่ไม่ใช่ action ของ Finance** *อัปเดต 2026-09-22:* `logic/sr-date.helper.ts` (2026-09-18) ปฏิเสธการ issue SR ที่ `sr_date` อยู่นอก inventory period ที่เปิด (`SR_DATE_OUTSIDE_OPEN_PERIOD`) และขอ date pattern จากผู้กระทำเมื่อวันนี้อยู่นอกทุก open period gate นี้บังคับใช้ที่การ submit ของ requester และ approve call ของ issuer; ตัว period master เองเป็น system config ทั่วไป ([system-config/period](/th/inventory/system-config/period)) ไม่ใช่ console ของ SR
...
| Finance GL-reconciliation queue, การตรวจสอบ journal-entry | **ไม่ได้ implement สำหรับ SR** โมดูล GL มีอยู่แต่ไม่มี hook ของ SR |
| การ block commit ในงวดปิด | **Implement แล้ว (2026-09-18) เป็น date gate ทั่วไป** ไม่ใช่หน้าจอของ Finance — ดู `SR_VAL_014` |
...
| การ reconcile / signoff ปลายงวดเป็น workflow แยก | **ไม่ได้ implement ในโมดูลนี้** — open-period gate มีจริง แต่การ reconcile/sign-off เป็นของโมดูล period-end ไม่ใช่ SR |

## 2. ควรทำอย่างไรกับหน้านี้

จนกว่าจะยืนยันว่ามี endpoint admin-void, ฟีเจอร์ post GL/journal ที่ต่อสายกับ SR หรือ console config RBAC/workflow เฉพาะโมดูลนี้ อย่าถือว่าข้อความ "Inventory Controller admin-void", "Finance block การ commit ในงวดปิด", "Sysadmin ตั้งค่า threshold การผ่อนคลาย SoD" หรือ "Auditor trace ลายเซ็น" ที่ปรากฏในหน้าอื่นของโมดูลนี้เป็นพฤติกรรมจริง
...
- Business rules: [02-business-rules.md](./02-business-rules.md) § 2 (`SR_VAL_014`, ยืนยันแล้ว 2026-09-22), § 4 (`SR_AUTH_009`–`SR_AUTH_013`, แก้ไขแล้ว), § 5 (`SR_POST_007`, `SR_POST_009`, `SR_POST_010`, `SR_POST_013`, แก้ไขแล้ว), § 6 (`SR_XMOD_008` — routing ตาม amount ยืนยันแล้วว่ามีจริงในรอบนี้; การ delegate ยังคงไม่ยืนยัน)
