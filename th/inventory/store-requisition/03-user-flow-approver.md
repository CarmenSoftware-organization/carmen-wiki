...
> **At a Glance**
> **Persona:** Approver — ผู้ถือขั้น workflow ที่ tag `enum_stage_role.approve` &nbsp;·&nbsp; **โมดูล:** [store-requisition](/th/inventory/store-requisition) &nbsp;·&nbsp; **ขั้น workflow:** in_progress (ขั้นที่ tag approve) → in_progress (ขั้นถัดไป) / voided / draft (send-back) &nbsp;·&nbsp; **สิทธิ์สำคัญ:** approve, ตัด approved_qty, reject (bundle รวมในการเรียก `PATCH .../approve` เดียวกัน), send-back (`/review`)
...
> **Re-sync 2026-09-22:** verb ทั้งหมดเป็น `PATCH`; `approved_qty` มาถึงโดยถูกเติมไว้ล่วงหน้าด้วย `requested_qty`; เพดาน `approved_qty ≤ requested_qty` **ไม่ถูกบังคับใช้** ที่ใดเลย; dialog Approve/Reject ใน footer ไม่มีช่องเหตุผล (มีเฉพาะ Send Back และ bulk Reject); Send Back ต้องระบุขั้นปลายทาง
...
| Reject บรรทัด, bundle รวมกับการตัดสินใจ approve ของบรรทัดอื่นในการเรียก `PATCH .../approve` เดียวกัน | ✅ (`SR_AUTH_005`, `SR_VAL_010`) |
| ส่งเอกสารทั้งฉบับกลับให้แก้ไข (`/review`; ผสมกับ approve/reject ในการเรียกเดียวกันไม่ได้) | ✅ (`SR_AUTH_005`) |
| ผสม approve / reject ต่อบรรทัดในการเรียก `PATCH .../approve` เดียวกัน | ✅ — ยืนยันใน `computeSrAction()` (`sr-form-schema.ts`) |
| อนุมัติ SR ของตน (กรณี Approver = Requester) | ยังไม่ยืนยันว่าถูก block — ไม่พบ SoD check ในโค้ด |
| เพิ่ม `approved_qty` เกิน `requested_qty` | ⚠️ **ไม่ถูก block** — ไม่มีเพดานใน DTO/service/form (`SR_VAL_010` แก้ไข 2026-09-22) |
| การเดินขั้นสุดท้าย (บันทึก `issued_qty`) | ใช้ endpoint `PATCH .../approve` เดียวกัน — gate ด้วยว่าผู้ใช้ถือ `enum_stage_role` ใด (`approve` เทียบกับ `issue`) ไม่ใช่สิทธิ์ commit แยกต่างหาก |
...
3. **การตัดสินใจต่อบรรทัด** `approved_qty` มาถึงโดยถูกเติมไว้ล่วงหน้าด้วย `requested_qty` (ตั้งค่าเริ่มต้นตอน submit) สำหรับแต่ละบรรทัด Approver เลือกหนึ่งใน:
   - **อนุมัติเต็ม**: ปล่อย `approved_qty = requested_qty` ไว้ ต่อบรรทัด: `approved_by_id`, `approved_by_name`, `approved_date_at = now()`, `approved_message` ทางเลือก
   - **ตัดลง**: ตั้ง `approved_qty ∈ (0, requested_qty)` ต่อบรรทัด: คอลัมน์ลายเซ็นเดียวกัน; `approved_message` อธิบายการตัด (เช่น "trimmed to source on-hand" — เหตุผลการตัดเดียวที่ยืนยันได้; ถ้อยคำ "par-level cap" และ "budget cap" เป็นเพียงตัวอย่างประกอบ ไม่ใช่ label ที่ระบบสร้างขึ้น) การเพิ่ม `approved_qty` เกิน `requested_qty` **ไม่ถูก reject** โดยชั้นใดเลย (`SR_VAL_010` ไม่ถูกบังคับใช้)
   - **Reject บรรทัด**: mark เป็น `reject` backend **ไม่** ตั้ง `approved_qty` เป็นศูนย์; บันทึกสถานะขั้นและ history ของบรรทัด เหตุผลพิมพ์ได้เฉพาะผ่าน action bulk **Reject** เหนือตารางรายการ (optional, `Enter reason (optional)…`) — dialog ยืนยัน Reject ใน footer ถูก render ด้วย `showMessage={false}` แม้ข้อความจะบอกว่า "Please provide a reason." Bundle รวมกับการตัดสินใจของบรรทัดอื่นในการเรียก `PATCH .../approve` เดียวกัน
   - **ส่งเอกสารทั้งฉบับกลับให้แก้ไข**: การเรียก `PATCH .../review` แยกต่างหาก ซึ่ง dialog ต้องระบุ **ขั้นปลายทาง** (`des_stage` จาก `GET .../workflow-previous-step-list`; ปุ่มยืนยันถูก disable จนกว่าจะเลือก) และรับข้อความ optional; ถ้าบรรทัดใดถูก mark เป็น "review" การ submit ทั้งหมดกลายเป็น send-back และตัวเลือก approve/reject ของบรรทัดอื่นใน submission เดียวกันนั้นจะไม่ถูก apply (`computeSrAction()`)
4. **การตัดสินใจแบบผสมข้ามบรรทัด** Approve และ reject ผสมกันได้ในการเรียก `PATCH .../approve` เดียวกัน; review ผสมกับอย่างใดอย่างหนึ่งใน submission เดียวกันไม่ได้ (ดูข้อ 3) หน้าจอที่อ่านในรอบนี้ไม่แสดง "ยอดสะสมของมูลค่าที่อนุมัติ/reject" — ถือว่าเป็นข้อความที่ยังไม่ยืนยัน
5. **ยืนยัน action** footer แสดงปุ่มเดียวเท่านั้นที่ derive จากสถานะต่อบรรทัด (`computeSrAction()`): **Approve** เมื่อทุกบรรทัดที่ตัดสินใจแล้วเป็น approve/reject โดยมี approve ≥ 1, **Reject** เมื่อทุกบรรทัดเป็น reject (`PATCH .../reject` ทั้งเอกสาร → `voided`), **Send Back** เมื่อบรรทัดใดเป็น review, ไม่มีปุ่มเมื่อยังไม่มีบรรทัดใดถูกตัดสินใจ ระบบตรวจ `SR_AUTH_014` (ผู้กระทำอยู่ใน `user_action.execute`) และ `doc_version` ที่ echo กลับ; **ไม่มีการ validate เพดานปริมาณ**
6. **เดิน workflow** เมื่อบรรทัดทั้งหมดบนขั้นปัจจุบันถูก action แล้ว ระบบเดิน `workflow_current_stage` ไปขั้นถัดไป — ขั้น `approve`/`issue` ถัดไปตาม `tb_workflow` หรือ `completed` ถ้านี่คือขั้นสุดท้าย การเรียก `PATCH .../reject` ทั้งเอกสารแยกต่างหาก (เปิดใช้เฉพาะเมื่อทุกบรรทัดถูก mark reject) ตั้ง `doc_status = voided` โดยตรง — **ไม่ใช่** `cancelled` (ดู [01-data-model.md](./01-data-model.md) §5 ข้อ 11)
...
- **Reject ทั้ง SR**: ทุกบรรทัดถูก mark reject ใน submission เดียว เปิดใช้ action `PATCH .../reject` ทั้งเอกสาร ระบบตั้ง `doc_status = voided` โดยตรง — **แก้ไขรอบนี้**: เวอร์ชันก่อนหน้าของหน้านี้บรรยายว่าจบที่ `cancelled`; ไม่มีการ assign `cancelled` เลยที่ใดใน `store-requisition.service.ts`
...
- Sibling: [02-business-rules.md](./02-business-rules.md) — `SR_VAL_010` (เพดานการอนุมัติ — เจตนาที่บันทึกไว้ ไม่ถูกบังคับใช้), `SR_AUTH_005`–`SR_AUTH_006` (อำนาจ approve / trim / send-back, กฎการผสม), `SR_POST_005`–`SR_POST_010` (การเดินขั้นสุดท้ายและ reject ทั้งเอกสาร → `voided`)
