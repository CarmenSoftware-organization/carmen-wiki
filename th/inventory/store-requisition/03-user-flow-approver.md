...
> **At a Glance**
> **Persona:** Approver (Department Head + ขั้นหลัง Ops / Cost Controller) &nbsp;·&nbsp; **โมดูล:** [store-requisition](/th/inventory/store-requisition) &nbsp;·&nbsp; **ขั้น workflow:** in_progress (ขั้นอนุมัติ) → in_progress (fulfillment) / cancelled / draft (send-back) &nbsp;·&nbsp; **สิทธิ์สำคัญ:** approve, trim approved_qty, reject (บรรทัด / ส่วนหัว), split-reject, send-back
...
Approver อาจโต้แย้งปัญหาปลายน้ำหลัง commit ด้วย (เช่น fulfiller short-fulfill บรรทัดที่อนุมัติ); การ resolution ผ่าน Inventory Controller variance review และ `[inventory-adjustment](/th/inventory/inventory-adjustment)` ไม่ใช่ผ่านการเปิด SR ใหม่
