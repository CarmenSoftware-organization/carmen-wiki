...
> **Executable coverage (2026-09-22):** `../carmen-inventory-frontend-e2e/tests/701-sr.spec.ts` (46 case ที่ run แต่มี `expect()` จริงเพียง 2 — ดู `docs/test-cases/gaps/701-sr-gap.md`, 62 gap case) และ `tests/720-stock-issue.spec.ts` (list / header / ปุ่ม Issue / แท็บ Stock Movement; `docs/test-cases/gaps/720-stock-issue-gap.md`, 44 gap case); narrative ที่ generate ใน `docs/user-stories/701-sr.md` และ `720-stock-issue.md` Stock replenishment: `tests/711-stock-replenishment.spec.ts` (33 case) หน้านี้ไม่ mirror case เหล่านั้น

## 1. ภาพรวม

หน้านี้คือ **จุดเริ่มต้นภาพรวม** สำหรับชุด test-scenarios ของโมดูล `store-requisition` จัดกลุ่ม coverage ของ SR ตาม persona ที่ interact กับเอกสารข้ามวงจรชีวิต, inventory ไฟล์ test ต่อ persona, จับ scenario การ handoff ข้าม persona ที่ร้อยเส้นทางแต่ละเส้นเข้าด้วยกัน และชี้ไปยัง spec ที่ run ได้ (`701-sr.spec.ts`, `720-stock-issue.spec.ts`) แทนที่จะ mirror มัน ขอบเขตกว้างกว่า pass เชิง functional ล้วน ๆ จงใจ: แต่ละไฟล์ persona รวม **happy path เชิง functional**, **กรณี RBAC / permission-denial** (ขับโดย fixture `requestor@blueledgers.com`) และ **edge cases** (input ว่าง / ไม่ถูกต้อง / ใหญ่ โหมด soft vs hard ของความพร้อมต้นทาง)

> ⚠️ **แก้ไขรอบนี้** สามกลุ่มข้อกล่าวอ้างที่ปรากฏในทุกไฟล์ persona ถูกตรวจสอบกับ `store-requisition.service.ts`, `store-requisition.logic.ts` และ frontend แล้วพบว่ายังไม่ยืนยัน: (1) **segregation of duties** — ไม่มีการ cross-check `requestor_id` / `approved_by_id` ใด ๆ ใน SR service ดังนั้น scenario "Requester ≠ Approver" และ "Approver ≠ Fulfiller" ไม่ใช่ control ที่บังคับใช้จริง; (2) **`cancelled` เป็นสถานะที่ไปถึงได้** — ไม่มี service method ใดกำหนดค่านี้เลย เส้นทางการยกเลิกที่ยืนยันได้เส้นเดียวคือ action `reject` ทั้งเอกสารซึ่งตั้งเป็น `voided` ใช้ได้โดยผู้ที่ถือขั้น workflow ปัจจุบัน (ไม่ใช่ "void" เฉพาะ admin); (3) **การเลือก lot ด้วยมือ, approval delegation, SLA time-out escalation, การเชื่อมโมดูล budget และการ post GL/journal-entry ที่ SR trigger** — ไม่พบสิ่งใดใน source ปัจจุบัน (การกำหนด lot เป็น FIFO อัตโนมัติ; ที่เหลือค้นหาแล้วไม่พบ `delegat` หรือ route ของ budget เลย และโมดูล GL ที่มีอยู่ตอนนี้ไม่มี hook ของ SR) **อัปเดต 2026-09-22:** การ block งวดปิดที่รอบ 2026-07-15 หาไม่พบตอนนี้มีจริงแล้ว (`sr-date.helper.ts`, `SR_VAL_014`) และกลุ่มข้อกล่าวอ้างที่สี่ตกไป: เพดานปริมาณสามค่า (`issued ≤ approved ≤ requested`) ไม่ถูกบังคับใช้โดยชั้นใดเลย (`SR_VAL_008`) **แต่ multi-tier value-threshold escalation กลับถูก mark ว่ายังไม่ยืนยันผิดพลาดโดยการแก้ไขรุ่นก่อนหน้าเอง** — การค้นหาติดตามผลยืนยันว่า `routing_rules` ของ workflow ที่ assign ให้สามารถ route ตาม `total_amount` ได้ (คำนวณสำหรับ SR โดย `sr-workflow.mapper.ts`) เพื่อ skip หรือกระโดดข้ามขั้น เป็นกลไกทั่วไปแบบเดียวกับที่ documented ไว้สำหรับ PR/PO (`SR_XMOD_008`) แถวด้านล่างถูกแก้ไข in place; ดู [01-data-model.md](./01-data-model.md) §5 และ [02-business-rules.md](./02-business-rules.md) สำหรับรายละเอียดเต็ม
...
- **"Receiver"** และ **"Audit / Config" (Inventory Controller / Finance / Sysadmin / Auditor)**: **ยังไม่ยืนยันว่าเป็น persona แยก** — ไม่พบ route, สมาชิก `enum_stage_role` หรือ feature เฉพาะ SR ที่ตรงกัน (discrepancy flag, admin-void console, GL verification, RBAC console) ใน source ปัจจุบัน (config routing ตาม amount แบบทั่วไป ไม่เฉพาะ SR ที่ Sysadmin เข้าถึงได้ผ่าน `/system-admin/workflow` ยืนยันแล้วว่ามีจริง — ดู `SR_XMOD_008` — แต่ไม่ใช่ feature ของ persona "Audit / Config" เฉพาะของ SR) หน้า test-scenario ของทั้งสองเป็น correction stub — ดู [04-test-scenarios-receiver.md](./04-test-scenarios-receiver.md) และ [04-test-scenarios-audit-config.md](./04-test-scenarios-audit-config.md)
...
| 6 | Stock-out ตอน issue — fulfiller บันทึกบางส่วน | Requester → Approver → Fulfiller (short-issue) | ระหว่างอนุมัติกับ issue การบริโภคอื่นลด on-hand ต้นทางต่ำกว่า `approved_qty`; dialog Inventory Information แสดงส่วนที่ขาด (ไม่มี pre-check ฝั่ง server) | SR `completed` พร้อม `issued_qty < approved_qty` บนบรรทัดที่กระทบ; ช่องว่างคำนวณได้จากคอลัมน์ที่เก็บไว้ Variant: Fulfiller issue เต็ม `approved_qty` อยู่ดี → fan-out throw `Insufficient stock` หลังจากส่วนหัวพลิกเป็น `completed` แล้ว (ไม่มี movement ถูกเขียน) — ดู `SR_VAL_013` |
...
| 10 | Open-period gate ตอน submit และ issue — **นำกลับมา 2026-09-22** | Requester → Fulfiller | Inventory period ถูกตั้งค่าให้วันนี้อยู่นอกทุก open period (หรือ SR ถูกลงวันที่ด้วย `sr_date_pattern = today` เข้า period ที่ยังไม่เปิด) | Submit คืน `SR_DATE_PATTERN_REQUIRED` (422) และ dialog date-pattern ปรากฏ; เลือก *open-period* ลงวันที่ SR เป็นวันสุดท้ายของ open period ปัจจุบัน ที่ขั้น issue: `SR_DATE_OUTSIDE_OPEN_PERIOD` เมื่อวันที่ของ SR ไม่อยู่ใน open period; `SR_ISSUE_DATE_PATTERN_REQUIRED` → dialog โดย *today* ถูกปฏิเสธ (`SR_ISSUE_DATE_TODAY_OUTSIDE_PERIOD`) movement ที่ post ลงวันที่ `issue_at` (`SR_VAL_014`, `SR_VAL_016`) |
...
| 12 | SR ที่ auto-create ผ่านวงจรชีวิตปกติ — **ยึดใหม่ 2026-09-22 ไปยัง stock replenishment** | Stock Replenishment wizard → Requester (review) → Approver → Fulfiller | มีแถวที่ต่ำกว่า par สำหรับสถานที่หนึ่ง; ผู้ใช้เลือกสถานที่ต้นทางและ SR workflow ใน wizard (`POST .../stock-replenishment/sr`) | draft SR หนึ่งใบต่อคู่ `(from_location, location_id)` ถูกสร้างผ่าน `StoreRequisitionLogic.create()`; จากนั้นเดิน `draft → in_progress → completed` เหมือน SR แบบ manual ใด ๆ เส้นทาง auto-create จาก recipe ที่เคยบรรยายที่นี่ไม่มีโค้ดรองรับ (`SR_XMOD_006`) |
| 13 | ~~การ reconcile period-close~~ — แก้ไขรอบนี้ | — | — | **ถูกลบ** scenario นี้ขึ้นกับการ post GL/journal-entry จาก SR ซึ่งยังไม่มี code path (โมดูล GL ไม่มี hook ของ SR) ส่วน open-period gate เองคือ Scenario 10 |
| 14 | ~~การเปลี่ยน config Workflow / RBAC~~ — แก้ไขรอบนี้ | — | — | **แคบลง** `tb_workflow` เป็นตารางจริงที่ tenant config ได้ ใช้ร่วมกับ PR/PO/GRN — การเปลี่ยน stage definitions และ `routing_rules` ตาม amount/department/category (มีจริง ยืนยันแล้ว — `SR_XMOD_008`) เป็น action config workflow ทั่วไป ไม่ใช่ feature เฉพาะ SR ไม่พบฟิลด์ SoD-relaxation-threshold เฉพาะ SR ให้ config |
| 15 | การลบ draft เฉพาะเจ้าของ เดี่ยวและ batch — **เพิ่ม 2026-09-22** | Requester A (เจ้าของ), Requester B | A มี draft สองใบ; B ไม่ใช่ platform super-admin | `DELETE .../store-requisitions/:id` ของ B บน draft ของ A → `SR_DELETE_FORBIDDEN` (403); `DELETE .../batch` ของ A ที่มี draft หนึ่งใบและ id `in_progress` หนึ่งตัว → ไม่ลบเลย `SR_BATCH_DELETE_BLOCKED` (400) พร้อมรายการที่ถูกบล็อกด้วย `reason = not_draft`; batch ของ A ที่มี draft ของตนสองใบ → soft-delete ทั้งคู่ |
| 16 | Duplicate SR — **เพิ่ม 2026-09-22** | Requester | SR ที่บันทึกแล้วใด ๆ ในโหมด view | **Duplicate** เปิด `/new?duplicate_id=<id>` โดยส่วนหัวและบรรทัดถูกเติมไว้ล่วงหน้าและฟอร์ม dirty อยู่แล้ว; **Save** สร้าง draft ใหม่พร้อมเลข `draft-<hex>` ใหม่; ต้นฉบับไม่ถูกแตะต้อง |

## 5. การ Map E2E Test

ไฟล์ Playwright สองไฟล์ขับ route ของ SR: `701-sr.spec.ts` (ฟอร์ม: create / edit / การตัดสินใจต่อบรรทัด / submit-approve-reject-send-back-issue; multi-role ผ่าน `createAuthTest` โดย `purchase@blueledgers.com` สำหรับเส้นทาง functional และ `requestor@blueledgers.com` สำหรับ permission denial) และ `720-stock-issue.spec.ts` (list, header, More/Duplicate/Print, การมองเห็นคอลัมน์ตาม role, ปุ่ม Issue, ยอดรวม, แท็บ Stock Movement) ตาม `docs/test-cases/gaps/701-sr-gap.md` case ส่วนใหญ่ของ `701` เดิน UI โดยไม่ assert อะไรเลย (มี `expect()` 2 ตัวในไฟล์) และ locator ของ page-object หลายตัวชี้ไปยังปุ่มที่ไม่มีอยู่จริง ("Save as Draft", "Submit for Approval", "Record Issuance", "Delegate Approvals", auto-save) — 62 case ในรายงาน gap คือ scenario ที่มี oracle รองรับ map ของ describe-block ด้านล่างเก็บไว้เพื่อการวางตัวเท่านั้น
