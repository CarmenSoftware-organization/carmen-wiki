...
| 7 | `JournalEntry` ในฐานะตารางที่ SR เป็นเจ้าของ | Tech Spec นิยาม interface `JournalEntry` พร้อมฟิลด์บัญชีระดับส่วนหัว/บรรทัด | ไม่มีตาราง journal-entry ที่ SR เป็นเจ้าของ และ — ต่างจากข้อ 6 ที่ยืนยันได้ว่าปลายทางคือ `tb_inventory_transaction` สำหรับ stock movement — **รอบนี้ไม่พบ code การ post GL/journal เลยแม้แต่น้อย** (ค้นหาทั่ว repo สำหรับ `journal`, `ledger` ในโมดูล SR และ dependency ของ `inventory-transaction` ไม่พบผลลัพธ์ใด ๆ) | อย่านำเสนอการ post GL ว่าเป็นผลลัพธ์ที่ SR trigger ได้จริงที่ยืนยันแล้ว ให้ถือว่าเป็นเพียงเจตนาการออกแบบจนกว่าจะพบ implementation ในโมดูล finance |
