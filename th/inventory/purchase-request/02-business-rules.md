...
- **Budget** — ตอน submit, `base_total_amount` ของ PR บวกการ allocate ต่อบรรทัดเข้ากับ budget category สร้าง **soft commitment** ในโมดูล budget (`BudgetData.softCommitmentPR`) Commitment ลด `availableBudget` ของงวดที่เกี่ยวข้อง Soft commitment ถูกปล่อยตอน `cancelled` / `voided` และแปลงเป็น hard commitment ตอนสร้าง `tb_purchase_order` (ดู `PR_POST_007` และ [purchase-order](/th/inventory/purchase-order))
