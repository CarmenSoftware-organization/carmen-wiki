...
> **At a Glance**
> **Persona:** Audit / Config (Auditor + System Administrator) &nbsp;·&nbsp; **โมดูล:** [purchase-request](/th/inventory/purchase-request) &nbsp;·&nbsp; **Surface ที่ยืนยันแล้ว:** activity log แบบ generic ต่อเอกสาร (Auditor, read-only — `/system-admin/activity-log`); workflow-stage editor แบบ generic (`/system-admin/workflow`), tax-profile (`/config/tax-profile`), currency + exchange-rate (`/config/currency`, `/config/exchange-rate`), และหน้า user / role (`/system-admin/user`, `/system-admin/role`) ที่ใช้ร่วมกันข้ามทุกประเภทเอกสาร ไม่ใช่เฉพาะ PR (System Administrator) &nbsp;·&nbsp; **ยังไม่ยืนยัน:** "Audit workspace" query builder เฉพาะ PR, "Configuration workspace" เฉพาะ PR, amount-threshold routing, delegation window, หรือ default ต่อประเภท PR
...
> - "PR Workflow Settings" map กับ stage editor **`/system-admin/workflow`** แบบ generic (จริง: stages, SLA, assigned users, `hide_fields` สำหรับแถว `workflow_type = purchase_request` เป็นหนึ่งในหลายประเภท) — แต่การค้นหาทั่ว repo ทั้ง workflow route ฝั่ง frontend และ backend สำหรับ `threshold` และ `delegat` ไม่พบกลไก amount-threshold routing และไม่พบกลไก approval-delegation-window เลย ทั้ง generic และเฉพาะ PR **นี่หมายความว่ารายการ `PR_AUTH_005`** (amount thresholds) **และ `PR_AUTH_006`** (delegation) **ใน [02-business-rules.md](./02-business-rules.md) บรรยาย design intent ที่ยังไม่ยืนยัน ไม่ใช่ live behavior ที่ verified** — หน้านี้ถือว่าทั้งสองยังไม่ยืนยัน; การแก้ไขระดับ business-rules เป็น follow-up นอกขอบเขตของหน้านี้
...
**System Administrator** ตั้งค่า workflow definition ที่ PR อ้างอิงผ่าน `workflow_id` (stages, `stage_role`, การเป็นสมาชิกของ `user_action.execute[]` — เป็น system-config functionality แบบ generic ที่ใช้ร่วมกันข้ามประเภทเอกสารผ่าน `/system-admin/workflow` ไม่ใช่เฉพาะ PR), tax rate (`/config/tax-profile`), currency และ exchange-rate master (`/config/currency`, `/config/exchange-rate`), และ RBAC user/role assignment (`/system-admin/user`, `/system-admin/role`) ไม่พบ amount-threshold editor, delegation-window manager, หรือ screen default ต่อประเภท PR เฉพาะ PR ใดๆ แยกต่างหาก System Administrator (ร่วมกับ Finance) ถือสิทธิ์ **void** ระดับสูงตาม `PR_AUTH_007` ซึ่งเป็น action จริงที่ยืนยันแล้ว แยกจาก screen configuration ข้างต้นทั้งหมด
...
| ตั้งค่า amount threshold หรือ delegation window | **ยังไม่ยืนยันว่ามีอยู่จริง** | **ยังไม่ยืนยันว่ามีอยู่จริง** |
