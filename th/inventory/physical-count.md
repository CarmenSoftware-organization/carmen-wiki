...
- **Rollup ตอน Submit**: ที่ `submit()`, backend จัดกลุ่มทุกบรรทัดที่ variance ไม่เป็นศูนย์ตามเครื่องหมาย และใน transaction เดียว สร้าง **อย่างมากหนึ่ง** `tb_stock_in` (ทุกบรรทัด variance บวก) และ **อย่างมากหนึ่ง** `tb_stock_out` (ทุกบรรทัด variance ลบ) — ทั้งคู่ถูก insert **ที่ `doc_status = completed` ทันที** ต่างจากหน้าจอ [inventory-adjustment](/th/inventory/inventory-adjustment) ซึ่งตั้งแต่ 2026-07-30 สร้างเป็น `draft` และ post เฉพาะเมื่อกด **Commit** rollup นี้ **ไม่** เรียก `InventoryTransactionService.executeAdjustmentIn`/`executeAdjustmentOut` เลย — ไม่มี import หรือเรียก helper ทั้งสองใน `physical-count.service.ts` ไม่มี row `tb_inventory_transaction` ถูกเขียนโดย action นี้ `adjustment_type_id` ถูกปล่อยเป็น `null` บน header ที่สร้างทั้งคู่ (ไม่มี reason code) และไม่มีฟิลด์ `info`/structured ใด ๆ เชื่อม stock-in/out ใหม่กลับไปยัง `tb_physical_count` ต้นทาง — ร่องรอยเดียวคือข้อความ `description`/`note` แบบ human-readable ที่ใช้ร่วมกัน ("Physical Count Adjustment - Period: …") ราคาต่อบรรทัด (`cost_per_unit`) ถูกตีมูลค่าด้วยวิธี costing เดียวระดับ tenant ที่อ่านครั้งเดียวต่อ submit (`enum_business_unit_config_key.physical_count_costing_method`, default `last_receiving`; ไม่พบหน้าจอ UI ใดที่ตั้งค่า key นี้)
- **รอบการนับถูกเปิดจาก Period End (ตั้งแต่ 2026-08-27)** `create()` ใน `physical-count.service.ts` ปฏิเสธการเปิดการนับเว้นแต่ `tb_physical_count_period.status = counting` ของงวดนั้น ("Physical Count Period is not in counting status") และ code path เดียวที่ตั้งค่า `counting` คือ **Start Period Close** บน `/inventory-management/period-end` (`POST /period-ends/start-counting`, `PeriodEndService.startCounting` — ถูก gate ด้วยเอกสาร GRN / stock-in / stock-out / SR ที่ยังค้างอยู่) หน้ารายการ physical-count สะท้อนสิ่งนี้: `pc-component.tsx` แสดง dialog **"Counting has not started"** พร้อมปุ่ม **Go to Period End** เมื่อ `period.status !== "counting"` แทนที่จะเรียก `POST /physical-counts` ช่องว่าง "ไม่มี code path ที่ตั้งค่า `counting`" จากฉบับ 2026-07-15 จึงถูกปิดแล้ว
...
- [system-config/period](/th/inventory/system-config/period) — งวดบัญชีที่การนับดำเนินอยู่ภายใต้ (`tb_inventory_period` → `tb_physical_count_period`); การปิดงวดบล็อกจนกว่าจะนับครบที่สถานที่จำเป็น (`period-end.validate.ts`)
...
- E2E tests: `../carmen-inventory-frontend-e2e/` — ยังไม่มี Playwright spec สำหรับโมดูลนี้; manual catalog `docs/test-cases/750-physical-count.md` (41 case, ตรวจทานซ้ำกับ component จริงเมื่อ 2026-09-20 — รวม dialog "Counting has not started" และเงื่อนไขก่อนหน้า Start Period Close), generated stories `docs/user-stories/750-physical-count.md` (33); gap catalog ของ period-end `docs/test-cases/gaps/900-period-end-gap.md` ครอบคลุมการ unlock (TC-PE-31xxxx)
...
> **Status:** re-sync แล้วเมื่อ 2026-09-22 เทียบกับ frontend จริง (`pc-component.tsx`, `pc-entry-component.tsx`, `pc-review-component.tsx`), backend (`physical-count.service.ts`, `physical-count-period.service.ts`, `period-end.service.ts`, `period-end.validate.ts`), Prisma schema (`tb_period*` → `tb_inventory_period*`, migration `20260916141000`) และ Bruno collection ยังไม่มี E2E Playwright spec; manual catalog `docs/test-cases/750-physical-count.md` (41 case) ถูกตรวจทานซ้ำกับ component จริงเมื่อ 2026-09-20 และตรงกับ implementation แล้ว ขณะที่ `docs/persona-doc/System Process/tx-08-physical-stocktake.md` ยังเป็นการออกแบบระดับวางแผน (transaction type ของตัวเอง, สถานะ `FINALIZED` ที่ post GL, location transaction lock, tolerance/recount) — ดู [physical-count/02-business-rules](/th/inventory/physical-count/02-business-rules) § 5.1
