...
| `submitted_at` | `DateTime @db.Timestamptz(6)` | Yes | คอลัมน์มีไว้เพื่อบันทึก action Submit ของผู้ขาย; ไม่มีโค้ดใดเขียนมันในปัจจุบัน — call Submit ของ portal ภายนอกเล็งไปที่ backend route ที่ไม่มีอยู่จริง (ดู § 5 ข้อ 6 และ [03-user-flow-vendor](/th/inventory/vendor-pricelist/03-user-flow-vendor)) |
...
| 6 | Telemetry การเข้า portal | `portalAccess { firstAccessAt, lastAccessAt, sessionCount, ipAddresses[] }` ต่อ invitation; `emailDetails { sentAt, deliveredAt, openedAt, clickedAt }` ต่อ invitation (design.md) | ไม่มีคอลัมน์ dedicated และ**ไม่มีโค้ดที่เขียนข้อมูลนี้ที่ไหนเลย** — การตรวจสอบทั่ว repo ของ `price-list.service.ts`, `request-for-pricing.service.ts` และ `check-price-list.service.ts` ยืนยันว่าไม่มีตัวไหนสร้างแถว `*_comment` เลย `tb_request_for_pricing_detail_comment` มีอยู่จริงและ CRUD ได้ผ่าน controller เฉพาะของมันเอง แต่เป็นแค่คอมเมนต์ที่ผู้ใช้เขียนเท่านั้น — ไม่มี entry telemetry ชนิด `system` อัตโนมัติ | Design-target เท่านั้น ถ้ามีการ implement telemetry จะลงใน `tb_request_for_pricing_detail_comment` เป็น entry `system`; ไม่มี write path แบบนี้อยู่ในวันนี้ |
...
| 11 | นโยบาย token security ของ email / portal | "Token cryptographically secure ที่มีเวลาการหมดอายุที่ตั้งค่าได้และข้อจำกัดการเข้าถึง"; "การติดตาม IP address"; "การควบคุม session timeout, ข้อจำกัด concurrent session, การตรวจจับ activity ที่น่าสงสัย" (requirements.md § Requirement 28) | `tb_request_for_pricing_detail.pricelist_url_token` (หนึ่งคอลัมน์); `tb_pricelist.url_token` (สำเนา denormalised) | **แก้ไข 2026-07-16 (เดิมระบุผิดว่าบังคับใช้โดย portal middleware):** ไม่มีการตรวจสอบวันหมดอายุ, ข้อจำกัด IP หรือ concurrent-session limit อยู่ที่ไหนเลยใน `check-price-list.service.ts` — token ถูกตรวจสอบแค่ว่ามีอยู่จริงเท่านั้น Design-target ยังไม่ถูก implement |
