...
> **ตรวจสอบซ้ำ 2026-09-22** เทียบกับ `schema.prisma` ที่ backend HEAD `180bd19c3`: `enum_pricelist_status` มีห้าค่า (`submitted` แทรกก่อน `active`, migration `20260721105944`), `submitted_at` ถูกเขียนแล้ว, `tb_vendor` เพิ่ม `tax_no` / `branch_no` / `rating` (`20260909203000`), `tb_vendor_master_certificate` → `tb_certificate` (`20260904133000`) และ detail response expand การอ้างอิงเป็น object (`vendor`, `currency`, `product`, `unit`, `tax_profile`, `currency` ของ template, `vendor` / `pricelist` / `pricelist_template` ของ RFQ — `@ExpandRefs`, 2026-09-17)
...
Header ในแต่ละ tier carry enum สถานะของมันเอง — `enum_pricelist_template_status` สำหรับ template (`draft`, `active`, `inactive`) และ `enum_pricelist_status` สำหรับ pricelist (`draft`, `submitted`, `active`, `inactive`, `expired`); request-for-pricing ไม่มีคอลัมน์สถานะเลย และ — ยืนยันโดยการอ่าน `request-for-pricing.service.ts` — ไม่มีโค้ดแอปที่ derive สถานะขึ้นมาเช่นกัน; สัญญาณต่อผู้ขายเดียวที่ frontend อ่านคือ `has_submitted: !!pricelist_id` วิธีการ submission ของ pricelist จับใน enum แยก `pricelist_submission_method` (`online`, `email`, `portal`, `manual`) แนวคิด carmen/docs ของ "campaign status" (`draft`, `active`, `paused`, `completed`, `cancelled`) และ "invitation submission status" (`pending`, `in-progress`, `submitted`, `approved`, `expired`) **ไม่มีคอลัมน์ Prisma และไม่มีการ derive ระดับแอปพลิเคชัน** — การตรวจสอบทั่ว repo ไม่พบสิ่งใดที่อ่าน `start_date`/`end_date`/`submitted_at` เพื่อคำนวณ state machine ทั้งสองแบบ ถือว่าทั้งสองเป็น design-target เท่านั้น (carmen/docs) ไม่ใช่พฤติกรรมปัจจุบัน

โมเดล Prisma slim กว่าเอกสาร design ของ carmen/docs อย่างมีนัยสำคัญ ซึ่งอธิบายชั้น analytics campaign/invitation เต็มรูปแบบ (อัตราการตอบสนอง, เวลาตอบสนองเฉลี่ย, คะแนนคุณภาพ, การติดตาม IP-address, จำนวน session, telemetry click-through, PII ที่เข้ารหัสตอน rest) ที่ **ไม่มีใน Prisma — และตามที่ยืนยันเมื่อ 2026-07-16 ไม่ได้ implement ในชั้นแอปพลิเคชันด้วยเช่นกัน** Section 5 ทำแคตตาล็อกของ divergence เมื่อ Prisma และ carmen/docs ไม่ตรงกัน Prisma เป็น canonical; ให้ถือว่า semantic ของ workflow และคำอธิบายกติกาใน carmen/docs เป็น design-target เว้นแต่ [02-business-rules](/th/inventory/vendor-pricelist/02-business-rules) ระบุว่า Confirmed
...
ตารางคอมเมนต์ / ไฟล์แนบของโมดูลนี้ถูกแยกไปอีกหน้า — ดู [01a — โมเดลข้อมูล — ตารางคอมเมนต์](/th/inventory/vendor-pricelist/01a-data-model-comments)

### 2.3 tb_request_for_pricing

object "campaign" ในภาษา carmen/docs ผูก template หนึ่งกับ vendor cohort ที่มีวันที่เริ่ม / สิ้นสุด, ข้อความ custom และการอ้างอิง email-template; แถว invitation ต่อผู้ขายทั้งหมด (`tb_request_for_pricing_detail`) แต่ละแถวมี `pricelist_url_token` ของตัวเอง ถูกสร้างใน `create()` call เดียวกันกับ header — ไม่มีขั้นตอน "launch" แยกต่างหาก **ไม่มีคอลัมน์สถานะ Prisma และไม่มีสถานะที่ derive ระดับแอป** — ยืนยันโดยการอ่าน `request-for-pricing.service.ts` ซึ่งไม่เคยคำนวณ `draft`/`active`/`paused`/`completed`/`cancelled` จาก `start_date`/`end_date`/จำนวน pricelist ฝั่ง detail เลย 5-state campaign lifecycle ของ carmen/docs เป็น design-target เท่านั้น
...
แถว invitation ต่อผู้ขาย Carry การอ้างอิงผู้ขายที่เชิญ, contact triplet (ผู้ติดต่อ / โทร / อีเมล), link ไปยัง pricelist (populate เมื่อ check-pricelist call auto-create draft ให้ token นี้เป็นครั้งแรก — ไม่ใช่ตอนผู้ขาย "save" เพราะ save call ของ portal เองไปไม่ถึง backend route ที่ทำงานได้จริง ดู [03-user-flow-vendor](/th/inventory/vendor-pricelist/03-user-flow-vendor)) และ — สำคัญที่สุด — `pricelist_url_token` ที่ให้สิทธิ์เข้า portal สร้างครั้งเดียวตอน RFQ ถูกสร้าง ไม่มีคอลัมน์ "invitation status": ไม่มีโค้ดแอปที่ infer `pending` / `in-progress` / `submitted` / `approved` / `expired` จากอะไรเลย — สัญญาณต่อแถวเดียวที่ frontend มีจริงคือ `has_submitted: !!pricelist_id` 5-state invitation lifecycle ของ carmen/docs เป็น design-target เท่านั้น
...
| `status` | `enum_pricelist_status` | No | สถานะเอกสาร; default `draft` `draft → submitted` (submit บน portal หรือแก้ด้วยมือ) ลบทุกแถว detail ที่ `price IS NULL OR price <= 0` (`price-list.zero-price.ts`) เพื่อไม่ให้การตั้งราคาปลายน้ำหยิบบรรทัด "ไม่เสนอราคา"; `active` เป็นค่าเดียวที่ `price-compare` / การ lookup active-vendor อ่าน |
| `url_token` | `String @db.VarChar` | Yes | Token การเข้า portal; สำเนา denormalised ของ invitation token เขียนเมื่อ portal auto-create draft ตัว token เอง resolve ผ่านแถว `tb_shot_url` ของ platform-schema (`url_token`, JWT `token`, `expired_at` = `end_date` ของ RFQ, `receiver_email`) |
...
| `submitted_at` | `DateTime @db.Timestamptz(6)` | Yes | **ถูกเขียนตั้งแต่ 2026-09 (เดิมไม่มีโค้ดเขียน):** stamp เมื่อสถานะ transition `draft → submitted` — โดย `POST …/pricelist-external/:url_token/submit` ของ portal ผู้ขาย (`CheckPriceListService.submit()`) หรือโดย `PriceListService.update()` เมื่อ Purchaser ตั้ง `status = submitted` transition อื่นไม่แตะต้องค่านี้ |
| `description` | `String @db.VarChar` | Yes | คำอธิบายข้อความอิสระ |
| `note` | `String @db.VarChar` | Yes | โน้ตข้อความอิสระ |
| `info` | `Json @db.JsonB` | Yes | กระเป๋า extension แบบ schemaless; default `{}` ไม่มี service ใดในปัจจุบันอ่านหรือเขียนมัน — รูปร่าง quality-score / validation-results ที่เคยบันทึกไว้ที่นี่เป็น design-target เท่านั้น (§ 5 ข้อ 7) |
| `dimension` | `Json @db.JsonB` | Yes | array cost-dimension; default `[]` |
| `doc_version` | `Int @db.Integer` | No | optimistic-concurrency version; default `0` |
| `created_at`, `created_by_id`, `updated_at`, `updated_by_id`, `deleted_at`, `deleted_by_id` | (audit มาตรฐาน) | Yes | คอลัมน์ audit มาตรฐาน |
...
| `rating` | `Int` | Yes | rating ต่อแถว; default `0` เก็บและ echo กลับเท่านั้น — ไม่มีโค้ดอ่าน (ต่างจาก `tb_vendor.rating` ระดับผู้ขาย § 2.7) |
...
### 2.7 ตาราง vendor master และ certificate (ถูกแตะตั้งแต่ 2026-07-29)

`tb_vendor` เป็นของ master-data แต่เป็นจุดยึดของทุกแถว pricelist และ RFQ และสามคอลัมน์ของมันเปลี่ยนในช่วงนี้ `tb_certificate` (certification master เบื้องหลัง `/vendor-management/certification`) และ `tb_vendor_certificate` (certificate ที่ผู้ขายถือ) ถูกระบุไว้เพราะหน้าจอย้ายเข้ามาในพื้นที่ Vendor Management ของ book นี้เมื่อ 2026-09-03

| ตาราง / ฟิลด์ | Prisma Type | Nullable | คำอธิบาย |
| ----- | ----------- | -------- | ----------- |
| `tb_vendor.tax_profile_id` / `tax_profile_name` / `tax_rate` | `String @db.Uuid` / `VarChar` / `Decimal(15,5)` | Yes | FK ไปยัง `tb_tax_profile` พร้อม snapshot; vendor detail response expand เป็น `tax_profile { id, name }` (ฟอร์ม React อ่านเป็น object ตั้งแต่ 2026-09-17) |
| `tb_vendor.tax_no` | `String @db.VarChar` | Yes | เลขประจำตัวผู้เสียภาษี (ไทย 13 หลัก รูปแบบไม่ถูกบังคับเพราะผู้ขายต่างชาติต่างกัน) มี index (`vendor_tax_no_idx`, non-unique) Migration `20260909203000_add_vendor_tax_branch_rating` |
| `tb_vendor.branch_no` | `String @db.VarChar` | Yes | เลขสาขา; `00000` = สำนักงานใหญ่ เป็น string เพราะเลขศูนย์นำหน้ามีความหมาย |
| `tb_vendor.rating` | `Int @db.Integer` | Yes | rating 1–5 แบบ manual, `null` = ยังไม่ให้คะแนน DB check `vendor_rating_chk` (`BETWEEN 1 AND 5`) และ Zod `int().min(1).max(5)` ใน `vendors.dto.ts` Backend/Bruno เท่านั้น — ฟอร์มผู้ขาย React ยังไม่มี input สำหรับ `tax_no`, `branch_no` หรือ `rating` |
| `tb_certificate` | model | — | เปลี่ยนชื่อจาก `tb_vendor_master_certificate` (migration `20260904133000_rename_eco_label_and_certificate`; index `certificate_code_u`, `certificate_name_u`) คอลัมน์: `code`, `name`, `description`, `note`, `is_active`, `attachments` JSON, `info`, `dimension`, audit Gateway `config_vendor-master-certificates` (`GET/POST/PATCH/DELETE /api/config/{bu}/vendor-master-certificates`, app-id `vendorMasterCertificate.*`) — URL และ app-id ยังใช้ชื่อเดิม |
| `tb_vendor_certificate` | model | — | รูปร่างไม่เปลี่ยน: `vendor_id`, `master_certificate_id → tb_certificate`, `certificate_no`, `issued_date`, `expiry_date`, `attachments`, `is_active`, `description`, `note` Gateway `config_vendor-certificates` (`GET …/vendor-certificates/vendor/:vendor_id`, `POST …/vendor/:vendor_id`, `PATCH/DELETE …/:id`) จัดการจาก section Certificates ของฟอร์มผู้ขาย (`vendor-certificate-section.tsx`, DataGrid ตั้งแต่ 2026-08-27) |
...
- **ไม่มีตาราง campaign-status และไม่มีการ derive เช่นกัน** enum `CollectionCampaign.status` ของ carmen/docs (`draft`, `active`, `paused`, `completed`, `cancelled`) **ไม่ใช่** คอลัมน์ Prisma บน `tb_request_for_pricing` และ (ยืนยันโดยการอ่าน `request-for-pricing.service.ts`) ไม่มีโค้ดแอปที่ derive มันจาก `start_date`, `end_date` หรือจำนวน pricelist ฝั่ง detail เลย Section 5 ทำแคตตาล็อก divergence นี้
...
- **`enum_pricelist_status`** (`tb_pricelist.status`): `draft`, `submitted`, `active`, `inactive`, `expired` (comment ใน schema: สร้างเอกสาร / vendor submitted / พร้อมใช้งานใน compare pricelist / ถูกยกเลิก / หมดอายุ) `draft` คือ window ที่ผู้ขายแก้ (การ save บน portal ยังคง `draft`); `submitted` ถูกตั้งโดย Submit บน portal หรือโดย Purchaser บนฟอร์มภายใน stamp `submitted_at` และตัดแถวที่ไม่มีราคา; `active` เป็น state เดียวที่ `price-compare` และการ lookup active-vendor อ่าน; `inactive` คือ state ระงับ (ถูกบังคับโดย `remove()` ด้วย); `expired` มีใน enum แต่ **ไม่มีโค้ดตั้งค่านี้** — ไม่มี cron auto-expire; filter สถานะของ frontend แสดงเฉพาะ `draft / submitted / active / inactive` state `under-review` / `approved` / `rejected` ของ carmen/docs ยังคงไม่ใช่ค่า enum
...
| 2 | enum campaign status | `'draft' \| 'active' \| 'paused' \| 'completed' \| 'cancelled'` | ไม่มีคอลัมน์ Prisma หรือ enum และ**ไม่มีการ derive ระดับแอปด้วย** — ยืนยันโดยการอ่าน `request-for-pricing.service.ts` ทั้งไฟล์; ไม่มีอะไรคำนวณ state นี้เลย | Design-target เท่านั้น สัญญาณต่อ RFQ เดียวที่ frontend มีจริงคือ `has_submitted` ต่อแถวผู้ขาย |
...
| 4 | สถานะการ submission invitation | `'pending' \| 'in-progress' \| 'submitted' \| 'approved' \| 'expired'` | ไม่มีคอลัมน์ Prisma และไม่มีการ derive ระดับแอปเช่นกัน — ยืนยันโดยการอ่าน `request-for-pricing.service.ts` และ `check-price-list.service.ts`; ทั้งสองไม่คำนวณสิ่งนี้เลย | Design-target เท่านั้น สัญญาณต่อแถวเดียวที่ frontend มีจริงคือ `has_submitted: !!pricelist_id`; ไม่มีการติดตาม first-access, ไม่มี comment `system` ตอน portal เปิด และไม่มี auto-expire บน `end_date` |
| 5 | enum pricelist status | `'draft' \| 'submitted' \| 'under-review' \| 'approved' \| 'rejected'` (design.md) | `enum_pricelist_status = { draft, submitted, active, inactive, expired }` (`submitted` เพิ่ม 2026-07-21) | Mapping: `submitted` ของ carmen/docs = `submitted` ใน Prisma (พร้อม `submitted_at`); `under-review` ไม่มีค่าแยก (pricelist ที่ `submitted` รอ Purchaser); `approved` = `active`; `rejected` ไม่มีค่าและไม่มี comment ถูกเขียน — Purchaser เพียงปล่อย pricelist ไว้ที่ `submitted`/`inactive` หรือแก้มัน |
| 6 | Telemetry การเข้า portal | `portalAccess { firstAccessAt, lastAccessAt, sessionCount, ipAddresses[] }` ต่อ invitation; `emailDetails { sentAt, deliveredAt, openedAt, clickedAt }` ต่อ invitation (design.md) | ไม่มีคอลัมน์ dedicated และยังคงไม่มีแถว comment **implement บางส่วนผ่าน `tb_activity` (2026-09-11/12):** `CheckPriceListService` บันทึก event ของ portal สามอย่าง — `create` (draft ถูก auto-create เมื่อเปิดครั้งแรก), `save` (`vendor.pricelist.draft_saved`) และ `submit` — พร้อม snapshot entity ก่อน/หลัง; endpoint send-email ของ RFQ บันทึก activity `email_sent` (`enum_activity_action`, migration `20260908120000`) ไม่มี telemetry การเปิด/คลิก/IP/session | ชี้ผู้อ่านไปที่ activity log สำหรับประวัติของ portal; ที่เหลือยังคงเป็น design-target |
| 7 | Quality score / ผล validate | `qualityScore` ตัวเลขต่อ pricelist, `validationResults` object structured (design.md § Data Models) | ไม่มีคอลัมน์ dedicated บน `tb_pricelist` และ**ไม่มีโค้ดที่คำนวณหรือเขียน quality score ที่ไหนเลย** — `create()`/`update()` ของ `price-list.service.ts` เก็บฟิลด์ตามที่ client ส่งมาเป๊ะ ๆ โดยไม่มีขั้นตอน scoring หรือ validation-engine ใด ๆ เกินกว่าการตรวจสอบฟิลด์พื้นฐานด้วย Zod | Design-target เท่านั้น ยังไม่ถูก implement |
...
| 11 | นโยบาย token security ของ email / portal | "Token cryptographically secure ที่มีเวลาการหมดอายุที่ตั้งค่าได้และข้อจำกัดการเข้าถึง"; "การติดตาม IP address"; "การควบคุม session timeout, ข้อจำกัด concurrent session, การตรวจจับ activity ที่น่าสงสัย" (requirements.md § Requirement 28) | `tb_request_for_pricing_detail.pricelist_url_token` (หนึ่งคอลัมน์); `tb_pricelist.url_token` (สำเนา denormalised); platform `tb_shot_url { url_token, token (JWT), expired_at, receiver_email }` | **แก้ไขอีกครั้ง 2026-09-22:** การหมดอายุ **ถูก** บังคับใช้แล้ว — `UrlTokenGuard` (gateway) lookup แถว short-URL และคืน 401 `url_token has expired` เมื่อ `expired_at` (= `end_date` ของ RFQ) ผ่านไปแล้ว; ตัว JWT เองถูก sign ด้วย `JWT_EXPIRES_IN` ข้อจำกัด IP, session limit และการ revoke ยังคงไม่ได้ implement |
