...
โมดูล pricelist อยู่ **ติดกับ [[purchase-request]], [[purchase-order]] และ [[good-receive-note]]** ใน chain procure-to-pay แถว PR detail สามารถอ้างอิง `tb_pricelist_detail.id` โดยตรงผ่าน `tb_purchase_request_detail.tb_pricelist_detail` (back-relation ฝั่ง pricelist detail); PO snapshot `price` ของมันจาก pricelist active ที่เวลา PR-to-PO conversion (สำเนา snapshot — ไม่มี FK สด) และ GRN รัน price-variance check ของมันกับ pricelist active เดียวกัน แถว pricelist อ้างอิง [[product]], `tb_unit`, `tb_tax_profile`, `tb_currency` และ `tb_vendor`; แถว request-for-pricing detail อ้างอิง `tb_vendor` (ผู้ขายที่ถูกเชิญ) และ `tb_pricelist` (pricelist ที่ผู้ขาย submit ในที่สุด, แบบเลือกได้จนกว่าจะมีการ submission ครั้งแรก)
...
- โมดูลที่เกี่ยวข้อง: [[product]] (ทุกแถว pricelist detail อ้างอิงสินค้า), [[purchase-request]] (บรรทัด PR default ราคาจาก pricelist active), [[purchase-order]] (PO snapshot ราคา pricelist ตอน PR-to-PO conversion), [[good-receive-note]] (GRN price-variance check รันกับ pricelist active)
