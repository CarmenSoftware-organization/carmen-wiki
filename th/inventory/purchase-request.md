...
- **Approval Level**: ขั้นตอนในสาย workflow ของ PR ที่ตั้งค่าได้ พร้อม role ผู้อนุมัติและเกณฑ์มูลค่า — กฎ routing ของ workflow ข้าม stage หรือกระโดดไป stage เมื่อยอด header (หรือ department / category) ข้ามค่าที่ตั้งไว้ ระดับทั่วไปคือ หัวหน้าแผนก (บังคับสำหรับทุก PR), budget controller (ตรวจสอบกับงบประมาณที่มี), finance review (บังคับสำหรับ PR มูลค่าสูงเกินเกณฑ์ที่ตั้งไว้) และการเซ็นอนุมัติสุดท้ายจาก procurement เส้นทางทั้งหมดถูกบันทึกไว้ใน activity log *(การ delegate การอนุมัติให้ผู้แทนเมื่อผู้อนุมัติหลักไม่อยู่ เคยถูกระบุในเอกสารรุ่นก่อนหน้า — ยังไม่ยืนยัน ไม่พบกลไกดังกล่าว ดู `PR_AUTH_006`)*
...
| System Administrator | ตั้งค่าขั้นตอน workflow และเกณฑ์ routing การอนุมัติ (panel Routing แบบ generic ใน `/system-admin/workflow`), รหัสภาษี, อัตราแลกเปลี่ยน, และการเชื่อมต่อกับโมดูล budget, inventory, vendor และ PO บริหาร role และสิทธิ์ของผู้ใช้ *(กฎ delegation และค่า default ของ PR type เคยถูกระบุในเอกสารรุ่นก่อนหน้า — ยังไม่ยืนยัน ไม่พบหน้าจอหรือกลไกที่ตรงกัน ดู `PR_AUTH_006`)* |
