...
**The enum type itself was never dropped.** `enum enum_email_sender_purpose` is still declared in the schema (`schema.prisma:1415-1425`), referencing zero columns anywhere — a repository-wide grep for the type name finds only its own declaration. It is also not the original three-value enum any more: migration `20260808120000_email_sender_purpose_per_flow` — one hour **before** the `20260808130000_email_profile_master_and_routing` redesign migration above, on the same day — added the five mail-flow names (`register`, `verify_email`, `invitation`, `forgot_password`, `notification`) onto this **same** enum type, so it now carries eight values spanning two unrelated eras of the design, referenced by nothing. That earlier migration's own comment states the fallback rule accurately *for the moment it ran*: "flows without their own profile fall back to `no_reply` in code" — true for the hour between the two migrations, and false an hour later once `20260808130000` replaced the fixed `no_reply` fallback with an operator-configured `default` in `email_routing` (§3).

**Two more doc-comments carry the same pre-redesign claim, in files this page cites elsewhere.** The Prisma model comment directly above `tb_email_sender_profile` (`schema.prisma:1428-1431`) still reads "One active row per purpose... so `no_reply` can live on a different provider than support" — the one-row-per-purpose model this same migration pair replaced. `EmailSenderProfileService`'s own class-level JSDoc (`email-sender-profile.service.ts:58-60`, micro-cluster) makes the identical claim in different words: "CRUD with secret encryption, masking, and one-non-deleted-row-per-purpose enforcement." Both are false of the code beneath them today — the service's own `create()`/`update()` (§5) enforce uniqueness on `name`, not `purpose`, and `purpose` no longer exists as a column at all. Do not use any of these three comments — the orphaned enum's, the table's, or the service class's — as a citation for current fallback or uniqueness behavior; all three describe the architecture the 2026-08-08 migration pair replaced.
...
- **Test the three specific sessions the landing page's §4.3 table names, not just "`email_setting.*` alone."** `email_setting.*` without any `platform_config.*` reproduces only the read-side load error — the Edit Routing button is gated on `!routingError`, so a failed `GET` means the button never renders, and the write-side trap is unreachable from that grant. The write-side trap needs `platform_config.read` present (so the panel loads and the button appears) with `platform_config.manage` absent — a different, narrower combination (landing page §4.3). Whether a real deployment ever seeds a role with `platform_config.read` but not `.manage` was not established from source and is called out there as an open question, not assumed.
...
- `../carmen-turborepo-backend-v2/packages/prisma-shared-schema-platform/prisma/migrations/20260808120000_email_sender_purpose_per_flow/migration.sql` — added the five flow-name values onto `enum_email_sender_purpose`, one hour before the redesign below (§2.1).
...
- `../carmen-turborepo-backend-v2/apps/micro-cluster/src/cluster/email-sender-profile/email-sender-profile.service.ts` (lines 58-60) — CRUD, `doc_version` enforcement, `passwordPatch()`, name-uniqueness (§5), and the stale class-level JSDoc (§2.1).
