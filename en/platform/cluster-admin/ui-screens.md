...
![Cluster Admin — UI Screens form screen](/screenshots/platform/cluster-admin/ui-screens-form.png)
...
3. **Header actions are Export only.** The list is otherwise the same shape: debounced search, a status Filters sheet, `localStorage`-persisted state (its own `_ca_business_units`-suffixed keys, distinct from the platform list's), and Created/Updated audit columns via the shared `auditColumns()`. Since PR #293 the **HQ** column sorts server-side (`is_hq`, a real column the backend already ordered by — the header had simply been closed with `enableSorting: false`).
...
**Layout:** a `BuPropertyPlate` hero (logo/avatar, an inline-editable name, `is_active`/`is_hq` toggles, the read-only `code`, and a `SeatMeter` for the cluster-wide seat pool) sits above a **6-tab document** — Overview, People, Hotel, Company, Configuration, and (since PR #291, 2026-09-09) **Licenses** — replacing the platform edit page's single continuous form for this narrower scope. The Licenses tab owns no form field, so it can never be selected by the save-time "jump to the tab with errors" logic; it is in `CLUSTER_BU_TAB_IDS` because the same list validates the `?tab=licenses` deep link that every row on `/cluster-admin/:clusterId/licenses` (§7) points at.
...
| People | Users tab | A fully editable `BusinessUnitUsersCard` (add/edit/remove BU membership). Before PR #291 this tab also held the read-only licences summary; that moved to its own tab |
| Licenses (PR #291) | Licenses tab | Two **read-only** cards, the same components the platform form renders: `BusinessUnitLicensesCard` (seat summary + this BU's subscriptions, fetched through the membership-checked `/clusters/:id/subscriptions` route rather than `/platform/subscriptions`, with the subscription rows rendered as plain text — `subscriptionHref={() => undefined}` — because `/licenses/subscriptions/:id/edit` needs `subscription.read`) and `BusinessUnitInterfaceLicensesCard` (the BU's interface licences, badges from the backend's `in_force`). Neither card gets a `createHref`/`editHref`, and both "Manage licences" links point at this shell's own `/cluster-admin/:clusterId/licenses`. The card deliberately omits the cluster-seat line the platform form shows, since the plate above already states it on every tab. Tab badge = subscription count + interface-licence count |
...
The one screen in this module confirmed to have **no write path anywhere**. The page's own comment explains why this is not a hidden-button decision but a structural one: a cluster admin holds no RBAC permission in their session at all (membership comes from `tb_cluster_user`, not `tb_user_tb_platform_role`), every licence-write endpoint requires `subscription.manage` at the backend, and this page therefore never calls `GET /platform/subscriptions` — since PR #291 it reads the cluster's contracts through `GET /api-system/clusters/:id/subscriptions`, the same membership-checked route `ClusterAdminRoute` mirrors, so the read no longer 403s. The 2026-09-06 revision of this page recorded that a grep of the page found no `<Link to=` at all; that is no longer true — the two cards added in PR #291 link each row to the owning BU's Licenses tab **inside this shell** (`/cluster-admin/:clusterId/business-units/:buId/edit?tab=licenses`, §5), never to the platform's `/licenses/subscriptions/:id/edit` or `/licenses/interface/:id/edit`, which need `subscription.read`. Every write affordance is still absent.

Layout (PR #291 reordered it around the four ledgers):
...
2. **`SubscriptionsCard`** (`clusterAdmin/licenses/SubscriptionsCard.tsx`, on a shared `CoverageCard` base) — every subscription in the cluster, grouped by BU and ordered by end date, each row with the backend-computed state badge, a coverage bar, and an expiring-soon badge against `subscription_days`. Rows link to the BU's Licenses tab in this shell **even for a caller who also holds `subscription.read`** — per the source comment, leaving the cluster-admin shell mid-task loses the reader's context.
3. **`InterfaceLicensesCard`** (`clusterAdmin/licenses/InterfaceLicensesCard.tsx`) — every interface licence across every BU in the cluster, fetched per BU with `Promise.allSettled` (`useClusterInterfaceLicenses`, mirroring the seat table so a BU that fails to answer fails the same way in both), flattened BU × licence and ordered by expiry, since a cluster admin's question is "which one must be renewed first." **Only `in_force` rows are drawn**; expired, scheduled, and capped-by-contract rows are counted in the header but not listed (the owner's ruling). Badges come from the backend's `in_force`/`state`, never from dates. A BU that could not be loaded is named in a "could not load N units" line with a Retry, not rendered as if it had no licences.
4. **`SeatsByBuTable`** — one row per business unit in the cluster, each showing its own summed active-seat count and nearest expiry, fetched in parallel per BU (`useClusterSeatLicenses`, `Promise.allSettled` since there is no per-cluster seat endpoint) and explicit about a failed-to-load row: it renders "could not load" text, never a silent `0`, since in this system zero seats means new users genuinely cannot be invited.
5. **`QuotaLedgerCard`** (collapsed by default) — every BU-quota purchase row for the cluster, with the single currently-winning licence (`activeLicense`, newest `start_date`) badged "In force" — reinforcing the winner-take-all counting rule [Licenses — Data Model](/en/platform/licenses/data-model) §3 documents, never a sum.
6. **`BuRankingCard`** (collapsed by default) — every business unit ranked by the same `rankBusinessUnits()` formula as `BusinessUnitList`'s Over-limit badge (§4), answering "if the cluster is over quota, which BU gets cut first" — a question only worth surfacing once the strip above has already turned a warning colour.
...
All paths `../carmen-platform` (HEAD `f1c69f1`, 2026-09-22).
