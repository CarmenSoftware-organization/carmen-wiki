...
The gateway's `parseOptions()` (`preconfig-imports.controller.ts:42-43,48-85`) validates `duplicate_mode` against the three legal strings and `clear_existing`/`accept_lookup_creation` against `boolean`, rejecting a wrong type with 400 — but its own doc-comment ("anything else … is a 400") overstates what the code does: an **unknown extra key** in the submitted `options` JSON is silently dropped, not rejected. Read the function, not the comment, if testing malformed payloads.
...
**This write is not gated by `cluster.update`, the permission key the ordinary edit page's own route requires.** `PUT /api-system/business-units/:business_unit_id` is guarded only by `AppIdGuard('businessUnit.update')` (`platform_business-units.controller.ts:374-375`) — an **application-identity allowlist check** against the `x-app-id` header (`app-id.guard.ts`), not a platform-RBAC permission check against the calling user. The endpoint's class-level guard is `KeycloakGuard` alone (authentication only); no `PlatformPermissionGuard`/`@RequirePlatformPermission` sits on this route at all.

The strongest form of this evidence is **inside the same controller**: its `POST` (create) and `DELETE` (delete) routes on this exact class both stack `AppIdGuard` **and** `PlatformPermissionGuard` + `@RequirePlatformPermission('cluster.create')` / `('cluster.delete')` (`platform_business-units.controller.ts:282-283`, `:633-634`) — the pattern every other mutating route in this book follows. `PUT` (update), alone among the three, drops the `PlatformPermissionGuard`/`@RequirePlatformPermission` pair and keeps only `AppIdGuard`. This is not a comparison across unrelated files that could be explained by differing conventions elsewhere; it is one class where two sibling routes demonstrably know how to add the check and the third one does not.

And this is a genuine user-session write, not a service-to-service call `AppIdGuard` alone would be a reasonable gate for: the shared `api` client sends both the user's own bearer token (`Authorization: Bearer ${token}`, from `localStorage`, attached per-request by the request interceptor) and a single build-time `x-app-id` value (`import.meta.env.REACT_APP_API_APP_ID`, the same for every user of the deployed SPA) on every call (`../carmen-platform/src/services/api.ts:8,23`) — so `AppIdGuard` passing tells the backend only "some session of this SPA build is calling," never anything about which permissions that particular user holds.

Concretely: a session holding `data_import.manage` but **not** `cluster.update` is blocked by the frontend from ever opening `/business-units/:id/edit`, but is **not** blocked, at either the frontend (no `<Can>` gate wraps `CompanyProfilePanel`'s Apply button) or the backend, from writing the same fields through this wizard's Company Profile step — the `cluster.update` key that looks like it should also gate this write is never checked on this path. This is a gap in the shared `PUT` endpoint itself, not something specific to this wizard; see [Business Units](/en/platform/business-units) §4 for the pointer back from that module's own permissions table.
...
- `../carmen-turborepo-backend-v2/apps/backend-gateway/src/platform/platform_business-units/platform_business-units.controller.ts:66-70,282-283,374-375,633-634` — the Business Units controller's class-level guard, the `PUT` route's `AppIdGuard`-only gate, and the sibling `POST`/`DELETE` routes' `PlatformPermissionGuard` + `@RequirePlatformPermission` pair (§3.5).
...
- `../carmen-platform/src/services/api.ts:8,23` — the shared client's build-time `x-app-id` default header and its per-request bearer-token interceptor, cited in §3.5 to establish this is a genuine user-session write.
