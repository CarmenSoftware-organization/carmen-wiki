...
> **At a Glance**
> **Module purpose:** Permission-based access control — catalog defines `resource.action` keys, roles bundle keys, scoped assignments bind roles to users, super-admin flag bypasses all checks &nbsp;·&nbsp; **Audience:** Developers and QA working on the Platform admin SPA and its authorization backend &nbsp;·&nbsp; **Key entities/tables:** `tb_platform_permission`, `tb_platform_role`, `tb_platform_role_tb_permission`, `tb_user_tb_platform_role` (scope via nullable `cluster_id`), `tb_platform_super_admin` — all five carry `doc_version` (2026-07-16 platform-wide optimistic-lock rollout) &nbsp;·&nbsp; **Screens:** Roles · Permission Catalog · Super Admins · User Platform &nbsp;·&nbsp; **Sub-pages:** 3 &nbsp;·&nbsp; **Permission keys renamed 2026-08-20:** `role.*` → `platform_role.*` everywhere, the standalone `rbac.read` key some earlier screens used was retired, and Permission Catalog's route moved from `/platform/permissions` to `/platform/category-permissions` (commit `8df0b10`) &nbsp;·&nbsp; **Since 2026-09-02:** `RoleManagement`/`RoleEdit` measure a role's permission count against the **catalog size**, not the widest role, and `RoleEdit`'s permission picker became a per-resource-row grid with toggle buttons that also shows withheld actions (`#252`/`#253`); `SuperAdminManagement` was rewritten a second time, from a `DataTable` into a people-roster list with a self-removal guard (`#244`)

**Scope note (2026-09-05 resync):** this page documents the **Roles** screens and **Permission Catalog** in full. **User Platform** and **Super Admins** are covered here at a summary level only — the current platform re-sync plan lists both as future standalone Platform modules (they already have independent nav rows, permission keys, and e2e suites), so a full line-by-line verification of those two screens is deferred to whichever task creates those modules. Facts stated below about them were confirmed against current source but the sub-pages have not been rewritten as exhaustively as the Roles/Permission Catalog material.
...
- **Menu-derived ordering.** A role's permission grid and the read-only grant view both list resources in the same order the sidebar itself uses (`NAV_RESOURCE_ORDER`/`resourceRank()` in `platformNav.ts`, derived from each nav item's `permission` prefix, first appearance wins). A resource with no menu row of its own (e.g. `rbac`) sorts after every menu-backed resource, in catalog order. This is why, say, `cluster` permissions always appear before `news` permissions in the picker — it mirrors the sidebar the reader just came from, not an alphabetical or database order.
