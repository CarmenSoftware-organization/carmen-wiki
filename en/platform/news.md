...
- **`published_at` is server-stamped, once.** On create with `status = published` and on the first transition into `published`, micro-cluster stamps `published_at = now()` — but only when the record has never carried a publish time. Moving back to draft or archived does **not** clear it, and re-publishing later keeps the *original* stamp. API callers may set or clear `published_at` explicitly; the SPA never sends the field and renders it read-only (helper text: Set automatically by the server when status becomes "Published".).
