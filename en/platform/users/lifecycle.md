...
**Success:** On a successful `POST`, the SPA calls `navigate('/users/:id/edit', { replace: true })` (where `:id` is the newly created user's UUID), switching to the edit route for the new record (which opens in view mode). A `toast.success('User created successfully')` appears.
