...
**A source comment in this codebase is stale — verify counts against generator data, not against it.** `FeatureCatalogPanel.tsx`'s own doc-comment and `ModuleShelf.tsx`'s describe "76 rows" / "10 modules + 66 children" — both predate the 2026-09-03 tree work. Counting `"key":`/`"parent_key": null` entries directly in `seed.license-feature.data.ts` (the generator's own output, `../carmen-turborepo-backend-v2` HEAD `ef4d6f08f`, 2026-09-22) gives **107 rows across 12 root modules** — `accounting`, `configuration`, `dashboard`, `interface`, `inventory_management`, `operation_plan`, `procurement`, `product_management`, `report`, `store_operations`, `system_admin`, `vendor_management` — of which **100 are seeded `active`** (sellable today) and **7 are seeded `inactive`** (all `accounting.*` keys registered ahead of their real endpoints, per Phase C of the design doc — seeded not-active specifically so a feature with no route behind it cannot sell on day one, the same class of bug the generator's own code comments say happened once already with `report.schedule`). The count moved from 89/11/79/10 (this page's 2026-09-06 baseline) by +19/−2 keys, diffed directly against the 2026-09-05 seed file: the **`interface` module** (12 keys: `interface`, three category rows `interface.{accounting,pos,pms}`, and eight brand leaves — `carmen_gl`, `blueledgers`, `external`, `micros`, `infrasys`, `square`, `opera`, `protel`), added 2026-09-08 when interface entitlement became a licence feature; five `configuration.*` keys (`chart_of_account_mapping`, `email_profile`, `email_template`, `cost_center`, `cost_center_group`); two `accounting.gl.*` keys (`budget`, `jv_template`, for the new GL budget/JV-template endpoints); and the rename `system_admin.period` → `system_admin.inventory_period` (2026-09-16, `20260916140000_rename_period_to_inventory_period` — the same migration renamed the permission resource and the `period.*` `api_name`s, and a follow-up `20260916150000` fixed `tb_license_feature_group_item.feature_key`, which the first migration had missed because that join is a soft string reference with no FK, leaving every BU's group pointing at a key that no longer existed and `LicenseInterceptor` answering `403 LICENSE_REQUIRED` on every `/inventory-periods` call). `system_admin.query_dataset` was removed. The `interface.*` keys and three of the `configuration.*` keys (`chart_of_account_mapping`, `email_profile`, `email_template`) arrive through a **new generator source set, `LICENSE_ONLY_RESOURCES`** (`permission.route-map.ts:429`): keys that are real and already sold but have **no permission and no route** behind them, which the generator emits as `active` and exempts from `assert_planned_sets_agree()`. They could not go through the pre-existing `PLANNED_LICENSE_RESOURCES` set, because that set forces `inactive` (a group could keep such a key but never add it, so nobody could ever sell an interface to a new BU) and additionally requires membership in `PLANNED_RESOURCES`, which would lie to `check-endpoint-permission`. No `interface.*` key appears in `license-catalog.generated.ts`'s route→feature map, so `LicenseInterceptor` never gates an interface route — enforcement is by the inventory frontend reading `GET /api/license` ([Licenses](/en/platform/licenses) §3.6). See [Data Model](/en/platform/license-catalog/data-model) §3 for the full shape and the hide-at-a-middle-tier hazard this tree structure creates.
...
### 3.4 Bundles are curated, not derived from the module tree — and since 2026-09-10 they have a kind

A bundle (`tb_license_feature_group`) is a hand-picked, free-form set of feature keys — it can span `inventory.count` and `report.daily` in one group, unlike a "module" which is always a contiguous branch of the tree (§3.2). Since migration `20260910000000_license_feature_group_kind` every bundle also carries `kind`: `standard` (sold on the main subscription — the default) or `interface` (sold one-per-licence on an interface licence, `INF`). The kind is chosen with a segmented control on the create form and is **locked afterward** — the edit form shows it read-only with the hint "Set when the group is created — changing it would move sold entitlements between license kinds", and the backend's update DTO does not accept it. The Bundles list marks `interface` bundles with a badge and exports the kind in CSV. The kind is not derived from the keys inside (the backfill on 2026-09-10 did derive it once, marking as `interface` every group whose live items were *all* `interface`/`interface.*`, but mixed and empty groups stayed `standard`) — the column is the source of truth, and the backend refuses with 400 both an `interface` group on a subscription and a `standard` group on an interface licence. The design reasoning — why a column and not a code prefix or key inspection — is in `../carmen-platform/docs/superpowers/specs/2026-09-09-interface-license-split-design.md` §2. Two rules govern what a bundle can actually hold: **the server enforces "a selected child drags its full ancestor chain in with it"** on every `PUT .../features` call (walking `parent_key` to full depth before validating, not just once at the first level) — so a bundle can never be saved holding a feature but missing one of its parent modules, which the license evaluator would otherwise treat as an incomplete grant. The picker UI (`FeatureSelectionCard`, used **only** by [UI Screens](/en/platform/license-catalog/ui-screens) §5 now — it was removed from the sales/subscription flow entirely in License Catalog's own Phase 4, per the same design doc) enforces the identical rule client-side, but the server-side check is the real gate, since `PUT` can be called directly.
...
- `../carmen-turborepo-backend-v2/packages/prisma-shared-schema-platform/prisma/seed.license-feature.data.ts` — the generator's own output; source of the verified 107/12/100/7 counts in §3.2.
- `../carmen-turborepo-backend-v2/packages/prisma-shared-schema-platform/prisma/permission.route-map.ts:429` — `LICENSE_ONLY_RESOURCES`, the source set behind the `interface.*` and three `configuration.*` keys (§3.2); `prisma/migrations/20260910000000_license_feature_group_kind/migration.sql` — the group `kind` column and its backfill (§3.4).
