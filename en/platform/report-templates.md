...
- [print-template-mapping](/en/platform/print-template-mapping) — **removed 2026-07-23/24** (historical page only); used to be the routing table that decided which `kind="print"` template rendered each document type. That decision is now made on this table directly via `template_type = "form"` + `report_group` + `is_default`, surfaced by the new [Form Groups](/en/platform/report-templates/form-groups) screen — see the print-template-mapping page for the full removal timeline and the one capability it did not carry over (per-BU default scoping)
...
- [Form Groups](/en/platform/report-templates/form-groups) — the `/report-form-groups` screen: fixed 12-code group cards, the "set as default" flow's two-step non-transactional `PUT` pair, and how it replaced print-template-mapping's grouped-card list.
