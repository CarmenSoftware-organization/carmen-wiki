...
- **Template type (`list` vs. `form`)**: The `template_type` column (renamed from `kind` on 2026-07-23; values also changed from `"report"`/`"print"` to `"list"`/`"form"`) splits the catalogue into user-facing tabular analytical reports (`"list"`) and single-record document layouts (`"form"`). Unlike the old `kind`, `template_type` **is** exposed and required in the SPA edit form (a select: List / Form) — it also changes which other fields the form shows (`is_standard`/BU-scope chips for list templates; `is_default` + a fixed `report_group` dropdown for form templates).
- **Report group and default (form templates only)**: For `template_type = "form"`, `report_group` is constrained by the SPA to a fixed 12-code list (`FORM_REPORT_GROUPS`: `PR, PO, GRN, SR, CN, SI, SO, IA, PC, SC, RFP, EOP`), and `is_default` marks the one live template per group used when a business unit hasn't picked one — enforced by a DB partial unique index (`idx_report_template_default_per_group`), not just client-side. The **Form Groups** screen (`/report-form-groups`) is the dedicated surface for setting this; see [Related Modules](#5-related-modules).
...
- [print-template-mapping](/en/platform/print-template-mapping) — **removed 2026-07-23/24** (historical page only); used to be the routing table that decided which `kind="print"` template rendered each document type. That decision is now made on this table directly via `template_type = "form"` + `report_group` + `is_default`, surfaced by the new [Form Groups screen](#1-overview) — see the print-template-mapping page for the full removal timeline and the one capability it did not carry over (per-BU default scoping)
...
- Backend: `../carmen-turborepo-backend-v2/packages/prisma-shared-schema-platform/prisma/migrations/20260723120000_print_form_default/migration.sql` — `is_default` added, `idx_report_template_default_per_group` unique index, `tb_print_template_mapping` dropped
...
- [UI Screens](/en/platform/report-templates/ui-screens) — `ReportTemplateManagement` list with Status + Source Type + Template Type filters, 2-pane `ReportTemplateEdit` (left: Template Info + Business Unit Scope + Metadata + Data Source cards; right: 3-tab CodeMirror — Dialog XML / Content XML / Preview), inline Browse-in-BU panel, sticky action bar, not-found gating, and doc_version optimistic locking.
