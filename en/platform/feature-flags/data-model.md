...
Two menu rows carry **no** feature key at all and so have no line in this table: **Feature Flags** itself (`/platform/features`, permission-gated only, §4 of the landing page) and **Super Admins**' *nav-level* superAdminOnly check is independent of its `feature: 'super_admins'` key (row 20) — the key still exists and still gates the row via `hide`/`inactive`, it is `superAdminOnly` that is the second, separate gate on top of it.
