...
> **At a Glance**
> **Screen:** `ReportFormGroupManagement` (`/report-form-groups`, added 2026-07-24) &nbsp;·&nbsp; **Route gate:** `report_template.read` (reused — no new permission keys) **and** `feature="report_form_groups"` — its own feature key, separate from Report Templates' `report_templates` &nbsp;·&nbsp; **Sidebar:** "Form Groups" entry in the Content group (`platformNav.ts`) &nbsp;·&nbsp; **Replaces:** print-template-mapping's grouped-card list — that module was removed from the product the same week (carmen-platform commit `de11377`, 2026-07-24) &nbsp;·&nbsp; **Data:** every `tb_report_template` row where `template_type = "form"`, grouped by `report_group` &nbsp;·&nbsp; **Since 2026-08-22:** each row shows a compact audit line (latest actor, created or updated)

## 1. Overview

Form Groups is the screen where an admin picks, per document type, which single-record **form** template a business unit gets by default when it hasn't chosen one explicitly. It replaced the print-template-mapping module's own grouped-card list in the same 2026-07-23/24 change that dropped `tb_print_template_mapping` and added `tb_report_template.is_default` — see [Related Modules](#6-related-modules) below for the full removal timeline and [Report Templates](/en/platform/report-templates) §1/§3 for the `template_type`/`is_default` column pair this screen edits. This page documents the screen itself; it was previously covered only in prose on the parent [Report Templates](/en/platform/report-templates) landing page and on the now-deleted print-template-mapping page.
...
- **Print Template Mapping** — the module this screen's job replaced, removed from the product 2026-07-24 (carmen-platform commit `de11377`; its backend-gateway proxy and `print_template_mapping.*` permission keys were removed the day before, 2026-07-23, carmen-turborepo-backend-v2 commit `c135bb21e`). One capability did not carry over: per-business-unit default scoping — the removed module could route a different default template per BU, and nothing in this screen's `is_default` mechanism replaces that.
