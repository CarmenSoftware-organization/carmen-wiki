...
- **Re-verified directly against `BroadcastCompose.tsx`: Reset, the audience's Business Unit tab, and every message/type/delivery field carry no `<Can>` wrapper at all — only Send does.** The two system-audience tabs are the one nuance: they are not wrapped in `<Can>` either, but they *are* conditionally rendered behind a raw `hasPermission('broadcast.send')` check (§2), which — as noted above — never actually excludes anyone, since reaching this page at all already requires that same key.
...
3. **Membership is still evaluated per query, not per send** for `system_all`/`bu` — a user added to a BU after a broadcast was sent still sees it, indefinitely in the unread list (no time window) and for 30 days in the recent list (`GET /api/notifications/recent`'s fixed 30-day window); a user removed stops seeing it. There is no recipient snapshot.
...
| 13 | `.send`/`.read`/`.update`/`.delete`-less session calls the matching endpoint directly | `PlatformPermissionGuard` rejects with 403 (message "Missing platform permission: `<key>`", per the fix recorded in `BroadcastCompose.tsx`'s own code comments for `broadcast.send`) before the request reaches micro-notification, for **all six** routes now, not just the two send endpoints | Verify each of the four keys independently — this task extended coverage well beyond the send-only enforcement the last sync verified |
...
| 16 | Scheduled time set in the past | Client validation rejects it ("Scheduled time must be in the future", checked against `Date.now()` at validation time); the Zod request schema (`scheduled_at: z.string().datetime().optional()`) has **no future check server-side** — an API caller can create a row with a past `scheduled_at` that is immediately visible (it already passes the `<= NOW()` filter) but was never live-pushed | Unchanged since the pre-redesign wiki. The SPA can also slip a past time through its own UI: validation runs when Send is clicked, but confirming the dialog does **not** re-validate, so a dialog left open past the scheduled instant still submits a now-past `scheduled_at` |
...
- **Read-state QA needs two users**: send a `system_all`/`bu` broadcast, confirm both see it unread (no `tb_user_broadcast_action` rows exist yet), have one mark it read, and confirm the other's unread state is untouched — unchanged by the redesign.
- **`dismissed_at` and `tb_user_broadcast_action.doc_version` remain genuinely dormant** — seeded/declared but unused by any code path; don't file their absence from the UI as defects. This is narrower than the pre-redesign wiki's equivalent note, which also listed `broadcast.read`, `end_at`, and the broadcast table's own `doc_version` as dormant — all three are now real and active (§1, §4).
