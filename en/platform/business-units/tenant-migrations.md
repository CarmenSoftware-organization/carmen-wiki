...
The Business Units edit page's Technical tab embeds `TenantMigrationCard` (existing BUs only), a scaled-down version of the fleet screen scoped to the one BU currently open: it checks that BU's own tenant-database migration status, applies pending migrations via the same streamed-progress mechanism behind the same confirm dialog, and — since `../carmen-platform` PR #297 (2026-09-16) — can **resolve** a stuck migration through the same `TenantMigrationResolveDialog` the fleet table uses. A failed status check is now kept on the card (`role="alert"`, raw message) instead of being swallowed into a four-second toast, so the Resolve button has an error to attach to and a migration name to pre-fill. Full screen-tour detail — layout, disabled states, the raw-output toggle — is documented in [Business Units — UI Screens](/en/platform/business-units/ui-screens) §4.5, where it lives alongside its one remaining sibling advanced card (Tenant Seed; the former third card, Interface Entitlement, was removed in PR #286 when interface entitlement became a licence — see [Licenses](/en/platform/licenses) §3.6).

The card and the fleet-wide screen call the exact same `tenantMigrationService` (`getStatus`, `deployStream`, `resolve`) and the same backend controller — there is one implementation of "check status," "apply migrations," and "resolve," surfaced from two screens. See [Tenant Migrations](/en/platform/tenant-migrations) §3.5 for the full comparison, including a re-verified correction: both surfaces gate **every** action, including the read-only status check, on `isSuperAdmin` — the card is not more permissive here than the fleet table. The one real difference is that the card additionally disables its buttons when the BU has no database pool/schema configured yet (`hasDbConnection`), a pre-check the fleet table does not perform.
...
- `../carmen-platform/src/components/TenantMigrationResolveDialog.tsx`, `src/utils/migrationError.ts` — the shared Resolve dialog and its name parser.
- [Business Units — UI Screens](/en/platform/business-units/ui-screens) §4.5 — full screen-tour detail for this card, alongside Tenant Seed.
