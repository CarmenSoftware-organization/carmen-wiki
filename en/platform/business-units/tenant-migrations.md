...
description: Fleet-wide standalone screen (/tenant-migrations) that checks and applies pending tenant-database schema migrations across every business unit — reuses cluster.read (and the tenant_migrations feature flag) at the route, but every actual action is additionally gated to super-admin, both client-side and at the backend guard.
published: true
date: 2026-09-05T07:35:00.000Z
...
- **Fleet Sync summary card** (`FleetSync`) — a horizontal card showing `{in-sync count} / {total BUs}` in large monospace type, a three-segment (green/amber/red) progress bar for in-sync/behind/errored, and a legend row with counts. Before the first "Check all" run it shows "Not checked yet" instead of the bar. The card's action slot on the right holds **Check all**, **Deploy all**, and **Export** buttons.
...
- **BU table** (`DataTable`, 3 sticky-left columns) — columns: **Code** (links to `/business-units/:id/edit`), **Name**, **Status** (a badge — "In sync" / "N behind" / "Error" / "Not checked" — plus an inline "Applying X/Y … current-migration-name" line while a per-row Apply is streaming, or an inline error message), **Last checked** (HH:MM:SS, client-local, narrowed to a fixed `w-28` since the last sync), and a row-actions column (icon-only **Check** and, only when that row has pending migrations, **Apply**). **The on-screen Pending column was removed since the last sync** — the Status badge already states the count ("3 behind" / "In sync"), so a separate numeric column next to it repeated the same fact for no benefit. The CSV export (below) still includes a Pending count column, where a bare number is genuinely useful for further calculation.
...
**Deploy all's own labeling changed since the last sync.** The button used to render as a permanently "loud" destructive-red action regardless of state — red before "Check all" had even run (when the page does not yet know what it would touch) and still red at 0 pending. It now reads `variant={behindCount > 0 ? 'destructive' : 'outline'}`: destructive-red only when there is a nonzero number of BUs behind, labeled with its own blast radius ("Deploy 3 behind") instead of the generic "Deploy all", and disabled (not merely muted) once a check has run and found nothing pending. The rationale, from the button's own source comment, is that this domain has no cancel/rollback endpoint, so the most visually urgent control on the screen should not be the one you can least undo.
...
- `../carmen-platform/src/components/TenantMigrationCard.tsx` — the per-BU embedded card documented in [Business Units — UI Screens](/en/platform/business-units/ui-screens) §4.5, sharing the same service.
- `../carmen-platform/src/App.tsx:251` — the `/tenant-migrations` route (`requiredPermission="cluster.read"` plus `feature="tenant_migrations"`); `src/components/nav/platformNav.ts` — the sidebar entry (nav items live here now, not in `Layout.tsx`).
...
- [Business Units](/en/platform/business-units) and [Business Units — UI Screens](/en/platform/business-units/ui-screens) §4.5 — the per-BU card and the `cluster.*` key-reuse gotcha this page inherits.
