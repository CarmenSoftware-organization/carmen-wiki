...
### 4.13 Advanced cards (existing BUs only, actions super-admin-gated)

Three cards render after Database Connection whenever `!isNew` — rendering itself is not gated on `canEdit` or `isSuperAdmin` at all: any session that reaches the edit page (i.e., holds `cluster.update` somewhere) sees all three cards, including a non-super-admin `cluster.update` holder. `isSuperAdmin` is checked only *inside* each card, as a `disabledReason` that locks that card's own actions:

- **Tenant Migrations** (`TenantMigrationCard`) — checks pending database schema migrations for this BU's own tenant database (`GET`-style status check) and applies them via a streamed deploy action with a live applied/total progress readout and a running log of migration names, behind a confirm dialog. The card itself always renders; its **Check status** and **Apply** buttons are disabled with a tooltip when not super-admin or when the BU has no `db_connection` configured yet — since even the status-check button is included, a non-super-admin never loads pending-migration data in the first place.
- **Tenant Seed** (`TenantSeedCard`) — the same status-check/confirm/streamed-progress pattern, but for seed scripts instead of migrations; supports selecting a subset of seed keys before running. Disabled the same way as Tenant Migrations — not super-admin, or no `db_connection` yet — covering both its status-check and Run buttons.
- **Interface Entitlement** (`InterfaceEntitlementCard`) — controls which named interface/brand this BU is licensed to show, loaded and saved by `buCode` independently of the rest of the document (its own Save button, not part of the page's sticky bar). An empty selection means "unrestricted" — every interface shows, matching the gateway's own show-all default. The card itself always renders, but when not super-admin *or* until the BU has been saved at least once (needs a `buCode`), the entitlement fetch is skipped entirely and the card body shows a plain "Super-admin required." / "Save the business unit first." message in place of the brand-selection UI — not just a disabled button.
...
- `../carmen-platform/src/components/{TenantMigrationCard,TenantSeedCard,InterfaceEntitlementCard}.tsx` — the three new advanced cards; visible to any editor, actions super-admin-gated (§4.13).
