...
The module's one genuinely distinctive shape is that **`subscription.read` alone is enough to open an edit page** — `/licenses/subscriptions/:id/edit`, `/licenses/seats/:id/edit`, `/licenses/bu-quota/:id/edit`, and `/licenses/interface/:id/edit` all require `subscription.read` at the route level, the same key their read-only list/detail siblings require, while the four sibling `/new` routes require `subscription.manage`. This is not a route-guard inconsistency: every field on every edit form is independently gated inside the component (§3), so a `subscription.read`-only session sees the record but cannot change it.
...
| `/subscriptions`, `/subscriptions/new`, `/subscriptions/:id/edit` | (none — bare redirects) | n/a | `App.tsx:263-265` |
...
| Save Changes / Create License (seat, BU-quota or interface purchase form) | `LicensePurchaseForm.tsx` (`canEdit`, `<Can permission="subscription.manage">` on both buttons) |
| "Add interface license" / "New subscription" links on the BU edit page's Licenses tab | `BusinessUnitEdit.tsx:793,810` — `createHref` is supplied only when `canCreateSubscription` (= `hasPermission('subscription.manage')`), the same key the destination routes check, so the link never leads to a `<Forbidden>` |
...
| `GET /api-system/business-units/:buId/interface-licenses` | `AppIdGuard('businessUnitInterfaceLicense.findAll')` only | `platform_business-unit-interface-licenses.controller.ts:100-101` |
...
| `PATCH /api-system/business-units/:buId/interface-licenses/:id` | same, `subscription.manage` | `platform_business-unit-interface-licenses.controller.ts:189-191` |
| `DELETE /api-system/business-units/:buId/interface-licenses/:id` | same, `subscription.manage` | `platform_business-unit-interface-licenses.controller.ts:236-238` |
| `GET /api-system/platform/interface-licenses[/:id]` | `AppIdGuard` only | `platform_business-unit-interface-licenses.controller.ts:315,354` (fleet controller, same file) |
...
All backend paths above are `../carmen-turborepo-backend-v2/apps/backend-gateway/src/platform/{platform_cluster-licenses,platform_business-unit-licenses,platform_business-unit-interface-licenses,platform_subscriptions}/*.controller.ts`. The interface-licence controller's header comment repeats the same reasoning the other two give for leaving reads ungated: `PlatformPermissionGuard` builds its permission set only from `tb_user_tb_platform_role`, so an RBAC key on the read path would 403 a membership-only cluster admin looking at their own cluster.
...
| 3 | Seat or interface licence — attempting to cancel | No Cancel button exists anywhere for a seat or interface licence (list row actions or the dedicated edit page) — `config.cancel` is `null` for `SEAT_CONFIG` and `INTERFACE_CONFIG`, and no such endpoint exists on the backend for either table | Do not treat the missing button as a permission problem to escalate — it is a capability that does not exist for these purchase kinds, confirmed at both the schema (no `cancelled_at` column) and the config level |
| 3a | `subscription.manage` session attaches an `interface`-kind group to a subscription (`PUT .../subscriptions/:id/groups`), or a `standard` group to an interface licence (`POST .../interface-licenses`) | **400 from the backend**, not a silent filter and not a 403 — the permission is fine, the payload is not. The SPA never offers either combination (the subscription form's `GroupSelectionCard` hides `interface` groups; the purchase form's picker lists only `interface` groups), so this is reachable only by direct API call | A 400 here is the group-kind rule (`enum_license_feature_group_kind`, [Data Model](/en/platform/licenses/data-model) §5), not a gating defect; do not file it against RBAC |
