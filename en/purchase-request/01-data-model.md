...
| 5 | Line status | TypeScript `PurchaseRequestItemStatus = "Pending" \| "Accepted" \| "Rejected" \| "Review"` | `tb_purchase_request_detail` has no scalar status field; per-line state is stored in the `stages_status` JSON column and `current_stage_status` string. | Either materialise a typed `stage_status` enum on the detail row, or document the JSON shape as canonical. |
