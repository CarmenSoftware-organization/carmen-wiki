...
| `completed` | `COMPLETED` | ✅ match | All lines counted; Count Lead submitted; variance recorded. Rollup adjustment created per `PHC_POST_001`. Does **not** satisfy End Period Close Stage 3 alone — `FINALIZED` (GL posted) is required per `BR-08`. |
