...
- Related: [product](/en/inventory/product) — Sysadmin owns the `is_used_in_recipe` flag on products that gates whether a product is eligible as a recipe ingredient.
- Related: [costing](/en/inventory/costing) — the cost-drift integration chain Sysadmin maintains.
- Related: [inventory](/en/inventory/inventory) — the theoretical-consumption integration chain Sysadmin maintains.
- Related: [store-requisition](/en/inventory/store-requisition) — the recipe → SR auto-create wiring Sysadmin owns.
