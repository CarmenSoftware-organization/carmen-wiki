...
- **SR submitted to central store** — handoff to **Approver** and **Fulfiller** in the [store-requisition](/en/inventory/store-requisition) flow (the SR's lifecycle continues independently); the Outlet Manager monitors but does not advance the SR's status.
...
- **Variance attributable to shrinkage** — handoff to **Audit / Config (Inventory Controller)** for investigation through `[inventory-adjustment](/en/inventory/inventory-adjustment)`; the Outlet Manager provides operational context but the resolution is at the inventory layer.
...
- Related: [store-requisition](/en/inventory/store-requisition) — the primary downstream document the Outlet Manager authors; recipe demand seeds the SR.
- Related: [inventory](/en/inventory/inventory) — outlet on-hand reconciliation against recipe-driven theoretical consumption is the food-cost-variance computation.
- Related: [inventory-adjustment](/en/inventory/inventory-adjustment) — corrective movements for shrinkage / spoilage flagged by variance investigation.
