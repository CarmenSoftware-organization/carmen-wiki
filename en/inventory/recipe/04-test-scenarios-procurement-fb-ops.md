...
- E2E spec: **none for recipe internals**; adjacent automation in `[purchase-order](/en/inventory/purchase-order)` covers the PO-side of the PF-HP-02 flow.
- Cross-link: [purchase-order](/en/inventory/purchase-order) — Procurement's primary downstream document; recipe-driven demand sizes the PO.
- Cross-link: [vendor-pricelist](/en/inventory/vendor-pricelist) — vendor cost data is the upstream input to product cost, then to recipe cost.
- Cross-link: [costing](/en/inventory/costing) — F&B Ops reviews cost data sourced from the costing module.
