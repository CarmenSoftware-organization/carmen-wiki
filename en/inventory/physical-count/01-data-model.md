...
- **`tb_physical_count_period`** — the period-level header grouping all count documents for one fiscal period (`period_id → tb_inventory_period`, `@@unique([period_id, deleted_at])`). Carries `status` on `enum_physical_count_period_status` (`draft`, `counting`, `completed`), defaulting to `draft`. **The `draft → counting` transition is owned by Period End (2026-08-27, `407ee4214`):** `PeriodEndService.startCounting` (`POST /period-ends/start-counting`) creates the row at `counting` — or moves an existing `draft` row to `counting` — after checking that no GRN / stock-in / stock-out / numbered SR dated in the period is still open; `closeCurrent` later marks it `completed`. `physical-count-period.service.ts`'s `findCurrent()` (used by the list screen) still auto-creates a missing row at `draft`, and `physical-count.service.ts`'s `create()` still rejects with `"Physical Count Period is not in counting status"` until the start-counting call has run — the frontend intercepts that state with the "Counting has not started" dialog.
...
tb_inventory_period
    ...
    └─1──*──► tb_physical_count_period  (status: draft → counting → completed;
                │                          draft → counting via POST /period-ends/start-counting — see § 2)
                │
...
- **`enum_physical_count_period_status`** — period-level lifecycle. Three values: `draft` (auto-provisioned by `findCurrent()`), `counting` (set only by Period End's `startCounting`; the state in which counts may be created), `completed` (set by `closeCurrent`). See § 2.
