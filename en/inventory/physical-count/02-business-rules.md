...
| ------- | --------- | ------------- | ----------------- |
| `PHC_VAL_001` | `tb_physical_count_period.status` must already be `counting` for the target period. | Create count document (`POST /physical-counts`) | Reject with `"Physical Count Period is not in counting status"`. The transition to `counting` is made by **Start Period Close** (`POST /period-ends/start-counting`, `PeriodEndService.startCounting`, 2026-08-27), which itself refuses while any GRN not `committed`/`voided`, any stock-in / stock-out not `completed`/`cancelled`/`voided`, or any numbered SR at `draft`/`in_progress` is dated in the period (`PERIOD_END_START_COUNTING_BLOCKED`, 422). The frontend (`pc-component.tsx`) never sends the create while `period.status !== "counting"` — it shows the "Counting has not started" dialog with **Go to Period End** instead. |
...
| `PHC_CALC_002` (on-hand recomputation) | `reviewItems()` computes `on_hand_qty` for **every** line as `Σ tb_inventory_transaction_detail.qty` at the count's `location_id`, grouped by `product_id`, with **no date cut-off** — it is the current ledger balance at the moment "Submit for Review" is clicked, not a value frozen when the sheet was created. `reviewItems()` also stamps `counted_at` / `counted_by_id` on lines being counted for the first time (fix `4eb1abfa5`, 2026-08-27) — before that fix a sheet counted only through the review step was rejected by `submit()` as "not counted". |
...
| `PHC_AUTH_002` | No zone-based, location-scoped-to-user, or "assigned counter" restriction was found in `physical-count.service.ts` — any user holding the module permission can open, enter, and submit any count document. A generic `tb_location_user` table exists in the schema for location-level access grants elsewhere in the product, but no reference to it was found anywhere in this module's own service code; treat a "counter is scoped to their assigned location" claim as unconfirmed for this module specifically. |
...
| Rule ID | Rule |
| ------- | ---- |
| `PHC_POST_001` | On `submit()`, the service partitions the detail rows with `diff_qty ≠ 0` by sign: all positive-variance lines go into **one** new `tb_stock_in`; all negative-variance lines go into **one** new `tb_stock_out`. Lines with `diff_qty = 0` produce no rollup row. Both created headers are inserted with `doc_status: enum_doc_status.completed` directly — there is no draft or approval stage, unlike the manual Stock In/Out screens, which since 2026-07-30 create a `draft` and post on **Commit** — the pattern previously attributed to the inventory-adjustment module's own Stock In/Out screens. |
...
- **E2E:** `../carmen-inventory-frontend-e2e/tests/` — no physical-count spec currently exists; manual catalog `docs/test-cases/750-physical-count.md` (41 cases, re-verified 2026-09-20); the period-end gap catalog `docs/test-cases/gaps/900-period-end-gap.md` (TC-PE-31xxxx) covers the Start-button unlock.
