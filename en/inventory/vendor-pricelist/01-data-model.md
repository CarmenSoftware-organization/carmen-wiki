...
| `submitted_at` | `DateTime @db.Timestamptz(6)` | Yes | Column exists to record a vendor Submit action; no code currently writes it — the external portal's Submit call targets a backend route that does not exist (see § 5 item 6 and [03-user-flow-vendor](/en/inventory/vendor-pricelist/03-user-flow-vendor)). |
...
| 6 | Portal access telemetry | Per-invitation `portalAccess { firstAccessAt, lastAccessAt, sessionCount, ipAddresses[] }`; per-invitation `emailDetails { sentAt, deliveredAt, openedAt, clickedAt }` (design.md). | No dedicated columns, and **no code writes this data anywhere** — a repo-wide check of `price-list.service.ts`, `request-for-pricing.service.ts`, and `check-price-list.service.ts` confirms none of them ever create a `*_comment` row. `tb_request_for_pricing_detail_comment` is real and CRUD-able via its own dedicated controller, but only as user-authored comments — there is no automatic `system`-typed telemetry entry. | Design-target only. If implemented, telemetry would land in `tb_request_for_pricing_detail_comment` as `system` entries; no such write path exists today. |
...
| 11 | Email / portal token security policy | "Cryptographically secure tokens with configurable expiration times and access restrictions"; "IP address tracking"; "session timeout controls, concurrent session limits, suspicious activity detection" (requirements.md § Requirement 28). | `tb_request_for_pricing_detail.pricelist_url_token` (one column); `tb_pricelist.url_token` (denormalised copy). | **Corrected 2026-07-16 (was wrongly stated as enforced by portal middleware):** no expiration check, IP restriction, or concurrent-session limit exists anywhere in `check-price-list.service.ts` — the token is checked for existence only. Design-target, not implemented. |
