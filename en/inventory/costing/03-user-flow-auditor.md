...
An earlier draft described a strictly-read-only Auditor persona running three dedicated tools: a lot-level cost-flow chain-of-custody trace (forward and backward through `tb_inventory_transaction_cost_layer`), a period-end snapshot verification tool (independently reconstructing `tb_inventory_period_snapshot` — renamed from `tb_period_snapshot` on 2026-09-16 — from the cost-layer ledger), and a FIFO-vs-Average shadow-drift audit. Verification against current source found none of it (re-checked 2026-09-22):
...
- **The "GL absorbs the difference" / "GL Inventory control-account" claims this page's audit checklist relied on do not exist** — see [02-business-rules](./02-business-rules.md) § 4/§ 6 corrections; a GL module now exists (2026-09) but inventory never posts into it, so there is still no inventory control account to audit against.
- **The "shadow average under FIFO" the drift audit relied on does not exist** — FIFO receipts write `average_cost_per_unit = 0` (`inventory-transaction.service.ts:998`).
...
|---|---|---|
| Auditor runs a lot chain-of-custody trace | The Transaction Log (`/inventory-management/transaction`) is the closest real screen — a filterable, read-only ledger view open to anyone with `inventory_management.view`; per document, the GRN's Stock Movement tab (`GET …/good-received-notes/:id/stock-movements`) shows the lots each receipt line became, via `tb_inventory_transaction_detail.good_received_note_detail_item_id` | [inventory/transaction](/en/inventory/inventory/transaction) |
| Auditor verifies `tb_inventory_period_snapshot` against the cost-layer ledger | No in-app reconciliation tool exists; this would be a manual query against the two tables described in [costing/01-data-model](/en/inventory/costing/01-data-model) § 2.1/2.3 | [costing/01-data-model](/en/inventory/costing/01-data-model) |
