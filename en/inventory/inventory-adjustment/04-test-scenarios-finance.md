...
| FN-PERM-08 | Finance attempts to back-post into a `closed` period | **Deny per `ADJ_VAL_011` / [inventory](/en/inventory/inventory) `INV_VAL_008`.** Re-open is Finance Manager scope per [inventory](/en/inventory/inventory) `INV_AUTH_006`. Finance requests re-open; Finance Manager grants if exceptional. |
...
| FN-VAL-02 | Approve fails on closed period (`ADJ_VAL_011`) | Period closes between Controller forward and Finance approval. | Rejection per [inventory](/en/inventory/inventory) `INV_VAL_008`; Finance can request Finance Manager re-open or coordinate current-period restatement. |
...
- Cross-link: [inventory](/en/inventory/inventory) — `INV_AUTH_005` (Finance approval scope), `INV_AUTH_006` (Finance Manager period transitions), `INV_XMOD_008` (inventory-to-GL reconciliation).
- Cross-link: [good-receive-note](/en/inventory/good-receive-note) — Finance may chain credit-note flow for vendor-recall recovery against the originating GRN.
- Cross-link: [vendor-pricelist](/en/inventory/vendor-pricelist) — cost-anomaly historical reference.
- Cross-link: [costing](/en/inventory/costing) — FIFO / WA cost picks Finance verifies on outbound adjustments.
