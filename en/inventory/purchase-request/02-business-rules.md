...
| `PR_VAL_003` | `department_id` must be set and the requestor must belong to that department. *(An earlier revision added "or be delegated authority for it" — unconfirmed, no delegation mechanism found; see `PR_AUTH_006`.)* | On create / on submit | Reject with `"Department is required and must match requestor membership"`. |
...
- **Budget** — On submit, the PR's `base_total_amount` plus any per-line allocation against a budget category creates a **soft commitment** in the budget module (`BudgetData.softCommitmentPR`). The commitment decrements `availableBudget` for the relevant period. Soft commitments are released on `voided`, and are converted to a hard commitment on `tb_purchase_order` creation (see `PR_POST_007` and [purchase-order](/en/inventory/purchase-order)).
