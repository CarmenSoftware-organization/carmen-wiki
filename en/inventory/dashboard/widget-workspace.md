...
> **Naming note (corrected 2026-07-16):** The real backend tables are `tb_dashboard_personal_widget` (this page's data — tenant schema, `packages/prisma-shared-schema-tenant/prisma/schema.prisma` line ~6205: `dataset_id`, `widget_type`, `order_index`, `params`, scoped by `user_id`) and its sibling `tb_dashboard_bu_widget` (line ~6185, BU-scoped, admin-curated widgets shown to every member of a business unit — a **different, out-of-scope** feature that feeds the per-module landing dashboards under Procurement/Inventory-management/etc., not this page). A prior version of this note cited four other tables as real — **all four are gone or never existed**: `tb_widget_dashboard`, `tb_widget_dashboard_item`, and `tb_widget_default_layout` never appear in any Prisma schema (tenant, platform, or file) or in micro-data/micro-report; `tb_widget_workspace` *did* briefly exist — created by migration `20260512180928_widget_system_replace_dashboard` (tenant schema) — but was dropped, along with `tb_widget_dashboard`, `tb_widget_dashboard_item`, `tb_widget_default_layout`, and `tb_widget_comment`, by migration `20260521040013_remove_widget_system` (`DROP TABLE IF EXISTS "tb_widget_workspace" CASCADE`, forward-only/idempotent, "Drop widget subsystem"). It has zero references in any current `schema.prisma` or in any `.ts`/`.go` source, and no data-explorer/saved-query feature exists anywhere in the codebase to have used it. There is no cross-link for it — [reporting-audit/widget](/en/inventory/reporting-audit/widget) is itself flagged as documenting this same fabricated table family and needs its own correction pass.
