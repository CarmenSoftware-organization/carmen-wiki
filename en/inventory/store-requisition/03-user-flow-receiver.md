...
> **At a Glance**
> **Persona:** Destination outlet representative / dock stock controller &nbsp;·&nbsp; **Module:** [store-requisition](/en/inventory/store-requisition) &nbsp;·&nbsp; **Workflow stages:** completed (already terminal — physical verification, no status change) &nbsp;·&nbsp; **Key permissions:** acknowledge receipt (comment), raise discrepancy comment escalation
...
> ℹ️ **Discrepancy handling:** A discrepancy comment does NOT move `doc_status` — the SR stays `completed`. Resolution is via `[inventory-adjustment](/en/inventory/inventory-adjustment)` co-authored by the Inventory Controller (`SR_XMOD_009`). For `sr_type = transfer` SRs paired with a destination GRN, the Receiver's flow may extend into the GRN module.
...
- **Discrepancy flagged, awaiting controller** — handoff to the **Inventory Controller** (Audit / Config persona) for investigation and corrective action. The SR stays `completed`; the controller works through the discrepancy and, where needed, posts an adjustment via `[inventory-adjustment](/en/inventory/inventory-adjustment)`.
...
- Related: [good-receive-note](/en/inventory/good-receive-note) — for tenants that pair an SR (source side) with a GRN (destination side) on inter-warehouse transfers, the Receiver's flow extends into [good-receive-note/03-user-flow-receiver](/en/inventory/good-receive-note/03-user-flow-receiver).
- Related: [inventory-adjustment](/en/inventory/inventory-adjustment) — the resolution path for material discrepancies; the adjustment document carries a back-reference to the SR for audit.
- Related: [inventory](/en/inventory/inventory) — destination on-hand visibility (for `sr_type = transfer`) was incremented at source-side commit; the Receiver's verification confirms the system on-hand matches physical.
