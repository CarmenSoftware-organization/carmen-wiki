...
> **Audit pattern:** standard `created_*` / `updated_*` / `deleted_*`; three-qty per line (`requested` / `approved` / `issued`); per-line approval / review / reject signature columns; header `issue_at` / `issue_by_id` stamped at the issue stage; **no monetary roll-ups on header** (SR is a qty document)
> **Re-verified 2026-09-22** against `schema.prisma` at backend HEAD `cd2e07f60`: no SR column was added or removed since 2026-07-29, but `sr_type` is now server-derived, `sr_date` / `sr_no` are finalised at submit, and the detail-endpoint response has moved to nested entity objects (`workflow`, `requestor`, `department`, `from_location`, `to_location`, `issue_by`, `product` — `@ExpandRefs`, 2026-09-17); the list endpoint still returns the flat `*_name` strings (`types/store-requisition.ts`).
...
| `sr_no` | `String @db.VarChar` | No | Human-readable SR reference number. `NOT NULL`, but a draft carries the placeholder `draft-<6 hex>` written by `StoreRequisitionService.create()`; the running-code number (type `STORE-REQUISITION`, date pattern taken from the frozen `sr_date`) is minted by `generateSRNo()` during `submit()` and never re-minted on a resubmit after send-back. |
| `sr_date` | `DateTime @db.Timestamptz(6)` | Yes | The requisition date. On a draft it is whatever the form sent (defaults to `now()`), and is only a placeholder; `submit()` overwrites it through `resolveSubmitSrDate()` (`logic/sr-date.helper.ts`, 2026-09-18): today if today is inside an open inventory period, otherwise the client's `sr_date_pattern` pick (`open-period` → last day of the current open period, `today` → today). The document number is derived from this frozen value. |
...
| `sr_type` | `enum_sr_type` | No | Movement type; default `transfer`. **Derived, not client-supplied:** `create()` calls `resolveAndValidateSrType()` → `deriveSrType()` (`logic/sr-type.helper.ts`): destination `direct` → `issue`; destination `inventory` or `consignment` → `transfer`; a `direct` source is rejected (`from_location must be inventory or consignment, not direct`, 400). The swagger response documents the same rule. |
...
| `workflow_history` | `Json @db.JsonB` | Yes | Append-only stage-transition timeline; default `{}`. Per the schema doc-comment each entry is `{ action, at, user: {id, name}, current_stage, next_stage }`, where `action ∈ submitted | approved | reviewed | rejected | completed` (`completed` is pushed by `WorkflowOrchestratorService` on the final approval and is not an `enum_last_action` member). |
...
| `requestor_id` | `String @db.Uuid` | Yes | User id of the requester (outlet manager who raised the SR). No Prisma `@relation` declared. Nullable in Prisma, but `ValidateSRBeforeSubmitSchema` requires it at submit. |
| `requestor_name` | `String @db.VarChar` | Yes | Snapshot of the requester's display name. |
| `department_id` | `String @db.Uuid` | Yes | Department id of the requesting outlet. No Prisma `@relation` declared. Nullable in Prisma, but required by the create DTO since 2026-09-17 (`1dbaf76b1`); if a legacy draft still lacks it, `submit()` derives it from the requester's `tb_department_user` membership (`logic/sr-department.helper.ts`) — one membership is stamped, zero → `SR_DEPARTMENT_NOT_FOUND`, more than one → `SR_DEPARTMENT_AMBIGUOUS` (422, lists the names). |
...
| `issue_by_id` | `String @db.Uuid` | Yes | User who issued the stock (migration `20260619120000_sr_issue_fields`). Written by `approve()` when the acting stage is `enum_stage_role.issue` and by `updateByIssue()` on an issue-stage save. No Prisma `@relation`; the detail endpoint expands it as the `issue_by` object. |
| `issue_at` | `DateTime @db.Timestamptz(6)` | Yes | The resolved issue date (same migration). Set together with `issue_by_id`; equals the value returned by `resolveIssueDate()` — today when today is in an open period, else the last day of the requisition's own period (`issue_date_pattern = open-period`). This date is also passed to the inventory transaction as `doc_date`, so the stock movement is placed in that date's period (`resolveDocumentPeriod`). |
...
| `requested_qty` | `Decimal @db.Decimal(20, 5)` | Yes | Quantity the outlet asked for; default `0`. The create DTO accepts any `z.number()`; `ValidateSRBeforeSubmitSchema` (`store-requisition.dto.ts`) requires `> 0` per line at submit. Decimal input is accepted by the form (2026-07-29). |
| `approved_qty` | `Decimal @db.Decimal(20, 5)` | Yes | Quantity the approver authorised; default `0`. `submit()` initialises it to `requested_qty` on every line; an approve-stage save or `approve()` then overwrites it with the payload value. **The cap `approved_qty ≤ requested_qty` is documented intent only** — the detail DTO is a bare `z.number()` and neither `saveStageRoleDetails()` nor `approve()` compares the two (see [02-business-rules](/en/inventory/store-requisition/02-business-rules) `SR_VAL_008`). |
| `issued_qty` | `Decimal @db.Decimal(20, 5)` | Yes | Quantity the store keeper actually released at fulfilment; default `0`. Written by an issue-stage save (`updateByIssue()`) or by the final `approve()` call. The cap `issued_qty ≤ approved_qty` is likewise not enforced; the only hard stop is the cost-layer consumption throwing `Insufficient stock` when on-hand at the source is short. |
...
- **`enum_location_type`** (`tb_location.location_type`, gates `sr_type`): `inventory`, `direct`, `consignment`. `consignment` behaves like `inventory` for SR purposes (a valid source; as a destination it yields `transfer`).
- **`enum_sr_type`**: SR movement type for `tb_store_requisition.sr_type`. Default `transfer`. Derived by `deriveSrType()` at create — never accepted from the client. Two values:
  - `issue` — stock leaves inventory and is immediately consumed at the destination's cost-centre (kitchen pull, bar pull). Produced when `to_location.location_type = 'direct'`. Single stock-OUT at source; value routed to the destination's consumption expense account on its cost-centre.
  - `transfer` — stock physically moves between two inventory-holding locations without yet being consumed. Produced when `to_location.location_type` is `inventory` or `consignment`. Paired stock-OUT at source and stock-IN at destination; value moves from one inventory account to another, no expense recognised yet.
...
| 7 | `JournalEntry` as an SR-owned table | Tech Spec defines a `JournalEntry` interface with header / line accounting fields. | There is no SR-owned journal-entry table, and — unlike item 6's confirmed `tb_inventory_transaction` destination for stock movements — **no downstream GL/journal-posting code was found at all** in this pass (`journal`, `ledger` searched repo-wide against the SR module and its `inventory-transaction` dependency, zero hits). | Do not present GL posting as a confirmed SR-triggered effect; treat it as design intent only until a finance-module implementation is found. |
...
| 11 | `cancelled` as a reachable status (added this pass, not a carmen/docs divergence) | Prior versions of this page (and the business-rules / user-flow pages) described a distinct requester-withdrawal path and an all-lines-rejected auto-cancel path both landing on `cancelled`, with `voided` reserved for a separate Inventory-Controller/Sysadmin "admin void". | `StoreRequisitionService.reject()` (`store-requisition.service.ts`) sets `doc_status = enum_doc_status.voided` unconditionally on every whole-document reject call; no method anywhere in the service assigns `cancelled`. The `voided` action is performed by whoever holds the current workflow stage — there is no distinct admin-only "void" endpoint. | Fixed in this pass — see the enum bullets above and [02-business-rules.md](/en/inventory/store-requisition/02-business-rules) §5. |
| 12 | Fulfiller/store-keeper lot selection (added this pass, not a carmen/docs divergence) | Prior versions of this page and the persona pages described the store keeper opening a "lot sub-form" to pick specific lots at issue time. | `inventory-transaction.service.ts`'s `createFifoConsumption()` selects lots automatically (`getAvailableFifoLots` / `consumeFifoLots`); no lot-selection UI exists in the SR frontend (`grep` for `lot_no` / `lotNo` / `LotSelect` under `routes/store-operation/store-requisition/` returns no hits). | Fixed in this pass — lot assignment is system-computed FIFO, not a manual Fulfiller action. |
| 13 | Three-quantity invariant (added 2026-09-22, not a carmen/docs divergence) | Prior versions of this page and `02-business-rules` stated `0 ≤ issued_qty ≤ approved_qty ≤ requested_qty` is "enforced at the value level on every save and transition". | `store-requisition-detail.dto.ts` types `approved_qty` / `issued_qty` as plain `z.number()`; `saveStageRoleDetails()` writes `detail[qtyField]` verbatim and `approve()` spreads the payload into the update; the frontend `sr-form-schema.ts` bounds only `requested_qty ≥ 0` (the e2e gap report `701-sr-gap.md` fact 6 records the same). Only `requested_qty > 0` at submit is real. | Corrected in place across this module — treat the ordering as a UI convention, not a control. |
| 14 | `sr_type` as client input (added 2026-09-22) | Earlier pages described the requester "choosing" `issue` vs `transfer` and a validation rule rejecting an incompatible destination. | `sr_type` is derived from the two `location_type`s by `deriveSrType()`; the only rejection is a `direct` source. The frontend mirrors this (`derivedSrType` in `sr-form.tsx`) and shows the type as a read-only label. | Corrected — there is no movement-type picker and no compatibility error to test. |
| 15 | Open-period gate on SR dates (added 2026-09-22) | The 2026-07-15 pass found no `period` reference in the SR module and marked any closed-period block unconfirmed. | `logic/sr-date.helper.ts` (2026-09-18) resolves `sr_date` at submit and `issue_at` at the issue stage against `tb_inventory_period` (`findOpenPeriodForDate` / `findCurrentOpenPeriod`), with error-catalog codes `SR_DATE_PATTERN_REQUIRED`, `SR_NO_OPEN_PERIOD`, `SR_DATE_OUTSIDE_OPEN_PERIOD`, `SR_ISSUE_DATE_PATTERN_REQUIRED`, `SR_ISSUE_DATE_TODAY_OUTSIDE_PERIOD` (all 422). | Corrected — `SR_VAL_014` in `02-business-rules` is now a confirmed rule. |

## 6. References

- **Backend logic cited above:** `../carmen-turborepo-backend-v2/apps/micro-business/src/inventory/store-requisition/store-requisition.service.ts` (`create`, `submit`, `approve`, `saveStageRoleDetails`, `generateSRNo`), `logic/sr-type.helper.ts`, `logic/sr-date.helper.ts`, `logic/sr-department.helper.ts`, `dto/store-requisition.dto.ts` (`StoreRequisitionCreateSchema`, `ValidateSRBeforeSubmitSchema`), `dto/store-requisition-detail.dto.ts`; error codes in `packages/error-catalog/src/catalog.ts`; migration `20260619120000_sr_issue_fields`.
