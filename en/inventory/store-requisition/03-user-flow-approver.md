...
> **Re-synced 2026-09-22:** all verbs are `PATCH`; `approved_qty` arrives pre-filled with `requested_qty`; the cap `approved_qty ≤ requested_qty` is **not enforced** anywhere; the footer Approve/Reject dialogs have no reason field (only Send Back and the bulk Reject do); Send Back requires a destination stage.
...
| Reject line, bundled with other lines' approve decisions in one `PATCH .../approve` call | ✅ (`SR_AUTH_005`, `SR_VAL_010`) |
| Send the whole document back for correction (`/review`; cannot mix with approve/reject in the same call) | ✅ (`SR_AUTH_005`) |
| Mix approve / reject per line in one `PATCH .../approve` call | ✅ — confirmed in `computeSrAction()` (`sr-form-schema.ts`) |
| Approve own SR (where Approver = Requester) | Unconfirmed whether blocked — no SoD check found in code |
| Raise `approved_qty` above `requested_qty` | ⚠️ **Not blocked** — no DTO/service/form cap exists (`SR_VAL_010` corrected 2026-09-22) |
| Final-stage advance (recording `issued_qty`) | Same `PATCH .../approve` endpoint — gated by which stage's `enum_stage_role` the user holds (`approve` vs `issue`), not a separate commit permission |
...
3. **Per-line decision.** `approved_qty` arrives pre-filled with `requested_qty` (initialised at submit). For each line the Approver chooses one of:
   - **Approve in full**: leave `approved_qty = requested_qty`. Per-line: `approved_by_id`, `approved_by_name`, `approved_date_at = now()`, optional `approved_message`.
   - **Trim down**: set `approved_qty ∈ (0, requested_qty)`. Per-line: same signature columns; `approved_message` explains the trim (e.g. "trimmed to source on-hand" — the only confirmed trim reason; "par-level cap" and "budget cap" wording is illustrative, not a system-generated label). Raising `approved_qty` above `requested_qty` is **not rejected** by any layer (`SR_VAL_010` not enforced).
   - **Reject the line**: mark it `reject`. The backend does **not** zero `approved_qty`; it records the stage status and history for the line. A reason can be typed only through the bulk **Reject** action above the item table (optional, `Enter reason (optional)…`) — the footer Reject confirm dialog is rendered with `showMessage={false}` even though its copy says "Please provide a reason." Bundled into the same `PATCH .../approve` call as other lines' decisions.
   - **Send back the whole document for correction**: a separate `PATCH .../review` call whose dialog requires a **destination stage** (`des_stage`, from `GET .../workflow-previous-step-list`; the confirm button stays disabled until one is chosen) and takes an optional message; if any line is marked "review," the whole submission becomes a send-back and approve/reject selections on other lines in that same submission are not applied (`computeSrAction()`).
4. **Mixed decisions across lines.** Approve and reject can mix in one `PATCH .../approve` call; review cannot mix with either in the same submission (see point 3). The screen does not show "running totals of approved/rejected value" in the components read this pass — treat that framing as unconfirmed.
5. **Confirm the action.** The footer shows exactly one button derived from the per-line statuses (`computeSrAction()`): **Approve** when every decided line is approve/reject with ≥ 1 approve, **Reject** when every line is reject (whole-document `PATCH .../reject` → `voided`), **Send Back** when any line is review, nothing when no line is decided. The system checks `SR_AUTH_014` (actor is in `user_action.execute`) and the `doc_version` echo; **no quantity cap is validated**.
6. **Workflow advance.** When all lines on the current stage have been actioned, the system advances `workflow_current_stage` to the next stage — the next `approve`/`issue`-tagged stage per `tb_workflow`, or `completed` if this was the final stage. A separate whole-document `PATCH .../reject` call (only enabled when every line is marked reject) sets `doc_status = voided` directly — **not** `cancelled` (see [01-data-model.md](./01-data-model.md) §5 item 11).
...
- **Reject the entire SR**: every line is marked reject in one submission, enabling the whole-document `PATCH .../reject` action. The system sets `doc_status = voided` directly — **corrected this pass**: prior versions of this page described this landing on `cancelled`; no `cancelled` assignment exists anywhere in `store-requisition.service.ts`.
...
- Sibling: [02-business-rules.md](./02-business-rules.md) — `SR_VAL_010` (approval cap — documented intent, not enforced), `SR_AUTH_005`–`SR_AUTH_006` (approve / trim / send-back authority, mixing rules), `SR_POST_005`–`SR_POST_010` (final-stage advance and whole-document reject → `voided`).
