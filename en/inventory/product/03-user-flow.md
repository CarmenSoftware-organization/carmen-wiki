...
> **At a Glance**
> **Module:** [product](/en/inventory/product) &nbsp;·&nbsp; **Personas:** Product Administrator (CRUD owner) &nbsp;·&nbsp; Purchaser (read-only) &nbsp;·&nbsp; Store Keeper (read-only)
...
| `active | inactive` | soft-delete | `soft-deleted` | Product Administrator (`PRD_AUTH_001`) | **No guard is implemented** (corrected 2026-09-22 — `products.service.ts` `delete()` only checks the row exists; the `PRD_LIFE_004` on-hand / document / recipe checks are design intent). Sets `deleted_at`, `deleted_by_id` on the product and its unit-conversion rows. The `(code, name)` becomes re-usable. |
...
- **There is no bulk import.** Earlier revisions described a dry-run / strict-commit import as the dominant create path; no import endpoint, screen, or Bruno request exists (verified 2026-09-22). Products are created one at a time through the form (or by direct API calls). The list can be **exported**.
...
| Store Keeper | Per-location stock policy needs adjustment (min / max / par / reorder feels wrong for a location) | Product Administrator | Store Keeper posts a comment on the product or on `tb_product_location` (via the product's Location Assignment tab). **Corrected this pass:** earlier drafts of this page routed the handoff to a separate "Inventory Controller" per `INV_AUTH_004` — no such edit surface exists; `tb_product_location`'s numeric fields are edited directly on the Product module's own edit form (the Location Assignment tab, `pd-location-tab.tsx`), the same surface the Product Administrator uses for every other field. The inventory module's own `INV_AUTH_004` is itself marked *(unconfirmed)* for this reason. |
...
- Sibling: [01-data-model.md](./01-data-model.md) — canonical `enum_product_status_type` (three values: `active | inactive | discontinued`) and the `enum_unit_type` (`order_unit | ingredient_unit`); the classification chain shape (three levels, not five). The `discontinued` state is now first-class — see Section 2 for transitions.
...
- Related modules: [inventory](/en/inventory/inventory) (consumer of `product_id` on every transaction; in-use guard for product delete), [costing](/en/inventory/costing) (`PRD_XMOD_003` — `standard_cost` source, FIFO/WA method is on business-unit not product), [good-receive-note](/en/inventory/good-receive-note) (consumer; deviation tolerances gate receiving variance), [store-requisition](/en/inventory/store-requisition) (consumer; product–location mapping drives par-level sizing), [purchase-request](/en/inventory/purchase-request) / [purchase-order](/en/inventory/purchase-order) (consumer; order-unit conversions), [vendor-pricelist](/en/inventory/vendor-pricelist) (consumer; product–vendor join), [recipe](/en/inventory/recipe) (consumer; product as ingredient — `is_used_in_recipe` filter on picker).
