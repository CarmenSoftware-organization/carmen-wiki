...
| Store Keeper | Per-location stock policy needs adjustment (min / max / par / reorder feels wrong for a location) | Product Administrator | Store Keeper posts a comment on the product or on `tb_product_location` (via the product's Locations tab). **Corrected this pass:** earlier drafts of this page routed the handoff to a separate "Inventory Controller" per `INV_AUTH_004` — no such edit surface exists; `tb_product_location`'s numeric fields are edited directly on the Product module's own edit form (the Locations tab, `pd-location-tab.tsx`), the same surface the Product Administrator uses for every other field. The inventory module's own `INV_AUTH_004` is itself marked *(unconfirmed)* for this reason. |
