...
| `product_status_type` | `enum_product_status_type` | No | Lifecycle status: `active` (appears on pickers, available for new transactions), `inactive` (temporarily frozen — past history retained, excluded from new documents, reversible), or `discontinued` (explicitly end-of-life — same exclusion as `inactive` but signals a permanent retirement decision for downstream reporting / re-order suppression). Default `active`. See Section 4 for transition rules. |
...
| `is_active` | `Boolean?` | Yes | Default `true`. Hard-disable flag (distinct from `product_status_type` — `is_active = false` removes the product from all pickers including admin views; `product_status_type = inactive` or `discontinued` removes it from new-transaction pickers but still admin-visible). |
...
The **product–vendor join** that lists which vendors supply a product, with the vendor's own product code and name for cross-reference. Used by [vendor-pricelist](/en/inventory/vendor-pricelist) and [purchase-request](/en/inventory/purchase-request) / [purchase-order](/en/inventory/purchase-order) to scope vendor pickers.
...
### 2.9 tb_product_image

The **product-image gallery** table (added 2026-05-20 — partially resolves the historic "no `tb_product_media`" divergence described in Section 5 item 3). Stores one row per uploaded image associated with a product; the file itself lives behind a `file_token` reference (S3 / object-store managed by the platform's file service, parallel to the `attachments` JSON pattern on comment tables).

| Field | Prisma Type | Nullable | Description |
| ----- | ----------- | -------- | ----------- |
| `id` | `String @db.Uuid` | No | Primary key; `gen_random_uuid()`. |
| `product_id` | `String @db.Uuid` | No | FK to `tb_product.id` with `onDelete: Cascade` — image rows are removed when the parent product is hard-deleted (distinct from the soft-delete pattern everywhere else in this module). |
| `file_token` | `String @db.VarChar` | No | Reference to the uploaded file in the platform's file service. Resolves to a CDN/S3 URL via the application layer. |
| `caption` | `String? @db.VarChar` | Yes | Optional human-readable caption shown beneath the image. |
| `alt_text` | `String? @db.VarChar` | Yes | Accessibility alt-text for screen readers and SEO. |
| `sort_order` | `Int` | No | Default `0`. Display order in the gallery; lower numbers appear first. |
| `is_primary` | `Boolean` | No | Default `false`. The designated primary / cover image (UI typically enforces at-most-one-true per product). |
| `created_at` / `created_by_id` | — | Yes | Standard audit timestamps. |
| `updated_at` / `updated_by_id` | — | Yes | Standard audit timestamps. |
| `deleted_at` / `deleted_by_id` | — | Yes | Soft-delete timestamps. Note: the parent-side cascade still applies on hard product delete, but day-to-day image removal is soft via `deleted_at`. |

**Constraints:** `@id` on `id`. FK `product_id → tb_product.id` with `onDelete: Cascade` (the only cascading FK in the product module — contrast with the `NoAction` pattern used elsewhere).
...
- **`enum_product_status_type`**: lifecycle status on `tb_product.product_status_type`. Default `active`. Three values:
  - `active` — the product appears on pickers and is available for new transactions.
  - `inactive` — the product is frozen as a **temporary / reversible** state. Past history (balances, posted documents, recipes) is retained, but the product is excluded from new-document pickers. The transition is auditable and can be scheduled to take effect on a future date by the application layer (no schema-level effective-date column). Use for seasonal pauses, recall holds, and similar revivable cases.
  - `discontinued` — the product is frozen as an **explicit end-of-life** state. Same picker exclusion as `inactive`, but the semantic signal is permanent retirement (no planned re-introduction). Drives downstream behaviours such as re-order suggestion suppression, vendor-pricelist hide, recipe-replacement prompts, and end-of-life inventory reporting. Reversible at the schema level but operationally discouraged.
...
- `enum_physical_count_costing_method` (on count documents): `standard`, `last`, `average`, `last_receiving`. Selects which cost source feeds count-variance posts; `standard` reads `tb_product.standard_cost`. See [costing/01-data-model](/en/inventory/costing/01-data-model) § 2.6.
...
| 3 | Product Media (images / documents / videos / 3D models) | `PROD-PRD.md` § 3.1.4 (FR-PRD-031..040): designated primary media, ordering, thumbnails, AI-powered auto-tagging, mobile-optimised delivery. `UI-PRD-013` references a media gallery on the product detail view. | **Partially resolved (2026-05-20).** `tb_product_image` is now a first-class table (see Section 2.9) with `file_token`, `caption`, `alt_text`, `sort_order`, and `is_primary` columns — covering the "designated primary image" / "ordering" / "media gallery" portions of the PRD. **Still not modelled:** non-image media (documents, videos, 3D models) which continue to ride `tb_product_comment.attachments`; thumbnail derivation; AI auto-tagging; mobile-optimised delivery (all of which are application-layer / CDN concerns rather than schema concerns). | The image case is closed by the schema. For non-image media, either add a separate `tb_product_media` table or extend `tb_product_image` with a `media_type` discriminator. Thumbnail / CDN / AI features are out of scope for this schema. |
| 4 | Product Lifecycle States | `PROD-PRD.md` § 3.1.5 (FR-PRD-041..050) and `product-master-prd.md`: lifecycle includes `Active`, `Inactive`, **`Discontinued`**, plus scheduled status changes, status history tracking, status-based visibility controls, lifecycle visualisation. | `enum_product_status_type` now has **three values**: `active`, `inactive`, `discontinued` (added in the May 2026 enum-cleanup pass). There is still no first-class status-history table (the implicit history is on the audit columns and on `tb_product_comment` system entries) and no schema-level scheduled-change column. | The `discontinued` enum value is now schema-aligned. Scheduled changes and status-history reporting remain application-layer concerns — implement via a `tb_product_comment` `type = system` entry on transition and an effective-date column in `info` JSON if needed. |
...
- Related modules: [inventory](/en/inventory/inventory) (every inventory balance and cost-layer references the product; product activation / deactivation gates new inventory transactions), [costing](/en/inventory/costing) (consumes `tb_product.standard_cost`; costing method lives at `tb_business_unit.calculation_method` not on the product), [vendor-pricelist](/en/inventory/vendor-pricelist) (pricelist lines reference products via `product_id`; vendor mapping comes through `tb_product_tb_vendor`), [purchase-request](/en/inventory/purchase-request) / [purchase-order](/en/inventory/purchase-order) / [good-receive-note](/en/inventory/good-receive-note) (every line references a product), [store-requisition](/en/inventory/store-requisition) (issue / transfer lines reference products), [recipe](/en/inventory/recipe) (recipe ingredient lines reference products as `product_id`, and sub-recipes reference other recipes — products with `is_used_in_recipe = true` appear on the picker), [physical-count](/en/inventory/physical-count) / [spot-check](/en/inventory/spot-check) / [inventory-adjustment](/en/inventory/inventory-adjustment) (count and adjustment documents reference products).
