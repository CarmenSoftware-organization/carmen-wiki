...
- **Per-location policy feedback routed to Product Administrator.** Min / max / par / reorder feedback is logged via comment on the product's Locations tab; the **Product Administrator** picks it up and edits the values on that same tab. The Store Keeper's involvement on the policy concern ends. (Corrected this pass — earlier drafts routed this to a separate "Inventory Controller"; no such edit surface exists.)
...
- Related: [inventory](/en/inventory/inventory) — `tb_product_location` is read here; the **edit** authority belongs to the Product Administrator on the product module's own Locations tab (corrected this pass — no separate inventory-module edit surface was found; [inventory/02-business-rules](/en/inventory/inventory/02-business-rules) `INV_AUTH_004` is itself marked unconfirmed). Per-location policy feedback routes to the Product Administrator.
