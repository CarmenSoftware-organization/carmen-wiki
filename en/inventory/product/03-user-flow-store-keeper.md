...
- Related: [inventory](/en/inventory/inventory) — `tb_product_location` is read here; the **edit** authority belongs to the Inventory Controller per [inventory/02-business-rules](/en/inventory/inventory/02-business-rules) `INV_AUTH_004`. Per-location policy feedback routes to that persona.
