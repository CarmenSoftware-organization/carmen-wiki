...
The **Store Keeper** persona is a **read-only consumer** of the product catalogue, operating at the **floor / location level** during receiving, picking, transfers, and counts. Within the product module their authority is **lookup only**: they scan barcodes (`tb_product.barcode`) for fast identification on mobile devices, view product detail (handling instructions from `tb_product.info` JSON, shelf life, storage requirements, perishable flags), reference per-location stock policy (`tb_product_location.min_qty / max_qty / par_qty / re_order_qty`) for context during counts and replenishment, and post comments (`tb_product_comment`) for barcode mismatches and operational issues. They do **not** create products, do **not** edit any master-data field, do **not** edit per-location stock policy — that authority sits with the Product Administrator on the product module's own Location Assignment tab (corrected this pass; see [03-user-flow-product-admin.md](./03-user-flow-product-admin.md) Section 1 — there is no separate Inventory-Controller edit surface) — do **not** edit unit conversions, and do **not** participate in product lifecycle transitions. Their transactional activity (receiving GRN, stock-in / stock-out, running physical / spot counts) lives in the [good-receive-note](/en/inventory/good-receive-note), [inventory](/en/inventory/inventory), [physical-count](/en/inventory/physical-count), and [spot-check](/en/inventory/spot-check) persona files — this page covers only the **product-catalogue lookup** surface they touch in support of that work.
...
1. **Open the receiving flow on mobile.** From within [good-receive-note](/en/inventory/good-receive-note)'s Receiver flow, the Store Keeper is at the GRN line entry screen. They tap the scan button.
...
The **stock-in / stock-out** flow (manual adjustments per [inventory/03-user-flow-store-keeper](/en/inventory/inventory/03-user-flow-store-keeper)):
...
- **Location-policy adjustment.** When the configured `min_qty` / `max_qty` / `par_qty` feels wrong for a location (constant stock-outs at the min, or constant overstock above max), the Store Keeper posts feedback on `tb_product_location` (via the product detail's **Location Assignment** tab, which since 2026-08 also shows the product's **shelf** at that location — [master-data/shelf](/en/inventory/master-data/shelf)). Note `re_order_qty` is no longer configured there; the API derives it from on-hand vs. `max_qty` / `par_qty`. The handoff is to the **Product Administrator** — corrected this pass; earlier drafts routed this to a separate "Inventory Controller" per `INV_AUTH_004`, but the only confirmed edit surface for these fields is the Product Administrator's own product-edit form.
...
- **Per-location policy feedback routed to Product Administrator.** Min / max / par / reorder feedback is logged via comment on the product's Location Assignment tab; the **Product Administrator** picks it up and edits the values on that same tab. The Store Keeper's involvement on the policy concern ends. (Corrected this pass — earlier drafts routed this to a separate "Inventory Controller"; no such edit surface exists.)
...
- Sibling: [02-business-rules.md](./02-business-rules.md) — validation rule `PRD_VAL_005` (barcode uniqueness — application-enforced), calculation rule `PRD_CALC_001` (classification path), authorization rules `PRD_AUTH_007` (Store Keeper read with barcode lookup) and `PRD_AUTH_008` (Store Keeper read of per-location policy — but not edit, corrected this pass to point to the Product Administrator, not a separate Inventory Controller); cross-module rule `PRD_XMOD_001` (inactive product rejected on new line) referenced in the decision branch.
- Related: [good-receive-note](/en/inventory/good-receive-note) — primary module for the dominant barcode-scan flow (step 1 entry); product master is consumed at line entry. The product's deviation tolerances (`PRD_CALC_003`) gate the GRN line's variance approval.
- Related: [inventory](/en/inventory/inventory) — `tb_product_location` is read here; the **edit** authority belongs to the Product Administrator on the product module's own Location Assignment tab (corrected this pass — no separate inventory-module edit surface was found; [inventory/02-business-rules](/en/inventory/inventory/02-business-rules) `INV_AUTH_004` is itself marked unconfirmed). Per-location policy feedback routes to the Product Administrator.
- Related: [physical-count](/en/inventory/physical-count) / [spot-check](/en/inventory/spot-check) — count execution uses barcode scan and per-location policy reference per Section 2 (count / spot-check flow).
- Related: [store-requisition](/en/inventory/store-requisition) — SR dispatch / picking uses barcode scan for bin-pick confirmation; product master is consumed at the pick step.
- Related: [inventory-adjustment](/en/inventory/inventory-adjustment) — stock-in / stock-out for manual adjustments; perishable flag from `tb_product.info` is referenced at line save per the per-product expiry rule.
