...
**A second finding, now resolved in the frontend: the create form no longer carries dead view/edit code.** The 2026-07-15 revision found that `ScForm` (`sc-form.tsx`) shipped unreachable `isView`/`isEdit` branches, Edit/Delete buttons and `useUpdateSpotCheck`/`useDeleteSpotCheck` hooks — it is only ever rendered from `spot-check-by-location-content.tsx` in "add" mode with `hideLocation`. On 2026-09-04 (`0647b32e`, "remove the ScForm edit path that was dead since day one") that code was deleted; `ScForm` is now a create-only form, and `spot-check/:id` still renders the entry/counting screen (`ScEntryComponent`). The backend `update()` / `delete()` pair (`PATCH /spot-checks/:id`, `spotCheck.update` / `spotCheck.delete` guards) still exists and works when called directly (Bruno `inventory/spot-check/PATCH-update-…`, `DELETE-remove-…`), but no button in the shipped UI reaches it.
...
- E2E tests: `../carmen-inventory-frontend-e2e/` — no Playwright spec exists for this module (`ls tests/ | grep -i 'spot\|check'`); manual test-case catalog `docs/test-cases/760-spot-check.md` (44 cases, re-verified against the live component 2026-09-20 — it now states explicitly that no view/edit/delete page exists) and generated stories `docs/user-stories/760-spot-check.md` (32)
