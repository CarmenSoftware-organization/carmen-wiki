...
| Approve at own stage (`in_progress`) | ✅ when assigned via `user_action.execute[]` for the current stage (`PO_AUTH_011`) — no amount threshold |
...
1. Receive the notification. The workflow stage cursor lands on this user's stage because the tenant's workflow definition assigned them there — not because of any amount or deviation calculation.
