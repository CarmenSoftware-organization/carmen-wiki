...
The PO document status is stored on `tb_purchase_order.po_status` and constrained to the values declared in `enum_purchase_order_doc_status`: `draft`, `in_progress`, `voided`, `approved`, `sent_or_print`, `partial`, `closed`, `completed` (`approved` added and `sent` renamed `sent_or_print` on 2026-09-14). The transitions below cover the legal moves between them; everything else is rejected by the service guards. Note that receipt-driven transitions (`{approved, sent_or_print} → partial → completed`) are triggered by GRN postings in the downstream [good-receive-note](/en/inventory/good-receive-note) module, not by direct user action on the PO — and a GRN may post against an `approved` PO before anyone has emailed it.
...
> ⚠️ **Corrected 2026-09-22:** the previous diagram merged approval and transmission into one `in_progress → sent` edge. Source HEAD splits them: final approval lands on `approved` (`purchase-order.logic.ts` `performApprove`), and `sent_or_print` is reached only when `POST .../purchase-orders/:id/send-email` succeeds or `POST .../purchase-orders/:id/mark-sent` is called (`markPoAsSent`, `updateMany where po_status = approved`). GRN receipt is unlocked at `approved` (`findOnePoForGrn`). Earlier corrections still stand: send-back (`/review`) keeps `po_status = in_progress` and only resets `workflow_current_stage`; the only path to `voided` is a direct, terminal `in_progress → voided` via `/reject`; ending a PO from `draft`, `approved`, `sent_or_print`, or `partial` goes through **Cancel** or **Close** (both land on `closed`, writing the remainder to `cancelled_qty`), not `voided`.
...
| `draft` | delete | `(none)` | Document owner (`created_by_id`) or platform super-admin | Soft-delete only (`deleted_at` set); allowed solely while the PO is in `draft` (`remove()`: "Only draft purchase orders can be deleted" + `isDocumentOwner` / `isSuperAdmin`). |
| `draft` / `in_progress` / `approved` / `sent_or_print` | cancel | `closed` | Whoever holds the action (no role gate beyond authentication) | Withdraws the commitment; every line's `cancelled_qty = order_qty − received_qty` (`cancel()`). Not available from `partial`. |
...
| `in_progress` | approve (final stage) | `approved` | Whoever holds the workflow's final stage | `isFinalApproval` from `buildApproveWorkflow`; `approval_date` set; `workflow_history` gains a `completed` entry. **Nothing is transmitted** — the PO is now receivable and emailable, but the vendor has not seen it. |
| `approved` | send email to vendor | `sent_or_print` | Whoever opens the **Send Email** dialog (`po-send-email-dialog.tsx`; no role gate) | `POST .../:id/send-email` `{ profile_id, to[], cc[], subject, body, attach_pdf }` through a BU email profile; optional PDF rendered by micro-report; `tb_activity` `email_sent` logged for every attempt; status moves only when the SMTP hand-off reports `sent: true`. Allowed again from `sent_or_print` / `partial` / `closed` / `completed` (re-send), where it just logs. |
| `approved` | mark sent (print / fax / phone) | `sent_or_print` | API caller (`POST .../:id/mark-sent`; no React button yet) | "Only approved orders can be marked as sent."; `tb_activity` "Marked as sent to vendor". |
...
| `{in_progress, approved, sent_or_print, partial}` | close | `closed` | Any authenticated user (the React header shows **Close** on `approved` / `sent_or_print` when `canClose`, `po-header.tsx` L128-131); no role gate in `closePO()` | Vendor cannot supply the outstanding quantity; remaining open qty (`> 0`) is written to `cancelled_qty`; buyer notified. |
| `approved` / `sent_or_print` | receive (partial) | `partial` | Receiver via GRN posting (location `can_use` required) | At least one PO line has `received_qty > 0` but `received_qty < order_qty − cancelled_qty` across the PO. State change is computed from line-level GRN postings (`updatePoStatuses`). |
| `approved` / `sent_or_print` | receive (full) | `completed` | Receiver via GRN posting | Every line satisfies `received_qty + cancelled_qty ≥ order_qty`; all lines closed via GRN in a single transaction. |
...
- [Purchaser](./03-user-flow-purchaser.md) — Creates POs manually, by converting approved PRs (grouped by vendor + delivery date + currency), or from a vendor price list; validates pricelist pricing; submits; once `approved`, emails the PO to the vendor (the step that reaches `sent_or_print`) and manages follow-up.
...
- [Vendor](./03-user-flow-vendor.md) — External party with no system login; receives the PO by email (with optional PDF) or an out-of-band print recorded via `mark-sent`, and fulfils delivery against agreed terms. No confirmed in-system acknowledgement or invoice feature.
...
| Approver (final stage) | Approve at final stage | Purchaser (to send) and Receiver (may already receive) | `approved` (workflow finished; nothing transmitted yet) |
| Purchaser (or anyone with the PO open) | Send Email to vendor / `mark-sent` | Vendor | `sent_or_print` (PO transmitted; awaiting first GRN) |
...
| Vendor | Physical delivery of goods | Receiver | `approved` or `sent_or_print` (system state unchanged until GRN is posted) |
