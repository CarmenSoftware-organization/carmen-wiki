...
> **At a Glance**
> **Persona:** Audit / Config (Auditor + System Administrator) &nbsp;·&nbsp; **Module:** [purchase-order](/en/inventory/purchase-order) &nbsp;·&nbsp; **Confirmed surface:** generic per-document activity/history view (Auditor, read-only); generic workflow-stage, amount/department/category routing-rule, and running-code (numbering) configuration shared across document types (System Administrator — the routing-rule tab is the same generic Routing tab used by PR/PO/SR, not PO-specific) &nbsp;·&nbsp; **Not confirmed:** a dedicated cross-document "audit workspace" query builder, or a PO-specific configuration workbench (numbering scheme editor, vendor-ranking editor, PR-to-PO grouping-rule editor, delegation-window manager)

> ⚠️ **Major correction this pass.** The previous version of this page described a dedicated **"Procurement Activity Queries"** audit workspace (query templates, filter chips, export-approval workflow, case-file notes) and a dedicated **"Configuration workspace"** with child surfaces for PO Numbering & Templates, PO Workflow Settings (including a high-value threshold and approval delegations), RBAC & Roles, Integration Settings, and PR-to-PO Rules. The audit workspace, the PO-specific configuration workbench, and the approval-delegation feature were not found in current source:
...
> - A repo-wide search for `segregation` and delegation-window code returned nothing relevant anywhere in `carmen-turborepo-backend-v2`.
> - Generic configuration surfaces **do** exist elsewhere in the product — workflow stage definitions, amount/department/category **routing rules**, and running-code (document numbering) are configured in the **system-config** module, shared across document types (PR, PO, GRN, SR, etc.), not as a PO-specific screen. This page's prior version conflated "PO uses a workflow and a numbering scheme" (true, and documented in [01-data-model.md](./01-data-model.md) / [02-business-rules.md](./02-business-rules.md)) with "PO has its own dedicated configuration workspace" (not confirmed).
>
> **The "high-value threshold" half of the original claim was itself wrongly dismissed by an earlier version of this correction** — a follow-up search confirms the generic **Routing** tab of `/system-admin/workflow` (`wf-routing.tsx`) lets a System Administrator attach `routing_rules` to a workflow that route on `total_amount` (also `department`/`category`), evaluated by `evaluateCondition`/`findNextStep` in `workflows.navagation.service.ts` on every submit/approve. It is real, generic (not PO-specific), and configured alongside — not instead of — the plain stage/numbering config described above. See [02-business-rules.md](./02-business-rules.md) `PO_AUTH_004`.
