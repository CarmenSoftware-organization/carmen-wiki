...
The GRN sits **downstream of [purchase-order](/en/inventory/purchase-order)** and **upstream of [inventory](/en/inventory/inventory)** in the procure-to-pay chain. Linkage to PO runs through two columns on `tb_good_received_note_detail` — `purchase_order_id` and `purchase_order_detail_id` — with `purchase_order_detail_id` carrying the explicit Prisma `@relation` back to `tb_purchase_order_detail`. Both columns are nullable so the same line table can also represent a manual GRN with no upstream PO (governed by the `doc_type` enum). **Corrected this pass — mutation timing:** the fan-out described below fires on the **`draft → saved`** transition, not on commit. `GoodReceivedNoteLogic.save()` (`good-received-note.logic.ts`) is what resolves every line to one or more rows in `tb_inventory_transaction` / `tb_inventory_transaction_detail` (via `tb_good_received_note_detail_item.inventory_transaction_id`, where on-hand increment, cost layer, and lot / expiry data live) and advances the PO line's `received_qty`; the subsequent `saved → committed` call only flips `doc_status` and locks the document — no further inventory or PO-side effect was found in `GoodReceivedNoteLogic.commit()`. **Unconfirmed:** no vendor-invoice / three-way-match (PO ↔ GRN ↔ invoice) or AP-posting code was found anywhere in the current backend or frontend — treat any such claim as design intent, not implemented behavior (mirrors the same finding already confirmed for the `purchase-order` module).
...
| `invoice_no` | `String @db.VarChar` | Yes | Vendor invoice number. Unique together with `vendor_id` across non-soft-deleted GRNs. |
