...
> **At a Glance**
> **Module purpose:** Multi-level, budget-aware internal demand workflow (`Draft` → `In Progress` (multi-stage approval, with Send-Back / Reject branches) → `Approved` → `Completed`, or `Voided` at any point) that hands a vendor-allocated requirement to procurement &nbsp;·&nbsp; **Audience:** Requestor, Department Head, Budget Controller, Finance, Purchaser, Procurement Manager, Auditor &nbsp;·&nbsp; **Key entities/tables:** `tb_purchase_request`, `tb_purchase_request_detail`, approval history, pricelist allocation, [purchase-request/my-approval](/en/inventory/purchase-request/my-approval) &nbsp;·&nbsp; **Sub-pages:** 15
...
The PR document status (`tb_purchase_request.pr_status`, `enum_purchase_request_doc_status`) is workflow-driven: `draft` (editable by the requestor, no budget or stock impact) → `in_progress` on submit (a soft budget commitment is created; the PR stays `in_progress` while it is routed through every intermediate stage of the approval chain, including any Send-Back-and-resubmit loop back to `draft`) → `approved` once the final approval stage clears (cleared for procurement and available for conversion to a purchase order) → `completed` once every line has been converted to a purchase order. A header-level **Reject** from any approver, or an administrative **Void**, terminates the chain immediately and moves the PR to `voided` — there is no separate `rejected` status; rejection and void converge on the same terminal state. Approval is **multi-level** and amount-driven — typically department head first, then budget controller, then finance review for high-value PRs, then final procurement sign-off, with a configurable per-workflow routing rule that can skip or escalate a stage once the header total crosses a threshold. *(A "delegation-of-authority" mechanism to keep the chain moving when an approver is unavailable was asserted in an earlier revision — unconfirmed; no such mechanism was found in the workflow engine, see [02-business-rules.md](/en/inventory/purchase-request/02-business-rules) `PR_AUTH_006`.)* Once submitted, a PR cannot be cancelled directly by the requestor; termination only happens through the workflow reject / void path, preserving the audit trail.
...
- **Approval Level**: A configurable stage in the PR workflow with an assigned approver role and amount threshold — the workflow's routing rules can skip or jump to a stage once the header total (or department / category) crosses a configured value. Typical levels are department head (mandatory for every PR), budget controller (validates against budget availability), finance review (mandatory for high-value PRs above a configured threshold), and final procurement sign-off. The entire path is recorded on the activity log. *(Deputy-delegation of approvals when the primary approver is unavailable was asserted in an earlier revision — unconfirmed, no such mechanism found; see `PR_AUTH_006`.)*
...
| System Administrator | Configures workflow stages and approval-routing thresholds (generic `/system-admin/workflow` Routing panel), tax codes, currency rates, and integration with budget, inventory, vendor, and PO modules. Manages user roles and permissions. *(Delegation rules and PR-type defaults were asserted in an earlier revision — unconfirmed, no matching screen or mechanism found; see `PR_AUTH_006`.)* |
