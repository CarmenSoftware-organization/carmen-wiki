...
4. **Check vendor pricelist deviation.** The screen renders the GRN line's effective unit price beside the active `[[vendor-pricelist]]` price for the vendor / item / receipt-date window; the price-variance flag fires when the GRN price drifts by more than the tenant tolerance. (Department Manager subset reviews the same panel scoped to lines on the department's cost-centre.)
