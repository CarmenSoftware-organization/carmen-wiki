...
- Related modules: [[purchase-order]] (upstream source via `purchase_order_detail_id`), [[purchase-request]] (origin, reached through the PO↔PR bridge id stored on the detail_item), [[inventory]] (downstream — the inventory transaction is where lot, expiry, and cost-layer data live), [[costing]] (FIFO / average-cost layer creation on commit), [[inventory-adjustment]] (post-commit corrections), [[vendor-pricelist]] (price-variance check against the GRN unit price), [[product]] (per-line product reference).
