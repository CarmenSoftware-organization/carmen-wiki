...
- Related modules: [[inventory]] (downstream — the inventory transaction is where lot, expiry, and cost-layer data live), [[costing]] (source-location weighted-average or FIFO valuation feeds the issued unit cost), [[recipe]] (recipe demand may auto-generate SRs for ingredient pulls), [[good-receive-note]] (inter-location transfers can pair an SR-OUT at the source warehouse with a GRN-IN at the destination warehouse), [[inventory-adjustment]] (post-commit corrections), [[product]] (per-line product reference).
