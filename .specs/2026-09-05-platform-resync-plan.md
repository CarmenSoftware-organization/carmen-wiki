...
One caution. **The byte sizes of the two locales are not comparable and must not be read as content volume** — Thai in UTF-8 runs about three bytes per character against one for ASCII, so a TH page routinely weighs half again as much as its EN twin while saying exactly the same thing. (This instruction originally cited 17.9 KB against 11.3 KB as evidence of a TH content surplus; the two files were 78 lines each, section for section identical. Compare lines or sections, never bytes.) Migrate each locale into its own page rather than regenerating TH from EN. Second, after migrating, reduce the sub-page to a short pointer keeping only the report-template-specific relationship, and rewrite any sentence whose meaning depended on it holding the full account. `report-templates.md` already links to it as a sub-page; that link must move to this module.
