...
6. **source_params binding** (or equivalent parameter binding section) — when the module links data-format fields to data-source parameters (e.g. Dialog XML filter names → `source_params` positional args), document the binding convention here. Placement is flexible: a binding tightly coupled to one format (as in `report-templates`, where binding is `Dialog`-only) may live as a sub-section of Format A instead (`§2.N`) — the reference implementation `en/platform/report-templates/xml-spec.md` placed it at §2.4 rather than as a standalone §6, and that is acceptable. Choose by whether the binding generalizes across formats.
